...
function frame(n,title,c){return `<div class="willena-pdf-frame"></div><div class="willena-pdf-header"><div class="willena-pdf-brand-logo-wrap"><img class="willena-pdf-brand-logo" src="/Assets/Images/color-logo1.png" alt="Willena English"></div><div class="willena-pdf-subbrand">STUDENT LEVEL REPORT</div><div class="willena-pdf-student"><strong>${esc(c.student_name||'Student')}</strong><span>${esc([c.school_name,c.school_grade].filter(Boolean).join(' · '))}</span></div></div><div class="willena-pdf-title-line"><div class="willena-pdf-pill">${title}</div></div><div class="willena-pdf-footer"><span>WILLENA ENGLISH · PLACEMENT REPORT</span><b>${String(n).padStart(2,'0')}</b></div>`}
...
function skillBlock(s,ko){const r=estimate(s),label=labels[s][ko?'ko':'en'];if(!r.assessed){const teacherAssessed=s==='speaking'||s==='writing',note=teacherAssessed?(ko?'이 영역은 원어민 원장님이 직접 평가합니다.':'This skill will be assessed by the native-speaking director.'):(ko?'이 영역은 최소 세 문항이 필요합니다.':'At least three questions are required.');return `<div class="willena-pdf-skill unassessed"><div class="willena-pdf-skill-head"><strong>${label}</strong><span>${teacherAssessed?(ko?'원어민 원장님 평가':'Director assessment'):(ko?'평가되지 않음':'Not assessed')}</span></div><p>${note}</p></div>`}return `<div class="willena-pdf-skill"><div class="willena-pdf-skill-head"><strong>${label}</strong><span>${name(r.level,ko)}${r.plus?'+':''}</span></div><div class="willena-pdf-segments">${skillPips(r.level)}</div><p>${esc(text(profile(r.level,s),'summary',ko,fallback(r.level,s,ko)))}</p></div>`}
...
function build(){const ko=document.documentElement.lang!=='en',best=overall(),c=data.candidate||{},currentTitle=ko?'현재 레벨':'Current level',stage=document.createElement('div');stage.className='willena-pdf-stage';stage.innerHTML=`<section class="willena-pdf-page">${frame(1,'PLACEMENT SUMMARY',c)}<div class="willena-pdf-hero-label">${ko?'추천 시작 레벨':'Recommended starting level'}</div><div class="willena-pdf-orbit"><span>${best<=2?(ko?'스타터':'STARTER'):(ko?'레벨':'LEVEL')}</span><strong>${display(best)}</strong></div>${hJourney(best,ko)}<div class="willena-pdf-one-card"><article class="willena-pdf-card willena-pdf-card-full"><div class="willena-pdf-bisect">${currentTitle}</div><p>${pathwaySummary(best,ko)}</p></article></div></section><section class="willena-pdf-page">${frame(2,'SKILL PROFILE',c)}<h2 class="willena-pdf-heading">${ko?'영역별 레벨':'Skill levels'}</h2><div class="willena-pdf-skills">${all.map(s=>skillBlock(s,ko)).join('')}</div></section>`;document.body.appendChild(stage);return stage}
...
async function download(){
  ...
  try{
    ...
    stage=build();
    await Promise.all([...stage.querySelectorAll('img')].map(img=>img.complete?Promise.resolve():new Promise(resolve=>{img.addEventListener('load',resolve,{once:true});img.addEventListener('error',resolve,{once:true})})));
