...
function hJourney(best,ko){const label=ko?`${best<=2?'스타터':name(best,true)} 배치 결과`:`${best<=2?'Starter':name(best,false)} placement result`;return `<div class="willena-pdf-placement-pips" role="img" aria-label="${label}">${skillPips(best)}</div>`}
