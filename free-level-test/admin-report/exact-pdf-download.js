...
function skillBlock(s,ko){const r=estimate(s),label=labels[s][ko?'ko':'en'];if(!r.assessed){const note=s==='speaking'||s==='writing'?(ko?'현재 버전의 테스트에는 이 영역을 평가할 문항이 충분하지 않습니다.':'This version does not include enough tasks to assess this skill.'):(ko?'이 영역은 최소 세 문항이 필요합니다.':'At least three questions are required.');return `<div class="willena-pdf-skill unassessed"><div class="willena-pdf-skill-head"><strong>${label}</strong><span>${ko?'평가되지 않음':'Not assessed'}</span></div><p>${note}</p></div>`}return `<div class="willena-pdf-skill"><div class="willena-pdf-skill-head"><strong>${label}</strong><span>${name(r.level,ko)}${r.plus?'+':''}</span></div><div class="willena-pdf-segments">${skillPips(r.level)}</div><p>${esc(text(profile(r.level,s),'summary',ko,fallback(r.level,s,ko)))}</p></div>`}
