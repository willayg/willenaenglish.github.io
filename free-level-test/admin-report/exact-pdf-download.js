...
function vJourney(best,ko){const lv=pathwayFor(best),idx=lv.indexOf(best);return lv.map((l,i)=>{const current=l===best,badge=current?(ko?'추천 시작 레벨':'Recommended level'):i<idx?(ko?'편안한 대안':'Comfortable alternative'):(ko?'도전 가능한 레벨':'Challenge level');return `<div class="willena-pdf-vrow ${current?'current':'adjacent'}"><div class="willena-pdf-vcard"><div class="willena-pdf-vbadge">${badge}</div><h3>${name(l,ko)}</h3><p>${esc(summary(l,ko))}</p></div></div>`}).join('')}
