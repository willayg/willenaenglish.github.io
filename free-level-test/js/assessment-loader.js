...
export async function loadQuestionBank(){
  const params=new URLSearchParams({
    select:"source_key,level_id,difficulty_rating,item_type,prompt_text,correct_answer,choices",
    status:"eq.published",
  ...
  const bank=Array.isArray(rows)?rows.map(normalise).filter(Boolean):[];
