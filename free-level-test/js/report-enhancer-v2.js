...
function snapshotAnswer(){
 ...
 return{choices,selectedValue,selectedTokens,text:clean(card.textContent)};
}
async function recordAnswer(snapshot){
 ...
 }else if(snapshot.selectedTokens.length){
  q=bank.find(item=>item.type==='sentence_unscramble'&&sameSet([...item.tokens].map(clean),snapshot.selectedTokens))||null;
 }
 if(!q||evidence.some(x=>x.id===q.id))return;
 if(q.type==='sentence_unscramble'){
  const normal=s=>clean(s).toLowerCase().replace(/[.,!?;:“”"()]/g,'');
  const correct=Array.isArray(q.tokens)&&snapshot.selectedTokens.length===q.tokens.length&&snapshot.selectedTokens.every((x,i)=>normal(x)===normal(q.tokens[i]));
  evidence.push({id:q.id,level:Number(q.level)||1,type:q.type,correct});return;
 }
...
document.addEventListener('click',e=>{
 if(e.target.closest('#next')){const snapshot=snapshotAnswer();recordAnswer(snapshot)}
...
function levelFromRows(rows){
 ...
 const highest=Math.min(MAX_LEVEL,Math.max(...rows.map(x=>Number(x.level)||1),1));
