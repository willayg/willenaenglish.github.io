import{loadQuestionBank}from'./assessment-loader.js?v=20260801-2';
...
loadQuestionBank().then(items=>bank=items).catch(error=>console.warn('Could not load report question bank',error));
...
function recordAnswer(){
 ...
 if(q.type==='sentence_unscramble'){
  ...
  const correct=Array.isArray(q.tokens)&&selected.length===q.tokens.length&&selected.every((x,i)=>normal(x)===normal(q.tokens[i]));
