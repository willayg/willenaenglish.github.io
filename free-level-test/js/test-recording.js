...
var answers=[],bankMap=new Map(),attempt=null,startAt=0,finalized=false,lastQuestionAt=0;
...
function ensureBank(){if(typeof window.loadQuestionBank!=='function')return Promise.resolve();return window.loadQuestionBank().then(function(rows){rows.forEach(function(q){bankMap.set(String(q.id),q)})}).catch(function(){})}
...
function captureAnswer(){var card=document.querySelector('.question-card');if(!card)return;var id=card.getAttribute('data-question-id');if(!id)return;var q=bankMap.get(String(id));if(!q)return;var selected=selectedValue(card,q);if(selected==null)return;var row={answer_index:answers.length+1,assessment_item_id:String(q.id),assessment_source_key:q.metadata&&q.metadata.source_key||null,question_level:Number(q.level)||null,item_type:q.type||null,prompt_snapshot:q.q||q.meaning||'',selected_answer:selected,correct_answer:correctValue(q),is_correct:same(selected,correctValue(q)),response_time_ms:lastQuestionAt?Date.now()-lastQuestionAt:null,metadata:{translation:Boolean(q.translation)}};answers.push(row);ensureAttempt().then(function(a){return post(Object.assign({action:'answer',attempt_id:a.id,session_token:a.session_token},row))}).catch(function(e){console.warn('[level-test-recording] answer save failed',e)})}
function parseLevel(){var text=document.body.innerText||'',m=text.match(/(?:Level|레벨|단계)\s*(\d{1,2})/i);return m?Number(m[1]):null}
function finishIfReady(){if(finalized||!attempt||!answers.length)return;var retry=document.querySelector('#retry'),home=document.querySelector('#home');if(!retry&&!home)return;finalized=true;var level=parseLevel();post({action:'finish',attempt_id:attempt.id,session_token:attempt.session_token,answers:answers,recommended_level:level,display_level:level,duration_seconds:startAt?Math.round((Date.now()-startAt)/1000):null,metadata:{completed_from:'browser-recorder',page_language:document.documentElement.lang||'ko'}}).then(function(){clearAttempt()}).catch(function(e){finalized=false;console.warn('[level-test-recording] finish save failed',e)})}
document.addEventListener('click',function(e){if(e.target.closest('#next'))captureAnswer();if(e.target.closest('#retry')||e.target.closest('#home')){answers=[];finalized=false;clearAttempt()}},true);
var observer=new MutationObserver(function(){var card=document.querySelector('.question-card');if(card)lastQuestionAt=Date.now();finishIfReady()});observer.observe(document.documentElement,{subtree:true,childList:true});
