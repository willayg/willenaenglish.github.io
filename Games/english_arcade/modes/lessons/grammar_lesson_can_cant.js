...
export async function runGrammarLessonCanCant(ctx = {}) {
  ...
  function renderExplainStep(stepEl, progress) {
    ...
    intro.innerHTML = lang === 'ko'
      ? "<b>can</b>?Ä '~?????àÎã§', <b>can't</b>??'~?????ÜÎã§'???ªÏù¥?êÏöî. Î≤ÑÌäº???åÎü¨???àÏ†úÎ•??ïÏù∏??Î≥¥ÏÑ∏??"
  ...
  function renderExamplesStep(stepEl, progress) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = lang === 'ko'
      ? "<b>Can</b> ???•Î†• ?àÏùå ??br/><b>Can't</b> ???•Î†• ?ÜÏùå ??br/>Î≤ÑÌäº???åÎü¨????ÎßéÏ? ?àÏ†úÎ•??ïÏù∏?òÏÑ∏??"
  ...
  function renderSortingStep(stepEl, progress) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = (lang === 'ko')
      ? '?ôÎ¨º???åÎü¨ <b>Can Fly</b> ?êÎäî <b>Can\'t Fly</b> Î∞îÍµ¨?àÏóê ?£Ïñ¥ Î≥¥ÏÑ∏?? Î™®Îëê ÎßûÏúºÎ©??§Ïùå ?®Í≥ÑÎ°??òÏñ¥Í∞????àÏñ¥??'
    ...
    const pool = makeBucket('pool', lang === 'ko' ? '?ôÎ¨º Î™®Ïùå' : 'Animals');
    const canBucket = makeBucket('can', lang === 'ko' ? 'Can Fly ?¶Ö' : 'Can Fly ?¶Ö');
    const cantBucket = makeBucket('cant', lang === 'ko' ? 'Can\'t Fly ?êò' : 'Can\'t Fly ?êò');
    ...
    checkBtn.onclick = () => {
      ...
      if (allCorrect) {
        ...
        message.textContent = lang === 'ko' ? '?ÑÎ≤Ω?¥Ïöî! ?????àÎäî ?ôÎ¨º?§ÏùÑ ??Í≥®Îûê?¥Ïöî.' : 'Perfect! You sorted the flying animals correctly.';
        ...
        message.textContent = lang === 'ko' ? 'Îπ®Í∞Ñ ?ôÎ¨º???§Ïãú ??≤® Î≥¥ÏÑ∏??' : 'Try again! Fix the red animals.';
        stepEl.insertBefore(message, stepEl.firstChild);
      }
    };
  }
  ...
  function renderFinishStep(stepEl) {
    ...
    body.innerHTML = (lang === 'ko')
      ? '<div style="font-weight:800;color:#19777e">?¥Ï†ú can/can\'tÎ•?Î∞îÎ•¥Í≤??¨Ïö©?????àÏñ¥??</div><div class="stars">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>'
      : '<div style="font-weight:800;color:#19777e">You now know when to use can or can\'t!</div><div class="stars">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>';
...
function buildSortingPool(canList, cantList) {
  ...
  const flyingAnimals = [
    { id: 'bird', answer: 'can', text: '?ê¶', name: 'Bird' },
    { id: 'owl', answer: 'can', text: '?¶â', name: 'Owl' },
    { id: 'eagle', answer: 'can', text: '?¶Ö', name: 'Eagle' },
    { id: 'bat', answer: 'can', text: '?¶á', name: 'Bat' }
  ];
  
  const nonFlyingAnimals = [
    { id: 'tiger', answer: 'cant', text: '?êØ', name: 'Tiger' },
    { id: 'wolf', answer: 'cant', text: '?ê∫', name: 'Wolf' },
    { id: 'cat', answer: 'cant', text: '?ê±', name: 'Cat' },
    { id: 'elephant', answer: 'cant', text: '?êò', name: 'Elephant' }
  ];
  ...
  return [
    {
      ...
      sentenceKo: canExample.exampleSentenceKo || '?òÎäî ?òÏòÅ?????àÏñ¥??',
      tipEn: "Use 'can' to show ability.",
      tipKo: "'can'?Ä ~?????àÎã§???ªÏù¥?êÏöî.",
      label: '??Can'
    },
    {
      id: 'cant',
      ...
      sentenceKo: cantExample.exampleSentenceKo || '?òÎäî ?????ÜÏñ¥??',
      tipEn: "Use 'can't' to show inability.",
      tipKo: "'can't'??~?????ÜÎã§???ªÏù¥?êÏöî.",
      label: '??Can\'t'
    }
  ];
}
...
const fallbackCan = [
  { id: 'fb_can_swim', word: 'I', exampleSentence: 'I can swim.', exampleSentenceKo: '?òÎäî ?òÏòÅ?????àÏñ¥??', emoji: '?Öüè? },
  { id: 'fb_can_read', word: 'You', exampleSentence: 'You can read.', exampleSentenceKo: '?àÎäî ?ΩÏùÑ ???àÏñ¥??', emoji: '?Öüì? },
  { id: 'fb_can_play', word: 'We', exampleSentence: 'We can play.', exampleSentenceKo: '?∞Î¶¨???Ä ???àÏñ¥??', emoji: '?Öüé? },
  { id: 'fb_can_help', word: 'He', exampleSentence: 'He can help.', exampleSentenceKo: 'Í∑∏Îäî ?ÑÏö∏ ???àÏñ¥??', emoji: '?Öüßë‚Äçüî? }
];

const fallbackCant = [
  { id: 'fb_cant_swim', word: 'He', exampleSentence: 'He can\'t swim.', exampleSentenceKo: 'Í∑∏Îäî ?òÏòÅ?????ÜÏñ¥??', emoji: '?åüè? },
  { id: 'fb_cant_fly', word: 'I', exampleSentence: 'I can\'t fly.', exampleSentenceKo: '?òÎäî ?????ÜÏñ¥??', emoji: '?åü¶? },
  { id: 'fb_cant_drive', word: 'She', exampleSentence: 'She can\'t drive.', exampleSentenceKo: 'Í∑∏Î????¥Ï†Ñ?????ÜÏñ¥??', emoji: '?åüö? },
  { id: 'fb_cant_lift', word: 'They', exampleSentence: 'They can\'t lift it.', exampleSentenceKo: 'Í∑∏Îì§?Ä ?????ÜÏñ¥??', emoji: '?åüí? }
];
