...
export async function runGrammarLessonWantWants(ctx = {}) {
  ...
  function renderExplainStep(stepEl, progress) {
    ...
    intro.innerHTML = lang === 'ko'
      ? "Ï£ºÏñ¥???∞Îùº <b>want</b> (?êÌï¥??, <b>wants</b> (?êÌï¥??Í∞Ä Î∞îÎÄåÏñ¥?? Î≤ÑÌäº???åÎü¨ ?¥Îñ§ ÎßêÏùÑ ?∞ÎäîÏßÄ ?ïÏù∏??Î≥¥ÏÑ∏??"
  ...
  function renderExamplesStep(stepEl, progress) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = lang === 'ko'
      ? "<b>???¨Îûå/Î¨ºÍ±¥</b>?Ä <b>wants</b> (?êÌï¥??<br/><b>?¨Îü¨ ?¨Îûå/Î¨ºÍ±¥</b>?Ä <b>want</b> (?êÌï¥??<br/>Î≤ÑÌäº???åÎü¨ ?àÏ†úÎ•??ïÏù∏??Î≥¥ÏÑ∏??"
  ...
  function renderSortingStep(stepEl, progress) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = lang === 'ko'
      ? 'Î¨∏Ïû•???åÎü¨ <b>want</b> ?êÎäî <b>wants</b> Î∞îÍµ¨?àÏóê ?£Ïñ¥ Î≥¥ÏÑ∏?? Î™®Îëê ÎßûÏúºÎ©??§Ïùå ?®Í≥ÑÎ°??òÏñ¥Í∞????àÏñ¥??'
  ...
  buckets.className = 'amareis-buckets buckets-two';
    ...
    const wantBucket = makeBucket('want', lang === 'ko' ? 'want (?êÌï¥??' : 'want');
    const wantsBucket = makeBucket('wants', lang === 'ko' ? 'wants (?êÌï¥??' : 'wants');
    ...
    checkBtn.onclick = () => {
      ...
      if (allCorrect) {
        ...
        message.textContent = lang === 'ko' ? '?ÑÎ≤Ω?¥Ïöî! Want?Ä WantsÎ•???Íµ¨Î∂Ñ?àÏñ¥??' : 'Great job! You know want vs. wants!';
  ...
  function renderFinishStep(stepEl) {
    ...
    congratsMsg.textContent = lang === 'ko'
      ? 'Ï∂ïÌïò?¥Ïöî! Want?Ä WantsÎ•?Î∞∞Ïõ†?¥Ïöî! ?éâ'
      : 'Congratulations! You learned want vs. wants! ?éâ';
    stepEl.appendChild(congratsMsg);

    const starsDiv = document.createElement('div');
    starsDiv.style.cssText = 'font-size:3rem;';
    starsDiv.textContent = '‚≠ê‚≠ê‚≠ê‚≠ê‚≠?;
  ...
  return [
    {
      id: 'want',
      verb: 'want',
      emoji: wantExample.emoji || '?í≠',
      sentenceEn: wantExample.exampleSentence || 'I want a toy.',
      sentenceKo: wantExample.exampleSentenceKo || '?òÎäî ?•ÎÇúÍ∞êÏùÑ ?êÌï¥??',
      tipEn: "Use 'want' with I, you, we, they, or plural nouns.",
      tipKo: "I, you, we, they Í∑∏Î¶¨Í≥?Î≥µÏàò Î™ÖÏÇ¨?Ä 'want'Î•??®Ïöî.",
      label: 'I / You / We / They'
    },
    {
      id: 'wants',
      verb: 'wants',
      emoji: wantsExample.emoji || '?éÅ',
      sentenceEn: wantsExample.exampleSentence || 'He wants a bicycle.',
      sentenceKo: wantsExample.exampleSentenceKo || 'Í∑∏Îäî ?êÏ†ÑÍ±∞Î? ?êÌï¥??',
      tipEn: "Use 'wants' with he, she, it, or one person or thing.",
      tipKo: "he, she, it Í∑∏Î¶¨Í≥????¨Îûå/Î¨ºÍ±¥Í≥?'wants'Î•??®Ïöî.",
      label: 'He / She / It'
    }
  ];
}
...
const fallbackWant = [
  { id: 'fb_want_i', word: 'I', prompt: 'I ___ a toy.', exampleSentence: 'I want a toy.', exampleSentenceKo: '?òÎäî ?•ÎÇúÍ∞êÏùÑ ?êÌï¥??', emoji: '?ß∏' },
  { id: 'fb_want_we', word: 'We', prompt: 'We ___ to play.', exampleSentence: 'We want to play.', exampleSentenceKo: '?∞Î¶¨???ÄÍ≥??∂Ïñ¥??', emoji: '?? },
  { id: 'fb_want_they', word: 'They', prompt: 'They ___ ice cream.', exampleSentence: 'They want ice cream.', exampleSentenceKo: 'Í∑∏Îì§?Ä ?ÑÏù¥?§ÌÅ¨Î¶ºÏùÑ ?êÌï¥??', emoji: '?ç¶' },
  { id: 'fb_want_kids', word: 'The kids', prompt: 'The kids ___ candy.', exampleSentence: 'The kids want candy.', exampleSentenceKo: '?ÑÏù¥?§Ï? ?¨ÌÉï???êÌï¥??', emoji: '?ç¨' }
];

const fallbackWants = [
  { id: 'fb_wants_he', word: 'He', prompt: 'He ___ a bike.', exampleSentence: 'He wants a bike.', exampleSentenceKo: 'Í∑∏Îäî ?êÏ†ÑÍ±∞Î? ?êÌï¥??', emoji: '?ö≤' },
  { id: 'fb_wants_she', word: 'She', prompt: 'She ___ a book.', exampleSentence: 'She wants a book.', exampleSentenceKo: 'Í∑∏Î???Ï±ÖÏùÑ ?êÌï¥??', emoji: '?ìö' },
  { id: 'fb_wants_it', word: 'It', prompt: 'It ___ water.', exampleSentence: 'It wants water.', exampleSentenceKo: 'Í∑∏Í≤É?Ä Î¨ºÏùÑ ?êÌï¥??', emoji: '?ê±' },
  { id: 'fb_wants_cat', word: 'My cat', prompt: 'My cat ___ to sleep.', exampleSentence: 'My cat wants to sleep.', exampleSentenceKo: '??Í≥†Ïñë?¥Îäî ?êÍ≥† ?∂Ïñ¥??', emoji: '?ò¥' }
];
