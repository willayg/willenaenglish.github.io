...
export async function runGrammarLessonAmAreIs(ctx = {}) {
  ...
  function renderIntroStep(stage, prog, stepEl) {
    ...
    intro.innerHTML = lang === 'ko'
      ? "Ï£ºÏñ¥???∞Îùº <b>am</b> (?¥Îã§), <b>is</b> (?¥Îã§), <b>are</b> (?¥Îã§)Í∞Ä Î∞îÎÄåÏñ¥?? Î≤ÑÌäº???åÎü¨???¥Îñ§ ÎßêÏùÑ ?∞ÎäîÏßÄ ?ïÏù∏??Î≥¥ÏÑ∏??"
  ...
  function renderExamplesStep(stage, prog, stepEl) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = lang === 'ko'
      ? "?ºÏ™ΩÎ∂Ä??<b>am</b> (?¥Îã§), <b>is</b> (?¥Îã§), <b>are</b> (?¥Îã§) ?àÎ¨∏?¥Ïóê?? ?åÎ¶¨Î•??¥Ïñ¥ ?ΩÏñ¥ Î≥¥ÏÑ∏??"
  ...
  function renderSortingStep(stage, prog, stepEl) {
    ...
    body.innerHTML = lang === 'ko'
      ? "Î¨∏Ïû•???åÎü¨??<b>am</b> (?¥Îã§), <b>is</b> (?¥Îã§), <b>are</b> (?¥Îã§) Î∞îÍµ¨?àÏóê ?£Ïñ¥ Î≥¥ÏÑ∏?? Î™®Îëê ÎßûÏúºÎ©??§Ïùå?ºÎ°ú Í∞????àÏñ¥??"
    ...
    checkBtn.onclick = () => {
      ...
      if (poolCount === 0 && wrongCount === 0) {
        ...
        message.textContent = lang === 'ko' ? "?ÑÎ≤Ω?¥Ïöî! am (?¥Îã§), is (?¥Îã§), are (?¥Îã§)Î•???Í≥®Îûê?¥Ïöî." : 'Perfect! You matched am, is, and are.';
        ...
        message.textContent = lang === 'ko' ? 'Îπ®Í∞Ñ Ïπ¥ÎìúÎ•??§Ïãú ??≤® Î≥¥ÏÑ∏??' : 'Try again! Fix the red cards.';
        stepEl.insertBefore(message, stepEl.firstChild);
      }
    };
  }
  ...
  function renderFinishStep(stage, prog, stepEl) {
    ...
    body.style.gap = '30px';
    body.innerHTML = lang === 'ko'
      ? '<div style="font-weight:800;color:#19777e">?¥Ï†ú am (?¥Îã§) / is (?¥Îã§) / are (?¥Îã§)Î•?Î∞îÎ•¥Í≤??????àÏñ¥??</div><div class="stars">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>'
      : '<div style="font-weight:800;color:#19777e">You now know when to use am, is, or are!</div><div class="stars">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>';
    stepEl.appendChild(body);
  ...
  function buildNavRow(onBack, onNext, currentLang) {
    ...
    const back = buildPrimaryButton(currentLang === 'ko' ? '?§Î°ú' : 'Back');
...
function buildExampleColumn(verb, list, lang) {
  ...
  titleWrap.textContent = verb === 'am'
    ? (lang === 'ko' ? "am (?¥Îã§ - I?Ä ?®Íªò)" : 'am ??with I')
    : verb === 'is'
      ? (lang === 'ko' ? "is (?¥Îã§ - ???¨Îûå/Î¨ºÍ±¥)" : 'is ??one person or thing')
      : (lang === 'ko' ? "are (?¥Îã§ - ?¨Îüø/you)" : 'are ??many or you');
  ...
  return [
    {
      ...
      sentenceKo: amExample.exampleSentenceKo || '?òÎäî ?âÎ≥µ?¥Ïöî.',
    ...
    },
    {
      id: 'is',
      verb: 'is',
      emoji: isExample.emoji || '?ßí',
      sentenceEn: isExample.exampleSentence || 'She is on the bus.',
      sentenceKo: isExample.exampleSentenceKo || 'Í∑∏Î???Î≤ÑÏä§???àÏñ¥??',
      tipEn: "Use 'is' with he, she, it, or one noun.",
      tipKo: "he, she, it, Í∑∏Î¶¨Í≥??®Ïàò Î™ÖÏÇ¨??'is'Î•??®Ïöî.",
      label: 'He / She / It'
    },
    {
      id: 'are',
      ...
      sentenceKo: areExample.exampleSentenceKo || 'Í∑∏Îì§?Ä ?ÄÍ≥??àÏñ¥??',
      tipEn: "Use 'are' with we, you, they, or plural nouns.",
      tipKo: "we, you, they ?êÎäî Î≥µÏàò Î™ÖÏÇ¨?êÎäî 'are'Î•??®Ïöî.",
      label: 'We / You / They'
    }
  ];
}
...
const fallbackAm = [
  { id: 'fallback_am_1', word: 'I', prompt: 'I ___ happy.', article: 'am', emoji: '??', exampleSentence: 'I am happy.', exampleSentenceKo: '?òÎäî ?âÎ≥µ?¥Ïöî.', explanation: "Use 'am' with I.", explanationKo: "I??'am'???®Ïöî." },
  { id: 'fallback_am_2', word: 'I', prompt: 'I ___ from Korea.', article: 'am', emoji: '?á∞?á∑', exampleSentence: 'I am from Korea.', exampleSentenceKo: '?òÎäî ?úÍµ≠?êÏÑú ?îÏñ¥??', explanation: "'am' links I with facts.", explanationKo: "I ???ïÎ≥¥?êÎäî 'am'." }
];

const fallbackIs = [
  { id: 'fallback_is_1', word: 'She', prompt: 'She ___ on the bus.', article: 'is', emoji: '?öå', exampleSentence: 'She is on the bus.', exampleSentenceKo: 'Í∑∏Î???Î≤ÑÏä§???àÏñ¥??', explanation: "Use 'is' with one person.", explanationKo: "???¨Îûå?Ä 'is'." },
  { id: 'fallback_is_2', word: 'The cat', prompt: 'The cat ___ sleepy.', article: 'is', emoji: '?ê±', exampleSentence: 'The cat is sleepy.', exampleSentenceKo: 'Í∑?Í≥†Ïñë?¥Îäî Ï°∏Î†§??', explanation: "One animal uses 'is'.", explanationKo: "?ôÎ¨º ?òÎÇò??'is'." }
];

const fallbackAre = [
  { id: 'fallback_are_1', word: 'They', prompt: 'They ___ ready.', article: 'are', emoji: '?ë´', exampleSentence: 'They are ready.', exampleSentenceKo: 'Í∑∏Îì§?Ä Ï§ÄÎπÑÎêê?¥Ïöî.', explanation: "Use 'are' with they.", explanationKo: "they?êÎäî 'are'." },
  { id: 'fallback_are_2', word: 'We', prompt: 'We ___ in class.', article: 'are', emoji: '?è´', exampleSentence: 'We are in class.', exampleSentenceKo: '?∞Î¶¨??ÍµêÏã§???àÏñ¥??', explanation: "Groups use 'are'.", explanationKo: "?¨Îüø?¥Î©¥ 'are'." }
];
