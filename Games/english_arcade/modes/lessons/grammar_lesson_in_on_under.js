...
export async function runGrammarLessonInOnUnder(ctx = {}) {
  ...
  function renderIntroStep(stage, prog, stepEl) {
    ...
    intro.innerHTML = lang === 'ko'
      ? "Î¨ºÍ±¥???ÑÏπòÎ•??òÌ?????<b>in</b> (?àÏóê), <b>on</b> (?ÑÏóê), <b>under</b> (?ÑÎûò??Î•??®Ïöî. Î≤ÑÌäº???åÎü¨???¥Îñ§ ?ÑÏπò?∏Ï? ?ïÏù∏??Î≥¥ÏÑ∏??"
    ...
    const animals = [
      { emoji: '?ê±', en: 'cat', ko: 'Í≥†Ïñë?? },
      { emoji: '?ê∂', en: 'dog', ko: 'Í∞? },
      { emoji: '?êµ', en: 'monkey', ko: '?êÏà≠?? },
      { emoji: '?ê∞', en: 'rabbit', ko: '?†ÎÅº' },
      { emoji: '?êª', en: 'bear', ko: 'Í≥? }
    ];
    ...
    const renderCard = (set) => {
      ...
      const sentenceKo = `${animal.ko}Í∞Ä ?ÅÏûê ${set.prep === 'in' ? '?àÏóê' : set.prep === 'on' ? '?ÑÏóê' : '?ÑÎûò??} ?àÏñ¥??`;
  ...
  function renderExamplesStep(stage, prog, stepEl) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = lang === 'ko'
      ? "?ºÏ™ΩÎ∂Ä??<b>in</b> (?àÏóê), <b>on</b> (?ÑÏóê), <b>under</b> (?ÑÎûò?? ?àÎ¨∏?¥Ïóê?? ?åÎ¶¨Î•??¥Ïñ¥ ?ΩÏñ¥ Î≥¥ÏÑ∏??"
  ...
  function renderSortingStep(stage, prog, stepEl) {
    ...
    body.innerHTML = lang === 'ko'
      ? "Ïπ¥ÎìúÎ•??ÑÎ•¥Í≥??ÅÏûê???¨Î∞îÎ•??ÑÏπò???ìÏúº?∏Ïöî."
    ...
    poolLabel.textContent = lang === 'ko' ? 'Ïπ¥Îìú?? : 'Cards';
    ...
    const checkAnswers = () => {
      ...
      message.textContent = lang === 'ko' ? '?ÑÎ≤Ω?¥Ïöî! ?ÑÏπò ?úÌòÑ?????¥Ìï¥?àÏñ¥??' : 'Perfect! You understand in, on, and under!';
      sortingContainer.appendChild(message);

      const nav = document.createElement('div');
      nav.className = 'lesson-nav';
      nav.style.marginTop = '20px';
      const continueBtn = buildPrimaryButton(lang === 'ko' ? '?§Ïùå ?®Í≥ÑÎ°? : 'Next');
  ...
  function renderFinishStep(stage, prog, stepEl) {
    ...
    body.style.gap = '30px';
    body.innerHTML = (lang === 'ko')
      ? '<div style="font-weight:800;color:#19777e">?¥Ï†ú ?àÏóê, ?ÑÏóê, ?ÑÎûò?êÎ? ?¨Î∞îÎ•¥Í≤å ÎßêÌï† ???àÏñ¥??</div><div class="stars">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>'
      : '<div style="font-weight:800;color:#19777e">Now you know how to use in, on, and under!</div><div class="stars">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>';
    stepEl.appendChild(body);
...
const fallbackIn = [
  { id: 'fb_in_box', word: 'cat', article: 'in', emoji: '?ê±', exampleSentence: 'The cat is in the box.', exampleSentenceKo: 'Í≥†Ïñë?¥Í? ?ÅÏûê ?àÏóê ?àÏñ¥??' },
  { id: 'fb_in_bag', word: 'pencils', article: 'in', emoji: '?èÔ∏è', exampleSentence: 'The pencils are in the bag.', exampleSentenceKo: '?∞ÌïÑ?§Ïù¥ Í∞ÄÎ∞??àÏóê ?àÏñ¥??' },
  { id: 'fb_in_cup', word: 'juice', article: 'in', emoji: '?•§', exampleSentence: 'The juice is in the cup.', exampleSentenceKo: 'Ï£ºÏä§Í∞Ä Ïª??àÏóê ?àÏñ¥??' }
];

const fallbackOn = [
  { id: 'fb_on_table', word: 'book', article: 'on', emoji: '?ìñ', exampleSentence: 'The book is on the table.', exampleSentenceKo: 'Ï±ÖÏù¥ ?ÅÏûê ?ÑÏóê ?àÏñ¥??' },
  { id: 'fb_on_plate', word: 'cookie', article: 'on', emoji: '?ç™', exampleSentence: 'The cookie is on the plate.', exampleSentenceKo: 'Ïø†ÌÇ§Í∞Ä ?ëÏãú ?ÑÏóê ?àÏñ¥??' },
  { id: 'fb_on_head', word: 'hat', article: 'on', emoji: '?é©', exampleSentence: 'The hat is on my head.', exampleSentenceKo: 'Î™®ÏûêÍ∞Ä ??Î®∏Î¶¨ ?ÑÏóê ?àÏñ¥??' }
];

const fallbackUnder = [
  { id: 'fb_under_bed', word: 'dog', article: 'under', emoji: '?ê∂', exampleSentence: 'The dog is under the bed.', exampleSentenceKo: 'Í∞ïÏïÑÏßÄÍ∞Ä Ïπ®Î? ?ÑÎûò???àÏñ¥??' },
  { id: 'fb_under_table', word: 'ball', article: 'under', emoji: '?ΩÔ∏è', exampleSentence: 'The ball is under the table.', exampleSentenceKo: 'Í≥µÏù¥ ?ÅÏûê ?ÑÎûò???àÏñ¥??' },
  { id: 'fb_under_tree', word: 'child', article: 'under', emoji: '?ëß', exampleSentence: 'The child is under the tree.', exampleSentenceKo: '?ÑÏù¥Í∞Ä ?òÎ¨¥ ?ÑÎûò???àÏñ¥??' }
];
...
function normalizeList(list, fallback) {
  ...
  return source.map((item, idx) => ({
    ...
    exampleSentenceKo: item.exampleSentenceKo || '?àÎ¨∏',
  }));
}
  ...
  return [
    {
      ...
      tipKo: 'Î¨ºÍ±¥???àÏ™Ω???àÏùÑ ?åÎäî "in"???®Ïöî.'
    },
    {
      id: 'on',
      prep: 'on',
      ...
      tipKo: 'Î¨ºÍ±¥???ÑÏóê ?àÏùÑ ?åÎäî "on"???®Ïöî.'
    },
    {
      id: 'under',
      prep: 'under',
      ...
      tipKo: 'Î¨ºÍ±¥???ÑÎûò???àÏùÑ ?åÎäî "under"???®Ïöî.'
    }
  ];
}
