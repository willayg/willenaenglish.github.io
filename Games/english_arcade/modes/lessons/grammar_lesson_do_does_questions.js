...
export async function runGrammarLessonDoDoesQuestions(ctx = {}) {
  ...
  function renderExplainStep(stepEl, progress) {
    ...
    intro.innerHTML = lang === 'ko'
      ? "질문을 만들 때 <b>do</b> (I/you/we/they), <b>does</b> (he/she/it)를 써요. 버튼을 눌러서 예제를 확인해 보세요!"
  ...
  function renderExamplesStep(stepEl, progress) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = lang === 'ko'
      ? "<b>I/You/We/They</b> → <b>Do</b><br/><b>He/She/It</b> → <b>Does</b><br/>버튼을 눌러서 더 많은 예제를 확인하세요!"
  ...
  function renderSortingStep(stepEl, progress) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = (lang === 'ko')
      ? '주어를 눌러 <b>Do</b> 또는 <b>Does</b> 바구니에 넣어 보세요. 모두 맞으면 다음 단계로 넘어갈 수 있어요.'
    ...
    checkBtn.onclick = () => {
      ...
      if (allCorrect) {
        ...
        message.textContent = lang === 'ko' ? '완벽해요! do/does를 잘 골랐어요.' : 'Perfect! You used do/does correctly.';
  ...
  function renderFinishStep(stepEl) {
    ...
    body.innerHTML = (lang === 'ko')
      ? '<div style="font-weight:800;color:#19777e">이제 do/does 질문을 바르게 만들 수 있어요!</div><div class="stars">⭐⭐⭐⭐⭐</div>'
  ...
  return [
    {
      ...
      sentenceKo: doExample.exampleSentenceKo || '너는 피자를 좋아하니?',
      tipEn: "Use 'Do' with I, you, we, they.",
      tipKo: "I, you, we, they에 대해 'Do'를 써요.",
      label: '👥 Do'
    },
    {
      id: 'does',
      ...
      sentenceKo: doesExample.exampleSentenceKo || '그는 피자를 좋아하니?',
      tipEn: "Use 'Does' with he, she, it.",
      tipKo: "he, she, it에 대해 'Does'를 써요.",
      label: '👤 Does'
    }
  ];
}
...
const fallbackDo = [
  { id: 'fb_do_you', word: 'you', exampleSentence: 'Do you like pizza?', exampleSentenceKo: '너는 피자를 좋아하니?', emoji: '🍕' },
  { id: 'fb_do_i', word: 'I', exampleSentence: 'Do I need a pen?', exampleSentenceKo: '내가 펜이 필요하니?', emoji: '🖊️' },
  { id: 'fb_do_we', word: 'we', exampleSentence: 'Do we have class today?', exampleSentenceKo: '우리 오늘 수업 있니?', emoji: '📚' },
  { id: 'fb_do_they', word: 'they', exampleSentence: 'Do they play soccer?', exampleSentenceKo: '그들은 축구를 하니?', emoji: '⚽' }
];

const fallbackDoes = [
  { id: 'fb_does_he', word: 'he', exampleSentence: 'Does he like music?', exampleSentenceKo: '그는 음악을 좋아하니?', emoji: '🎵' },
  { id: 'fb_does_she', word: 'she', exampleSentence: 'Does she have a cat?', exampleSentenceKo: '그녀는 고양이가 있니?', emoji: '🐱' },
  { id: 'fb_does_it', word: 'it', exampleSentence: 'Does it rain today?', exampleSentenceKo: '오늘 비가 오니?', emoji: '🌧️' },
  { id: 'fb_does_john', word: 'John', exampleSentence: 'Does John play guitar?', exampleSentenceKo: '존은 기타를 치니?', emoji: '🎸' }
];
