...
export async function runGrammarLessonContractionsBe(ctx = {}) {
  ...
  function render() {
    ...
    title.textContent = grammarName || (lang === 'ko' ? 'Be ?ôÏÇ¨ Ï∂ïÏïΩ?? : 'Contractions (Be)');
  ...
  function renderLanguageStep(stepEl, nav, progress) {
    ...
    label.textContent = lang === 'ko' ? '?∏Ïñ¥Î•??†ÌÉù?òÏÑ∏?? : 'Choose your language';
  ...
  progress.textContent = formatStepLabel(stepIndex);
    ...
    body.innerHTML = lang === 'ko'
      ? '<b>Ï∂ïÏïΩ??/b>?Ä ???®Ïñ¥Î•?Î∞Ä?¥ÏÑú ???®Ïñ¥Î°?ÎßåÎìú??Í±∞Ïòà?? ?? <b>I am ??I\'m</b>'
  ...
  progress.textContent = formatStepLabel(stepIndex);
    ...
    body.textContent = lang === 'ko'
      ? '?¨Îùº?¥ÎçîÎ•??úÎûòÍ∑∏Ìï¥???®Ïñ¥Î•??®Íªò Î∞Ä?¥Î≥¥?∏Ïöî!'
  ...
  progress.textContent = formatStepLabel(stepIndex);
    ...
    body.className = 'lesson-body';
    body.textContent = lang === 'ko'
      ? "'s, 're, 'm Ï§ëÏóê??ÎßûÎäî Ï∂ïÏïΩ?ïÏùÑ Í≥®Îùº Î≥¥ÏÑ∏??"
    ...
    const basePrompt = lang === 'ko'
      ? "Ï£ºÏñ¥Î•??ÑÎ•∏ ??Ï∂ïÏïΩ?ïÏùÑ ?†ÌÉù?òÏÑ∏??"
    ...
    pronounTitle.textContent = lang === 'ko' ? 'Ï£ºÏñ¥' : 'Subjects';
    ...
    contractionTitle.textContent = lang === 'ko' ? 'Ï∂ïÏïΩ???†ÌÉù' : 'Choose Ending';
    ...
    const handlePronounClick = (btn) => {
      ...
      setStatus(lang === 'ko' ? 'Ï∂ïÏïΩ?ïÏùÑ Í≥®Îùº Î≥¥ÏÑ∏??' : 'Now pick the ending!');
    };

    const handleOptionClick = (btn) => {
      if (btn.disabled) return;
      if (!activePronoun) {
        setStatus(lang === 'ko' ? 'Î®ºÏ? Ï£ºÏñ¥Î•?Í≥®Îùº???¥Ïöî.' : 'Pick a subject first.', 'error');
        return;
      }
      ...
      if (chosen === expected) {
        ...
        const successMsg = pair
          ? (lang === 'ko'
            ? `${pair.pronoun} + ${pair.fullForm} ??${pair.contraction}! ?òÌñà?¥Ïöî!`
        ...
        setStatus(lang === 'ko' ? '?§Ïãú ?úÎèÑ??Î¥êÏöî!' : 'Oops, try again!', 'error');
        setTimeout(() => {
  ...
  progress.textContent = formatStepLabel(stepIndex);
    ...
    body.style.gap = '18px';
    body.innerHTML = lang === 'ko'
      ? '<div style="font-weight:800;color:#19777e">Ï∂ïÏïΩ?ïÏùÑ ?ÑÎ≤Ω?òÍ≤å Î∞∞Ïõ†?¥Ïöî!</div><div class="stars">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>'
      : '<div style="font-weight:800;color:#19777e">You mastered be-verb contractions!</div><div class="stars">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>';
    stepEl.appendChild(body);
  ...
  function makeNavButton(kind, currentLang, onClick) {
    ...
    if (kind === 'back') {
      btn.textContent = currentLang === 'ko' ? '?§Î°ú' : 'Back';
      btn.onclick = () => {
        onClick();
      };
    } else if (kind === 'next') {
      btn.textContent = currentLang === 'ko' ? '?§Ïùå' : 'Next';
      btn.onclick = () => {
        onClick();
      };
    } else if (kind === 'menu') {
      btn.textContent = currentLang === 'ko' ? 'Î©îÏù∏ Î©îÎâ¥' : 'Main Menu';
      btn.onclick = () => {
        onClick();
      };
    } else if (kind === 'play') {
      btn.textContent = currentLang === 'ko' ? 'Í≤åÏûÑ ?úÏûë' : "Let's Play";
      btn.onclick = () => {
        onClick();
      };
    } else {
  ...
  function makeSlider(row) {
    ...
    label.textContent = lang === 'ko'
      ? `${row.pronoun} + ${row.fullForm} Î•??©Ï≥êÎ¥êÏöî`
  ...
  contractionSpan.style.transform = 'scale(0.9)';
    ...
    tip.textContent = lang === 'ko'
      ? `?¨Îùº?¥ÎçîÎ•??ÄÏßÅÏù¥Î©?${row.contraction} ??Í∞Ä) ?òÌ??òÏöî`
...
const fallbackItems = [
  { id: 'i_am', word: 'I', pronoun: 'I', fullForm: 'am', contraction: "I'm", exampleSentence: "I'm ready to go!", exampleSentenceKo: "??Ï∂úÎ∞ú??Ï§ÄÎπÑÍ? ?êÏñ¥??", explanation: "I am becomes I'm." },
  { id: 'you_are', word: 'You', pronoun: 'You', fullForm: 'are', contraction: "You're", exampleSentence: "You're a great friend.", exampleSentenceKo: "???ïÎßê Ï¢ãÏ? ÏπúÍµ¨??", explanation: "You are becomes You're." },
  { id: 'she_is', word: 'She', pronoun: 'She', fullForm: 'is', contraction: "She's", exampleSentence: "She's playing piano.", exampleSentenceKo: "Í∑∏Î????ºÏïÑ?∏Î? ÏπòÍ≥† ?àÏñ¥??", explanation: "She is becomes She's." },
  { id: 'he_is', word: 'He', pronoun: 'He', fullForm: 'is', contraction: "He's", exampleSentence: "He's my teacher.", exampleSentenceKo: "Í∑∏Îäî ???†ÏÉù?òÏù¥?êÏöî.", explanation: "He is becomes He's." },
  { id: 'it_is', word: 'It', pronoun: 'It', fullForm: 'is', contraction: "It's", exampleSentence: "It's sunny outside.", exampleSentenceKo: "Î∞ñÏóê???¥Í? ?òÏöî.", explanation: "It is becomes It's." },
  { id: 'we_are', word: 'We', pronoun: 'We', fullForm: 'are', contraction: "We're", exampleSentence: "We're ready to learn.", exampleSentenceKo: "?∞Î¶¨??Î∞∞Ïö∏ Ï§ÄÎπÑÍ? ?êÏñ¥??", explanation: "We are becomes We're." },
  { id: 'they_are', word: 'They', pronoun: 'They', fullForm: 'are', contraction: "They're", exampleSentence: "They're playing soccer.", exampleSentenceKo: "Í∑∏Îì§?Ä Ï∂ïÍµ¨Î•??òÍ≥† ?àÏñ¥??", explanation: "They are becomes They're." }
];
