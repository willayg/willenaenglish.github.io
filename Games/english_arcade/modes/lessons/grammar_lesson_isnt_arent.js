...
export async function runGrammarLessonIsntArent(ctx = {}) {
  ...
  function renderExplainStep(stepEl, progress) {
    ...
    intro.innerHTML = lang === 'ko'
      ? "부정형 be동사를 만들 때 <b>isn't</b> (he/she/it), <b>aren't</b> (you/we/they)를 써요. 버튼을 눌러서 예제를 확인해 보세요!"
  ...
  function renderExamplesStep(stepEl, progress) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = lang === 'ko'
      ? "<b>He/She/It</b> → <b>Isn't</b><br/><b>You/We/They</b> → <b>Aren't</b><br/>버튼을 눌러서 더 많은 예제를 확인하세요!"
  ...
  function renderSortingStep(stepEl, progress) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = (lang === 'ko')
      ? '주어를 눌러 <b>Isn\'t</b> 또는 <b>Aren\'t</b> 바구니에 넣어 보세요. 모두 맞으면 다음 단계로 넘어갈 수 있어요.'
    ...
    checkBtn.onclick = () => {
      ...
      if (allCorrect) {
        ...
        message.textContent = lang === 'ko' ? '완벽해요! isn\'t/aren\'t를 잘 골랐어요.' : 'Perfect! You used isn\'t/aren\'t correctly.';
  ...
  function renderFinishStep(stepEl) {
    ...
    body.innerHTML = (lang === 'ko')
      ? '<div style="font-weight:800;color:#19777e">이제 isn\'t/aren\'t를 바르게 사용할 수 있어요!</div><div class="stars">⭐⭐⭐⭐⭐</div>'
  ...
  return [
    {
      ...
      tipKo: "he, she, it에 대해 'isn't'를 써요.",
      label: '👤 Isn\'t'
    },
    {
      id: 'arent',
      ...
      tipKo: "you, we, they에 대해 'aren't'를 써요.",
      label: '👥 Aren\'t'
    }
  ];
}
...
const fallbackIsnt = [
  ...
  { id: 'fb_isnt_working', word: 'It', exampleSentence: 'It isn\'t working.', exampleSentenceKo: '그것은 작동하지 않아요.', emoji: '⚙️' },
  { id: 'fb_isnt_ready', word: 'Tom', exampleSentence: 'Tom isn\'t ready.', exampleSentenceKo: '톰은 준비되지 않았어요.', emoji: '⏰' }
];

const fallbackArent = [
  { id: 'fb_arent_happy', word: 'We', exampleSentence: 'We aren\'t happy.', exampleSentenceKo: '우리는 행복하지 않아요.', emoji: '😊' },
  { id: 'fb_arent_ready', word: 'You', exampleSentence: 'You aren\'t ready yet.', exampleSentenceKo: '아직 준비 안 됐어요.', emoji: '⏰' },
