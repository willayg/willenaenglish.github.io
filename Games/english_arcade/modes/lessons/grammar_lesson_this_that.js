...
export async function runGrammarLessonThisThat(ctx = {}) {
  ...
  const fallbackThis = [
    { id: 'fb_this_phone', word: 'phone', emoji: '?ì±', exampleSentence: 'This is a phone.', exampleSentenceKo: '?¥Í≤É?Ä ?ÑÌôî?àÏöî.' },
    { id: 'fb_this_book', word: 'book', emoji: '?ìò', exampleSentence: 'This is a book.', exampleSentenceKo: '?¥Í≤É?Ä Ï±ÖÏù¥?êÏöî.' },
    { id: 'fb_this_cup', word: 'cup', emoji: '??, exampleSentence: 'This is a cup.', exampleSentenceKo: '?¥Í≤É?Ä ÏªµÏù¥?êÏöî.' }
  ];
  const fallbackThat = [
    { id: 'fb_that_tree', word: 'tree', emoji: '?å≥', exampleSentence: 'That is a tree.', exampleSentenceKo: '?ÄÍ≤ÉÏ? ?òÎ¨¥?àÏöî.' },
    { id: 'fb_that_kite', word: 'kite', emoji: '?™Å', exampleSentence: 'That is a kite.', exampleSentenceKo: '?ÄÍ≤ÉÏ? ?∞Ïù¥?êÏöî.' },
    { id: 'fb_that_bus', word: 'bus', emoji: '?öå', exampleSentence: 'That is a bus.', exampleSentenceKo: '?ÄÍ≤ÉÏ? Î≤ÑÏä§?àÏöî.' }
  ];
  ...
  prog.textContent = displayStep(1, lang);
    ...
    intro.innerHTML = (lang === 'ko')
      ? "Í∞ÄÍπåÏö¥ Í≤ÉÏ? <span class=\"tt-highlight\">this</span>(?¥Í≤É), Î©ÄÎ¶??àÎäî Í≤ÉÏ? <span class=\"tt-highlight\">that</span>(?ÄÍ≤??¥ÎùºÍ≥?ÎßêÌï¥?? ?¨Îùº?¥ÎçîÎ•??ÄÏßÅÏó¨ Î≥¥ÏÑ∏??"
  ...
  object.textContent = nearItem?.emoji || '?ìò';
    ...
    thisKo.textContent = '?¥Í≤É?Ä ?ÑÌôî?àÏöî.';
    const thatKo = document.createElement('div');
    thatKo.className = 'tt-label';
    thatKo.textContent = '?ÄÍ≤ÉÏ? ?ÑÌôî?àÏöî.';
  ...
  slider.value = '8';
    ...
    tip.textContent = (lang === 'ko')
      ? 'Í∞ÄÍπåÏù¥ ?åÏñ¥?§Î©¥ ?¥Í≤É, Î©ÄÎ¶?Î≥¥ÎÇ¥Î©??ÄÍ≤ÉÏúºÎ°?Î∞îÎÄåÏñ¥??'
  ...
  const blend = clamp((val - 18) / 26, 0, 1);
      ...
      const activeNear = nearItem || { exampleSentence: 'This is near me.', exampleSentenceKo: '?¥Í≤É?Ä ???ÜÏóê ?àÏñ¥??', emoji: '?ìò' };
      const activeFar = farItem || { exampleSentence: 'That is over there.', exampleSentenceKo: '?ÄÍ≤ÉÏ? ?ÄÏ™ΩÏóê ?àÏñ¥??', emoji: '?èîÔ∏? };
  ...
  prog.textContent = displayStep(2, lang);
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = (lang === 'ko')
      ? "<span class=\"tt-highlight\">this</span>(?¥Í≤É)?Ä Î∞îÎ°ú ?¨Í∏∞???àÎäî Î¨ºÍ±¥, <span class=\"tt-highlight\">that</span>(?ÄÍ≤??Ä ?ÄÏ™ΩÏóê ?àÎäî Î¨ºÍ±¥?¥Ïóê??" 
  ...
  function renderSortingStep(stage, prog, stepEl) {
    ...
    body.innerHTML = (lang === 'ko')
      ? "Î¨∏Ïû•???åÎü¨??<span class=\"tt-highlight\">this</span>(?¥Í≤É) Î∞îÍµ¨???êÎäî <span class=\"tt-highlight\">that</span>(?ÄÍ≤? Î∞îÍµ¨?àÏóê ?£Ïñ¥ Î≥¥ÏÑ∏??" 
    ...
    const bucketThis = makeBucket('this', lang === 'ko' ? 'this (?¥Í≤É)' : 'this (near me)');
    const bucketThat = makeBucket('that', lang === 'ko' ? 'that (?ÄÍ≤?' : 'that (far away)');
    ...
    checkBtn.onclick = () => {
      ...
      if (wrong === 0 && pool.body.childElementCount === 0) {
        ...
        message.textContent = lang === 'ko' ? '?ÑÎ≤Ω?¥Ïöî! ?¥Í≤ÉÍ≥??ÄÍ≤ÉÏùÑ ??Í≥®Îûê?¥Ïöî.' : 'Great job! You sorted this vs. that.';
  ...
  function renderFinishStep(stage, prog, stepEl) {
    ...
    body.style.gap = '30px';
    body.innerHTML = (lang === 'ko')
      ? '<div style="font-weight:800;color:#19777e">?¥Ï†ú Í∞ÄÍπåÏö¥ Í≤ÉÏ? ?¥Í≤É, Î®?Í≤ÉÏ? ?ÄÍ≤ÉÏù¥?ºÍ≥† ÎßêÌï† ???àÏñ¥??</div><div class="stars">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>'
      : '<div style="font-weight:800;color:#19777e">Now you know when to say this or that!</div><div class="stars">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>';
    stepEl.appendChild(body);
...
function normalizeList(list, article, fallback) {
  ...
  return source.map((item, idx) => ({
    ...
    article,
    emoji: item.emoji || (article === 'this' ? '?ìó' : '?èîÔ∏?),
    exampleSentence: item.exampleSentence || (article === 'this' ? 'This is near me.' : 'That is over there.'),
    exampleSentenceKo: item.exampleSentenceKo || (article === 'this' ? '?¥Í≤É?Ä ???ÜÏóê ?àÏñ¥??' : '?ÄÍ≤ÉÏ? ?ÄÏ™ΩÏóê ?àÏñ¥??'),
  }));
}
...
function buildExampleColumn(label, list, lang) {
  ...
  header.textContent = label === 'this'
    ? (lang === 'ko' ? 'this (?¥Í≤É)' : 'this (near)')
    : (lang === 'ko' ? 'that (?ÄÍ≤?' : 'that (far)');
...
function highlightSentence(text, lang) {
  ...
  if (lang === 'ko') {
    return clean.replace(/?¥Í≤É(?!\s)/g, '<span class="tt-highlight">?¥Í≤É</span> ').replace(/?ÄÍ≤??!\s)/g, '<span class="tt-highlight">?ÄÍ≤?/span> ');
  }
