...
export async function runGrammarLessonPluralsEs(ctx = {}) {
  ...
  function renderIntroStep(stage, prog, stepEl) {
    ...
    intro.innerHTML = lang === 'ko'
      ? "s, x, z, ch, sh로 끝나면 보통 복수에서 'es'를 붙여요!"
    ...
    highlight.innerHTML = `
      ...
      <div class="card-rule">${lang === 'ko' ? "끝이 s, x, z, ch, sh면 <strong>+es</strong>" : "Ends in s, x, z, ch, sh → add <strong>es</strong>"}</div>
      <div class="card-row">
        <div class="card-side">
      ...
      <div class="card-tip">${lang === 'ko' ? "끝 글자를 잘 보고 'es'를 붙여요!" : 'Check the ending, then add es!'}</div>
    `;
  ...
  function renderExamplesStep(stage, prog, stepEl) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = lang === 'ko'
      ? "왼쪽은 단수(하나), 오른쪽은 'es'를 붙인 복수 예문이에요. 소리 내어 읽어 보세요!"
  ...
  function renderSortingStep(stage, prog, stepEl) {
    ...
    const bucketPlu = makeBucket('plural', lang === 'ko' ? "복수 (+'es')" : "Plural (+es)");
    ...
    checkBtn.onclick = () => {
      ...
      if (poolCount === 0 && wrongCount === 0) {
        ...
        message.textContent = lang === 'ko' ? "완벽해요! 'es' 규칙을 잘 찾았어요." : "Awesome! You spotted every +es plural.";
  ...
  function renderFinishStep(stage, prog, stepEl) {
    ...
    body.style.gap = '30px';
    body.innerHTML = lang === 'ko'
      ? "<div style=\"font-weight:800;color:#19777e\">es를 붙이는 규칙을 배웠어요!</div><div class=\"stars\">⭐⭐⭐⭐⭐</div>"
      : "<div style=\"font-weight:800;color:#19777e\">You learned when to add es!</div><div class=\"stars\">⭐⭐⭐⭐⭐</div>";
    stepEl.appendChild(body);
  ...
  function buildExampleColumn(kind, list, currentLang) {
    ...
    titleWrap.textContent = kind === 'singular'
      ...
      : (currentLang === 'ko' ? "복수 (+es)" : "Plural (+es)");
...
const fallbackSingular = [
  { id: 'fb_es_bus', word: 'bus', article: 'singular', emoji: '🚌', exampleSentence: 'One bus is here.', exampleSentenceKo: '버스 한 대가 여기 있어요.' },
  { id: 'fb_es_box', word: 'box', article: 'singular', emoji: '📦', exampleSentence: 'A box is on the table.', exampleSentenceKo: '상자 하나가 탁자 위에 있어요.' },
  { id: 'fb_es_dish', word: 'dish', article: 'singular', emoji: '🍽️', exampleSentence: 'Wash one dish.', exampleSentenceKo: '그릇 하나를 씻어요.' },
  { id: 'fb_es_watch', word: 'watch', article: 'singular', emoji: '⌚', exampleSentence: 'This watch is shiny.', exampleSentenceKo: '이 시계가 반짝여요.' }
];

const fallbackPlural = [
  { id: 'fb_es_buses', word: 'buses', article: 'plural', emoji: '🚌🚌', exampleSentence: 'Many buses are here.', exampleSentenceKo: '버스들이 여기 있어요.' },
  { id: 'fb_es_boxes', word: 'boxes', article: 'plural', emoji: '📦📦', exampleSentence: 'Boxes are on the table.', exampleSentenceKo: '상자들이 탁자 위에 있어요.' },
  { id: 'fb_es_dishes', word: 'dishes', article: 'plural', emoji: '🍽️🍽️', exampleSentence: 'Wash the dishes.', exampleSentenceKo: '그릇들을 씻어요.' },
  { id: 'fb_es_watches', word: 'watches', article: 'plural', emoji: '⌚⌚', exampleSentence: 'He collects watches.', exampleSentenceKo: '그는 시계를 모아요.' }
];
