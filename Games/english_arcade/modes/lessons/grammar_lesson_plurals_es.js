// Grammar Lesson Runner ??Plurals that add "es"
// Mirrors the plurals_s lesson but explains the add-es spelling rule for kids.

import { startSession, endSession } from '../../../../students/records.js';

export async function runGrammarLessonPluralsEs(ctx = {}) {
  const { grammarFile, grammarName, playSFX } = ctx;
  const root = document.getElementById('gameArea');
  if (!root) return;

  ensureBaseStyles();
  ensurePluralsEsStyles();
  ...
  try {
    ...
    console.warn('[lesson-plurals-es] failed to load grammar list', err);
  }
  ...
  try {
    sessionId = startSession({
      mode: 'grammar_lesson_plurals_es',
      wordList: sessionWords,
      listName: grammarFile || grammarName || null,
      meta: { category: 'grammar', file: grammarFile, lesson: grammarName || 'Add ES Plurals' },
    });
  } catch (err) {
    console.debug('[plurals-es] startSession failed', err?.message);
  }
  ...
  function renderLanguageStep(stage, prog, stepEl) {
    ...
    const koBtn = buildLanguageButton('?úÍµ≠??);
  ...
  function renderIntroStep(stage, prog, stepEl) {
    ...
    intro.innerHTML = lang === 'ko'
      ? "s, x, z, ch, shÎ°??ùÎÇòÎ©?Î≥¥ÌÜµ Î≥µÏàò?êÏÑú 'es'Î•?Î∂ôÏó¨??"
      : "If a word ends with s, x, z, ch, or sh, we usually add 'es' to make it plural!";
    stepEl.appendChild(intro);

    const singularExample = singularList[0] || fallbackSingular[0];
    const pluralExample = pluralList.find((item) => item.word === `${singularExample.word || ''}es`) || pluralList[0] || fallbackPlural[0];

    const highlight = document.createElement('div');
    highlight.className = 'plurals-es-highlight-card';
    highlight.innerHTML = `
      <div class="card-heading">${lang === 'ko' ? '?àÏãúÎ°?Í∑úÏπô???¥Ìé¥Î¥êÏöî!' : 'See how the rule works!'}</div>
      <div class="card-rule">${lang === 'ko' ? "?ùÏù¥ s, x, z, ch, shÎ©?<strong>+es</strong>" : "Ends in s, x, z, ch, sh ??add <strong>es</strong>"}</div>
      <div class="card-row">
        <div class="card-side">
          <div class="card-label">${lang === 'ko' ? '?®Ïàò (1Í∞?' : 'Singular (1)'}</div>
          <div class="card-emoji">${escapeHtml(singularExample.emoji || '?öå')}</div>
          <div class="card-word">${escapeHtml(singularExample.word || 'bus')}</div>
          <div class="card-sentence">${escapeHtml(singularExample.exampleSentence || 'One bus is here.')}</div>
        ...
        <div class="card-side">
          <div class="card-label">${lang === 'ko' ? 'Î≥µÏàò (?¨Îü¨ Í∞?' : 'Plural (many)'}</div>
          <div class="card-emoji">${escapeHtml(pluralExample?.emoji || '?öå?öå')}</div>
          <div class="card-word">${escapeHtml(pluralExample?.word || 'buses')}</div>
          <div class="card-sentence">${escapeHtml(pluralExample?.exampleSentence || 'Many buses are here.')}</div>
          ${pluralExample?.exampleSentenceKo ? `<div class="card-sentence card-sentence-ko">${escapeHtml(pluralExample.exampleSentenceKo)}</div>` : ''}
        </div>
      </div>
      <div class="card-tip">${lang === 'ko' ? "??Í∏Ä?êÎ? ??Î≥¥Í≥† 'es'Î•?Î∂ôÏó¨??" : 'Check the ending, then add es!'}</div>
    `;
  ...
  function renderExamplesStep(stage, prog, stepEl) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = lang === 'ko'
      ? "?ºÏ™Ω?Ä ?®Ïàò(?òÎÇò), ?§Î•∏Ï™ΩÏ? 'es'Î•?Î∂ôÏù∏ Î≥µÏàò ?àÎ¨∏?¥Ïóê?? ?åÎ¶¨ ?¥Ïñ¥ ?ΩÏñ¥ Î≥¥ÏÑ∏??"
      : "Left shows the singular word, right shows the plural with 'es'. Read them aloud!";
    stepEl.appendChild(intro);

    const grid = document.createElement('div');
    grid.className = 'plurals-es-example-grid';
  ...
  function renderSortingStep(stage, prog, stepEl) {
    ...
    body.innerHTML = lang === 'ko'
      ? "?®Ïñ¥Î•??åÎü¨ ?®Ïàò ?êÎäî Î≥µÏàò Î∞îÍµ¨?àÏóê ?£Ïñ¥ Î≥¥ÏÑ∏?? Î™®Îëê ÎßûÌûàÎ©??§Ïùå?ºÎ°ú Í∞????àÏñ¥??"
    ...
    const pool = makeBucket('pool', lang === 'ko' ? '?®Ïñ¥ Î™®Ïùå' : 'Word Pool');
    const bucketSing = makeBucket('singular', lang === 'ko' ? '?®Ïàò' : 'Singular');
    const bucketPlu = makeBucket('plural', lang === 'ko' ? "Î≥µÏàò (+'es')" : "Plural (+es)");
    ...
    const backBtn = buildSecondaryButton(lang === 'ko' ? '?§Î°ú' : 'Back');
    ...
    checkBtn.onclick = () => {
      ...
      if (poolCount === 0 && wrongCount === 0) {
        ...
        message.textContent = lang === 'ko' ? "?ÑÎ≤Ω?¥Ïöî! 'es' Í∑úÏπô????Ï∞æÏïò?¥Ïöî." : "Awesome! You spotted every +es plural.";
        stepEl.insertBefore(message, stepEl.firstChild);

        if (!continueBtn) {
          continueBtn = buildPrimaryButton(lang === 'ko' ? '?§Ïùå ?®Í≥ÑÎ°? : 'Next Step');
        ...
        message.textContent = lang === 'ko' ? 'Îπ®Í∞Ñ Ïπ¥ÎìúÎ•??§Ïãú ??≤® Î≥¥ÏÑ∏??' : 'Move the red cards to the correct basket.';
        stepEl.insertBefore(message, stepEl.firstChild);
      }
    };
  }
  ...
  function renderFinishStep(stage, prog, stepEl) {
    ...
    body.style.gap = '30px';
    body.innerHTML = lang === 'ko'
      ? "<div style=\"font-weight:800;color:#19777e\">esÎ•?Î∂ôÏù¥??Í∑úÏπô??Î∞∞Ïõ†?¥Ïöî!</div><div class=\"stars\">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>"
      : "<div style=\"font-weight:800;color:#19777e\">You learned when to add es!</div><div class=\"stars\">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>";
    stepEl.appendChild(body);
      ...
      try {
        endSession(sessionId, {
          ...
          summary: {
            ...
            category: 'grammar',
            context: 'lesson',
            grammarName: grammarName || 'Add ES Plurals',
          },
          listName: grammarFile || grammarName || null,
          wordList: sessionWords,
        });
      } catch (err) {
        console.debug('[plurals-es] endSession failed', err?.message);
      }

      try {
        const ev = new CustomEvent('wa:session-ended', { detail: { summary: { correct: 1, total: 1, grammarName: grammarName || 'Add ES Plurals', category: 'grammar' } } });
  ...
  function render() {
    ...
    title.textContent = grammarName || 'Add ES Plurals';
  ...
  function displayStep(index) {
    const steps = [lang === 'ko' ? '?∏Ïñ¥ ?†ÌÉù' : 'Choose Language', lang === 'ko' ? '1?®Í≥Ñ' : 'Step 1', lang === 'ko' ? '2?®Í≥Ñ' : 'Step 2', lang === 'ko' ? '3?®Í≥Ñ' : 'Step 3', lang === 'ko' ? '?ÑÎ£å' : 'Complete'];
    return steps[index] || '';
  }
  ...
  function ensurePluralsEsStyles() {
    if (document.getElementById('wa-lesson-plurals-es-styles')) return;
    const st = document.createElement('style');
    st.id = 'wa-lesson-plurals-es-styles';
    st.textContent = `
      #gameArea .plurals-es-highlight-card{margin:18px auto 0;width:100%;max-width:720px;background:#ffffff;border:2px solid #d1e6f0;border-radius:20px;box-shadow:0 18px 36px -26px rgba(25,119,126,0.35);padding:22px 26px;display:flex;flex-direction:column;gap:18px}
      #gameArea .plurals-es-highlight-card .card-heading{text-align:center;font-weight:800;font-size:clamp(1.15rem,3.6vmin,1.6rem);color:#19777e}
      #gameArea .plurals-es-highlight-card .card-rule{text-align:center;font-weight:800;color:#ff6fb0;font-size:clamp(1.05rem,3.2vmin,1.35rem)}
      #gameArea .plurals-es-highlight-card .card-rule strong{color:#19777e}
      #gameArea .plurals-es-highlight-card .card-row{display:flex;flex-direction:column;gap:18px}
      @media (min-width:640px){#gameArea .plurals-es-highlight-card .card-row{flex-direction:row}}
      #gameArea .plurals-es-highlight-card .card-side{flex:1;background:linear-gradient(180deg,#f5fdff 0%,#ffffff 100%);border:2px solid rgba(33,179,190,0.22);border-radius:16px;padding:16px;display:flex;flex-direction:column;gap:8px;align-items:center;text-align:center}
      #gameArea .plurals-es-highlight-card .card-label{font-weight:800;color:#21b3be;font-size:1rem}
      #gameArea .plurals-es-highlight-card .card-emoji{font-size:2.6rem}
      #gameArea .plurals-es-highlight-card .card-word{font-size:1.4rem;font-weight:800;color:#ff6fb0}
      #gameArea .plurals-es-highlight-card .card-sentence{font-size:1.05rem;color:#27323a}
      #gameArea .plurals-es-highlight-card .card-sentence-ko{font-size:0.95rem;color:#516170;font-weight:600}
      #gameArea .plurals-es-highlight-card .card-tip{text-align:center;font-weight:700;color:#256f75;font-size:1.05rem}
      #gameArea .plurals-es-example-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:16px}
      #gameArea .plurals-es-example-card{background:#ffffff;border:2px solid #d1e6f0;border-radius:16px;padding:14px 16px;text-align:left;box-shadow:0 10px 20px -18px rgba(0,0,0,0.22);display:flex;flex-direction:column;gap:5px;font-size:0.98rem;color:#334155}
      #gameArea .plurals-es-example-card .card-title{font-weight:800;color:#19777e}
    `;
    document.head.appendChild(st);
  }
  ...
  function buildExampleColumn(kind, list, currentLang) {
    ...
    titleWrap.textContent = kind === 'singular'
      ? (currentLang === 'ko' ? '?®Ïàò (?òÎÇò)' : 'Singular (one)')
      : (currentLang === 'ko' ? "Î≥µÏàò (+es)" : "Plural (+es)");
    col.appendChild(titleWrap);

    list.slice(0, 6).forEach((item) => {
      const card = document.createElement('div');
      card.className = 'plurals-es-example-card';
      card.innerHTML = `
  ...
  function buildNavRow(onBack, onNext, currentLang, sfx) {
    ...
    const back = buildSecondaryButton(currentLang === 'ko' ? '?§Î°ú' : 'Back');
    back.onclick = () => { sfx?.('click'); onBack(); };
    const next = buildPrimaryButton(currentLang === 'ko' ? '?§Ïùå' : 'Next');
    next.onclick = () => { onNext(); };
...
const fallbackSingular = [
  { id: 'fb_es_bus', word: 'bus', article: 'singular', emoji: '?öå', exampleSentence: 'One bus is here.', exampleSentenceKo: 'Î≤ÑÏä§ ???ÄÍ∞Ä ?¨Í∏∞ ?àÏñ¥??' },
  { id: 'fb_es_box', word: 'box', article: 'singular', emoji: '?ì¶', exampleSentence: 'A box is on the table.', exampleSentenceKo: '?ÅÏûê ?òÎÇòÍ∞Ä ?ÅÏûê ?ÑÏóê ?àÏñ¥??' },
  { id: 'fb_es_dish', word: 'dish', article: 'singular', emoji: '?çΩÔ∏?, exampleSentence: 'Wash one dish.', exampleSentenceKo: 'Í∑∏Î¶á ?òÎÇòÎ•??ªÏñ¥??' },
  { id: 'fb_es_watch', word: 'watch', article: 'singular', emoji: '??, exampleSentence: 'This watch is shiny.', exampleSentenceKo: '???úÍ≥ÑÍ∞Ä Î∞òÏßù?¨Ïöî.' }
];

const fallbackPlural = [
  { id: 'fb_es_buses', word: 'buses', article: 'plural', emoji: '?öå?öå', exampleSentence: 'Many buses are here.', exampleSentenceKo: 'Î≤ÑÏä§?§Ïù¥ ?¨Í∏∞ ?àÏñ¥??' },
  { id: 'fb_es_boxes', word: 'boxes', article: 'plural', emoji: '?ì¶?ì¶', exampleSentence: 'Boxes are on the table.', exampleSentenceKo: '?ÅÏûê?§Ïù¥ ?ÅÏûê ?ÑÏóê ?àÏñ¥??' },
  { id: 'fb_es_dishes', word: 'dishes', article: 'plural', emoji: '?çΩÔ∏èüçΩÔ∏è', exampleSentence: 'Wash the dishes.', exampleSentenceKo: 'Í∑∏Î¶á?§ÏùÑ ?ªÏñ¥??' },
  { id: 'fb_es_watches', word: 'watches', article: 'plural', emoji: '?ö‚åö', exampleSentence: 'He collects watches.', exampleSentenceKo: 'Í∑∏Îäî ?úÍ≥ÑÎ•?Î™®ÏïÑ??' }
];
