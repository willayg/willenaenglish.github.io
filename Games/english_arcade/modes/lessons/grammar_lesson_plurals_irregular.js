...
// Teaches kids about irregular plural forms like mouse?ímice, person?ípeople, child?íchildren.
...
export async function runGrammarLessonPluralsIrregular(ctx = {}) {
  ...
  function renderIntroStep(stage, prog, stepEl) {
    ...
    intro.innerHTML = lang === 'ko'
      ? "?ºÎ? ?®Ïñ¥?§Ï? Î≥µÏàòÍ∞Ä ?????ÑÏ†Ñ???¨Îùº?∏Ïöî. ?¥Îü∞ ?®Ïñ¥?§ÏùÑ Î∂àÍ∑úÏπ?Î≥µÏàò?ºÍ≥† ?¥Ïöî!"
    ...
    highlight.innerHTML = `
      <div class="card-heading">${lang === 'ko' ? '?àÏãúÎ°?Í∑úÏπô???¥Ìé¥Î¥êÏöî!' : 'See how irregular plurals work!'}</div>
      <div class="card-rule">${lang === 'ko' ? "<strong>?ÑÏ†Ñ???§Î•¥Í≤?/strong> Î≥Ä?¥Ïöî" : "The word <strong>changes completely</strong>"}</div>
      <div class="card-row">
        <div class="card-side">
          <div class="card-label">${lang === 'ko' ? '?®Ïàò (1Í∞?' : 'Singular (1)'}</div>
          <div class="card-emoji">${escapeHtml(singularExample.emoji || '?ëß')}</div>
        ...
        <div class="card-side">
          ...
          <div class="card-emoji">${escapeHtml(pluralExample?.emoji || '?ëß?ë¶')}</div>
      ...
      <div class="card-tip">${lang === 'ko' ? "?®Ïñ¥Î•??∏Ïõå???¥Ïöî! Í∑úÏπô???ÜÏñ¥??" : "You have to memorize them! No pattern here."}</div>
    `;
  ...
  function renderExamplesStep(stage, prog, stepEl) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = lang === 'ko'
      ? "?ºÏ™Ω?Ä ?®Ïàò(?òÎÇò), ?§Î•∏Ï™ΩÏ? Î∂àÍ∑úÏπôÌïòÍ≤?Î≥Ä??Î≥µÏàò ?àÎ¨∏?¥Ïóê?? ?åÎ¶¨ ?¥Ïñ¥ ?ΩÏñ¥ Î≥¥ÏÑ∏??"
  ...
  function renderSortingStep(stage, prog, stepEl) {
    ...
    const bucketPlu = makeBucket('plural', lang === 'ko' ? "Î≥µÏàò (Î∂àÍ∑úÏπ?" : "Plural (irregular)");
    ...
    checkBtn.onclick = () => {
      ...
      if (poolCount === 0 && wrongCount === 0) {
        ...
        message.textContent = lang === 'ko' ? "?ÑÎ≤Ω?¥Ïöî! Î∂àÍ∑úÏπ?Î≥µÏàòÎ•?Î™®Îëê Ï∞æÏïò?¥Ïöî." : "Awesome! You spotted every irregular plural.";
  ...
  function renderFinishStep(stage, prog, stepEl) {
    ...
    body.style.gap = '30px';
    body.innerHTML = lang === 'ko'
      ? "<div style=\"font-weight:800;color:#19777e\">Î∂àÍ∑úÏπ?Î≥µÏàòÎ•?Î∞∞Ïõ†?¥Ïöî!</div><div class=\"stars\">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>"
      : "<div style=\"font-weight:800;color:#19777e\">You learned irregular plurals!</div><div class=\"stars\">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>";
    stepEl.appendChild(body);
  ...
  function buildExampleColumn(kind, list, currentLang) {
    ...
    titleWrap.textContent = kind === 'singular'
      ...
      : (currentLang === 'ko' ? "Î≥µÏàò (Î∂àÍ∑úÏπ?" : "Plural (irregular)");
...
const fallbackSingular = [
  { id: 'fb_irr_child', word: 'child', article: 'singular', emoji: '?ëß', exampleSentence: 'One child is playing.', exampleSentenceKo: '?ÑÏù¥ ??Î™ÖÏù¥ ?ÄÍ≥??àÏñ¥??' },
  { id: 'fb_irr_person', word: 'person', article: 'singular', emoji: '?ë•', exampleSentence: 'One person is here.', exampleSentenceKo: '?¨Îûå ??Î™ÖÏù¥ ?¨Í∏∞ ?àÏñ¥??' },
  { id: 'fb_irr_mouse', word: 'mouse', article: 'singular', emoji: '?ê≠', exampleSentence: 'One mouse is in the house.', exampleSentenceKo: 'Ï•???ÎßàÎ¶¨Í∞Ä ÏßëÏóê ?àÏñ¥??' },
  { id: 'fb_irr_man', word: 'man', article: 'singular', emoji: '?ë®', exampleSentence: 'One man is working.', exampleSentenceKo: '?®Ïûê ??Î™ÖÏù¥ ?ºÌïòÍ≥??àÏñ¥??' }
];

const fallbackPlural = [
  { id: 'fb_irr_children', word: 'children', article: 'plural', emoji: '?ëß?ë¶', exampleSentence: 'Many children are playing.', exampleSentenceKo: '?ÑÏù¥?§Ïù¥ ?ÄÍ≥??àÏñ¥??' },
  { id: 'fb_irr_people', word: 'people', article: 'plural', emoji: '?ë•?ë•', exampleSentence: 'Many people are here.', exampleSentenceKo: 'ÎßéÏ? ?¨Îûå?§Ïù¥ ?¨Í∏∞ ?àÏñ¥??' },
  { id: 'fb_irr_mice', word: 'mice', article: 'plural', emoji: '?ê≠?ê≠', exampleSentence: 'Many mice are in the house.', exampleSentenceKo: 'Ï•êÎì§??ÏßëÏóê ?àÏñ¥??' },
  { id: 'fb_irr_men', word: 'men', article: 'plural', emoji: '?ë®?ë®', exampleSentence: 'Many men are working.', exampleSentenceKo: '?®Ïûê?§Ïù¥ ?ºÌïòÍ≥??àÏñ¥??' }
];
