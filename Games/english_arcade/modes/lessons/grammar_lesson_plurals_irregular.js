...
export async function runGrammarLessonPluralsIrregular(ctx = {}) {
  ...
  function renderIntroStep(stage, prog, stepEl) {
    ...
    intro.innerHTML = lang === 'ko'
      ? "일부 단어들은 복수가 될 때 완전히 달라져요. 이런 단어들을 불규칙 복수라고 해요!"
    ...
    highlight.innerHTML = `
      <div class="card-heading">${lang === 'ko' ? '예시로 규칙을 살펴봐요!' : 'See how irregular plurals work!'}</div>
      <div class="card-rule">${lang === 'ko' ? "<strong>완전히 다르게</strong> 변해요" : "The word <strong>changes completely</strong>"}</div>
      <div class="card-row">
        <div class="card-side">
      ...
      <div class="card-tip">${lang === 'ko' ? "단어를 외워야 해요! 규칙이 없어요." : "You have to memorize them! No pattern here."}</div>
    `;
  ...
  function renderExamplesStep(stage, prog, stepEl) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = lang === 'ko'
      ? "왼쪽은 단수(하나), 오른쪽은 불규칙하게 변한 복수 예문이에요. 소리 내어 읽어 보세요!"
  ...
  function renderSortingStep(stage, prog, stepEl) {
    ...
    checkBtn.onclick = () => {
      ...
      if (poolCount === 0 && wrongCount === 0) {
        ...
        message.textContent = lang === 'ko' ? "완벽해요! 불규칙 복수를 모두 찾았어요." : "Awesome! You spotted every irregular plural.";
  ...
  function renderFinishStep(stage, prog, stepEl) {
    ...
    body.style.gap = '30px';
    body.innerHTML = lang === 'ko'
      ? "<div style=\"font-weight:800;color:#19777e\">불규칙 복수를 배웠어요!</div><div class=\"stars\">⭐⭐⭐⭐⭐</div>"
...
const fallbackSingular = [
  { id: 'fb_irr_child', word: 'child', article: 'singular', emoji: '👧', exampleSentence: 'One child is playing.', exampleSentenceKo: '아이 한 명이 놀고 있어요.' },
  { id: 'fb_irr_person', word: 'person', article: 'singular', emoji: '👥', exampleSentence: 'One person is here.', exampleSentenceKo: '사람 한 명이 여기 있어요.' },
  { id: 'fb_irr_mouse', word: 'mouse', article: 'singular', emoji: '🐭', exampleSentence: 'One mouse is in the house.', exampleSentenceKo: '쥐 한 마리가 집에 있어요.' },
  { id: 'fb_irr_man', word: 'man', article: 'singular', emoji: '👨', exampleSentence: 'One man is working.', exampleSentenceKo: '남자 한 명이 일하고 있어요.' }
];

const fallbackPlural = [
  { id: 'fb_irr_children', word: 'children', article: 'plural', emoji: '👧👦', exampleSentence: 'Many children are playing.', exampleSentenceKo: '아이들이 놀고 있어요.' },
  { id: 'fb_irr_people', word: 'people', article: 'plural', emoji: '👥👥', exampleSentence: 'Many people are here.', exampleSentenceKo: '많은 사람들이 여기 있어요.' },
  { id: 'fb_irr_mice', word: 'mice', article: 'plural', emoji: '🐭🐭', exampleSentence: 'Many mice are in the house.', exampleSentenceKo: '쥐들이 집에 있어요.' },
  { id: 'fb_irr_men', word: 'men', article: 'plural', emoji: '👨👨', exampleSentence: 'Many men are working.', exampleSentenceKo: '남자들이 일하고 있어요.' }
];
