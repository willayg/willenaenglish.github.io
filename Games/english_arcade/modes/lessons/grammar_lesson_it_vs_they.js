...
export async function runGrammarLessonItVsThey(ctx = {}) {
  ...
  const fallbackIt = [
    { id: 'fallback_it_cat', word: 'cat', article: 'it', emoji: '?ê±', exampleSentence: 'It is a sleepy cat.', exampleSentenceKo: 'Í∑∏Í≤É?Ä Ï°∏Î¶∞ Í≥†Ïñë?¥Ïóê??', explanation: "Use 'it' when there is only one." },
    { id: 'fallback_it_robot', word: 'robot', article: 'it', emoji: '?§ñ', exampleSentence: 'It is a shiny robot.', exampleSentenceKo: 'Í∑∏Í≤É?Ä Î∞òÏßù?¥Îäî Î°úÎ¥á?¥Ïóê??', explanation: "Use 'it' for a single machine." },
    { id: 'fallback_it_flower', word: 'flower', article: 'it', emoji: '?å∏', exampleSentence: 'It is a pink flower.', exampleSentenceKo: 'Í∑∏Í≤É?Ä Î∂ÑÌôç??ÍΩÉÏù¥?êÏöî.', explanation: "One flower needs 'it'." },
    { id: 'fallback_it_penguin', word: 'penguin', article: 'it', emoji: '?êß', exampleSentence: 'It is a baby penguin.', exampleSentenceKo: 'Í∑∏Í≤É?Ä ?ÑÍ∏∞ ??∑Ñ?¥Ïóê??', explanation: "Single animals use 'it'." }
  ];
  const fallbackThey = [
    { id: 'fallback_they_cats', word: 'cats', article: 'they', emoji: '?ê±?ê±', exampleSentence: 'They are fluffy cats.', exampleSentenceKo: 'Í∑∏Í≤É?§Ï? ?¨Í∑º??Í≥†Ïñë?¥Îì§?¥Ïóê??', explanation: "Use 'they' when there is more than one." },
    { id: 'fallback_they_robots', word: 'robots', article: 'they', emoji: '?§ñ?§ñ', exampleSentence: 'They are helper robots.', exampleSentenceKo: 'Í∑∏Í≤É?§Ï? ?ÑÏ?Ï£ºÎäî Î°úÎ¥á?§Ïù¥?êÏöî.', explanation: "Many machines use 'they'." },
    { id: 'fallback_they_flowers', word: 'flowers', article: 'they', emoji: '?åº?åº', exampleSentence: 'They are yellow flowers.', exampleSentenceKo: 'Í∑∏Í≤É?§Ï? ?∏Î? ÍΩÉÎì§?¥Ïóê??', explanation: "More than one flower is 'they'." },
    { id: 'fallback_they_penguins', word: 'penguins', article: 'they', emoji: '?êß?êß', exampleSentence: 'They are marching penguins.', exampleSentenceKo: 'Í∑∏Í≤É?§Ï? ?âÏßÑ?òÎäî ??∑Ñ?§Ïù¥?êÏöî.', explanation: "A group of animals uses 'they'." }
  ];
  ...
  function renderStep2Intro(stage, prog, stepEl, nav) {
    ...
    blurb.innerHTML = (lang === 'ko')
      ? `?òÎÇò??<b>it</b> (Í∑∏Í≤É), ??Í∞??¥ÏÉÅ?Ä <b>they</b> (Í∑∏Í≤É??Í∑??¨Îûå??Î•??®Ïöî.`
    ...
    const nextExampleBtn = button((lang === 'ko') ? '?§Î•∏ ??Î≥¥Í∏∞' : 'Next Example');
    ...
    const backBtn = button((lang === 'ko') ? '?§Î°ú' : 'Back');
    const proceedBtn = button((lang === 'ko') ? '?§Ïùå ?®Í≥ÑÎ°? : 'Go to Next Step');
    ...
    const setActivePronoun = (pronoun, advance = false) => {
      ...
      pronounLabel.textContent = (pronoun === 'it')
        ? ((lang === 'ko') ? "'it' ??Í∑∏Í≤É (?òÎÇò)" : '"it" means just one')
        : ((lang === 'ko') ? "'they' ??Í∑∏Í≤É??Í∑??¨Îûå??(?¨Îü¨ Í∞?" : '"they" means more than one');
      emojiEl.textContent = item.emoji || (pronoun === 'it' ? '?êæ' : '?êæ?êæ');
  ...
  function renderStep3Examples(stage, prog, stepEl, nav) {
    ...
    intro.innerHTML = (lang === 'ko')
      ? "Ïπ¥ÎìúÎ•?Î≥¥Í≥† ?¥Îñ§ ?ÄÎ™ÖÏÇ¨Î•??∞ÎäîÏßÄ Í∏∞Ïñµ??Î≥¥ÏÑ∏??"
      : 'Look at the cards to see when we say ?úit??or ?úthey??';
  ...
  function renderStep4Sort(stage, prog, stepEl, nav) {
    ...
    body.innerHTML = (lang === 'ko')
      ? "??Í∞úÎäî <b>it</b> (Í∑∏Í≤É), ?¨Îü¨ Í∞úÎäî <b>they</b> (Í∑∏Í≤É??Í∑??¨Îûå?? Î∞îÍµ¨?àÏóê ?£Ïñ¥ Î≥¥ÏÑ∏??"
    ...
    const pool = makeBucket('pool', (lang === 'ko') ? '?®Ïñ¥ Î™®Ïùå' : 'Word Pool');
      const bucketIt = makeBucket('it', (lang === 'ko') ? "it (Í∑∏Í≤É - ?òÎÇò)" : 'it (one)');
      const bucketThey = makeBucket('they', (lang === 'ko') ? "they (Í∑∏Í≤É??Í∑??¨Îûå??- ?¨Îü¨ Í∞?" : 'they (more than one)');
    ...
    const checkBtn = button((lang === 'ko') ? '?ïÎãµ ?ïÏù∏' : 'Check Answers');
  ...
  stepEl.appendChild(navLocal);
    ...
    checkBtn.onclick = () => {
      ...
      if (poolCount === 0 && correct === total) {
        ...
        message.textContent = (lang === 'ko') ? "?ÑÎ≤Ω?¥Ïöî! 'it' (Í∑∏Í≤É)?Ä ?òÎÇò, 'they' (Í∑∏Í≤É??Í∑??¨Îûå?????¨Îü¨ Í∞?" : 'Awesome! "it" is for one, "they" is for more than one!';
        stepEl.insertBefore(message, stepEl.firstChild);
        
        if (!continueBtn) {
          continueBtn = button((lang === 'ko') ? '?§Ïùå ?®Í≥ÑÎ°? : 'Next');
        ...
        message.textContent = (lang === 'ko') ? 'Îπ®Í∞Ñ Ïπ¥ÎìúÎ•??§Ïãú ??≤® Î≥¥ÏÑ∏??' : 'Try again! Move the red cards.';
        stepEl.insertBefore(message, stepEl.firstChild);
      }
    };
  }
  ...
  function renderStep5Finish(stage, prog, stepEl, nav) {
    ...
    body.style.gap = '30px';
    body.innerHTML = (lang === 'ko')
      ? '<div style="font-weight:800;color:#19777e">?¥Ï†ú it (Í∑∏Í≤É)Í≥?they (Í∑∏Í≤É??Í∑??¨Îûå????Íµ¨Î≥Ñ?????àÏñ¥??</div><div class="stars">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>'
      : '<div style="font-weight:800;color:#19777e">Now you can tell "it" from "they"!</div><div class="stars">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>';
    stepEl.appendChild(body);

    const navWrap = document.createElement('div');
    navWrap.className = 'lesson-nav';
    const backToModes = button((lang === 'ko') ? 'Î™®ÎìúÎ°??åÏïÑÍ∞ÄÍ∏? : 'Back to Modes');
    backToModes.onclick = () => {
      try {
  ...
  function render() {
    ...
    const back = button((lang === 'ko') ? '?§Î°ú' : 'Back');
    const next = button((lang === 'ko') ? '?§Ïùå' : 'Next');
...
function buildPronounColumn(pronoun, list, lang) {
  ...
  heading.textContent = pronoun === 'it'
    ? (lang === 'ko' ? "it (Í∑∏Í≤É - ?òÎÇò)" : 'it ??just one')
    : (lang === 'ko' ? "they (Í∑∏Í≤É??Í∑??¨Îûå??- ?¨Îü¨ Í∞?" : 'they ??more than one');
...
function defaultPronounSentenceKo(word, pronoun) {
  if (pronoun === 'they') return `Í∑∏Í≤É?§Ï? ${word}?§Ïù¥?êÏöî.`;
  return `Í∑∏Í≤É?Ä ${word}?¥Ïóê??`;
}
...
function defaultPronounExplanationKo(pronoun) {
  return pronoun === 'it' ? "'it' (Í∑∏Í≤É)?Ä ??Í∞úÏùº ???®Ïöî." : "'they' (Í∑∏Í≤É??Í∑??¨Îûå??????Í∞??¥ÏÉÅ?????®Ïöî.";
}
