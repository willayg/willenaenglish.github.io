...
export async function runGrammarLessonIsAreQuestions(ctx = {}) {
  ...
  function renderExplainStep(stepEl, progress) {
    ...
    intro.innerHTML = lang === 'ko'
      ? "질문을 만들 때 <b>is</b> (단수: he/she/it), <b>are</b> (복수: you/we/they)을 사용해요. 버튼을 눌러 예제를 보세요!"
  ...
  function renderExamplesStep(stepEl, progress) {
    ...
    const categorySets = [
      {
        id: 'singular',
        label: lang === 'ko' ? 'is → 단수 주어' : 'is → Singular',
        examples: isList,
        pointer: 0,
        question: 'IS',
      },
      {
        id: 'plural',
        label: lang === 'ko' ? 'are → 복수 주어' : 'are → Plural',
        examples: areList,
        pointer: 0,
        question: 'ARE',
      }
  ...
  function renderSortingStep(stepEl, progress) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = (lang === 'ko')
      ? '주어를 눌러서 <b>Is</b> 또는 <b>Are</b> 바구니에 넣어 보세요! 모두 맞으면 다음 단계로 갈 수 있어요!'
    ...
    const isBucket = makeBucket('is', 'Is 바구니');
    const areBucket = makeBucket('are', 'Are 바구니');
    ...
    checkBtn.onclick = () => {
      ...
      if (allCorrect) {
        ...
        message.textContent = lang === 'ko' ? '빨간 주어를 다시 한번 보세요!' : 'Try again! Fix the red subjects.';
        stepEl.insertBefore(message, stepEl.firstChild);
      }
    };
  }
...
function ensureIsAreStyles() {
  ...
  st.textContent = `
    ...
    #gameArea .isare-subject-row button{border:2px solid #21b3be;background:#ffffff;color:#ff6fb0;font-weight:700;padding:10px 18px;border-radius:999px;cursor:pointer;transition:transform .15s ease, box-shadow .15s ease, background .15s ease;font-family:'Poppins', Arial, sans-serif}
  ...
  return [
    {
      ...
      sentenceKo: isExample.exampleSentenceKo || '그는 네 형이에요?',
      tipEn: "Use 'Is' with he, she, it.",
      tipKo: "He/She/It에는 'Is'를 사용해요.",
      label: 'is → 단수 주어'
    },
    {
      id: 'are',
      ...
      sentenceKo: areExample.exampleSentenceKo || '그들은 네 친구들이에요?',
      tipEn: "Use 'Are' with you, we, they.",
      tipKo: "You/We/They에는 'Are'를 사용해요.",
      label: 'are → 복수 주어'
    }
  ];
}
...
const fallbackIs = [
  { id: 'fb_is_he', word: 'he', exampleSentence: 'Is he your brother?', exampleSentenceKo: '그는 네 형이에요?', emoji: '👦' },
  { id: 'fb_is_she', word: 'she', exampleSentence: 'Is she okay?', exampleSentenceKo: '그녀는 괜찮아요?', emoji: '👧' },
  { id: 'fb_is_it', word: 'it', exampleSentence: 'Is it a cat?', exampleSentenceKo: '그것은 고양이인가요?', emoji: '🐱' },
  { id: 'fb_is_this', word: 'this', exampleSentence: 'Is this correct?', exampleSentenceKo: '이것이 맞나요?', emoji: '✅' }
];

const fallbackAre = [
  { id: 'fb_are_you', word: 'you', exampleSentence: 'Are you ready?', exampleSentenceKo: '준비됐나요?', emoji: '🙂' },
  { id: 'fb_are_they', word: 'they', exampleSentence: 'Are they your friends?', exampleSentenceKo: '그들은 네 친구들인가요?', emoji: '👥' },
  { id: 'fb_are_we', word: 'we', exampleSentence: 'Are we late?', exampleSentenceKo: '우리는 늦었나요?', emoji: '⏰' },
  { id: 'fb_are_books', word: 'books', exampleSentence: 'Are there any books?', exampleSentenceKo: '책들이 있나요?', emoji: '📚' }
];
