...
export async function runGrammarLessonIsAreQuestions(ctx = {}) {
  ...
  try {
    ...
    console.warn('[lesson-is-are-questions] failed to load list', err);
  }

  const isList = normalizeList(items.filter((it) => isQuestion(it, 'is')), fallbackIs);
  const areList = normalizeList(items.filter((it) => isQuestion(it, 'are')), fallbackAre);
  const sortingPool = buildSortingPool(isList, areList);
  ...
  try {
    ...
    console.debug('[IsAreQuestionsLesson] startSession failed', err?.message);
  }
  ...
  function render() {
    ...
    title.textContent = grammarName || 'Is vs Are Questions';
  ...
  function renderLanguageStep(stepEl, progress, stage) {
    ...
    heading.textContent = (lang === 'ko') ? '?∏Ïñ¥Î•??†ÌÉù?òÏÑ∏?? : 'Choose your language';
  ...
  function renderExplainStep(stepEl, progress) {
    ...
    intro.innerHTML = lang === 'ko'
      ? "ÏßàÎ¨∏??ÎßåÎì§ ??<b>is</b> (?®Ïàò: he/she/it), <b>are</b> (Î≥µÏàò: you/we/they)Î•??®Ïöî. Î≤ÑÌäº???åÎü¨???àÏ†úÎ•??ïÏù∏??Î≥¥ÏÑ∏??"
    ...
    questionRow.className = 'isare-subject-row';

    const cardDisplay = document.createElement('div');
    cardDisplay.className = 'isare-highlight-card';

    const questionSets = buildQuestionSets(isList, areList);
    let current = null;
    let currentType = 'is';
    let isPointer = 0;
    let arePointer = 0;
    ...
    const renderCard = (set, advance) => {
      ...
      if (advance) {
        if (set.id === 'is') {
          isPointer = (isPointer + 1) % isList.length;
          const nextExample = isList[isPointer];
          ...
          arePointer = (arePointer + 1) % areList.length;
          const nextExample = areList[arePointer];
          set.sentenceEn = nextExample.exampleSentence || set.sentenceEn;
          set.sentenceKo = nextExample.exampleSentenceKo || set.sentenceKo;
          set.emoji = nextExample.emoji || set.emoji;
        }
    ...
    const nextExampleBtn = buildSecondaryButton(lang === 'ko' ? '?§Ïùå ?àÏ†ú' : 'Next Example');
  ...
  function renderExamplesStep(stepEl, progress) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = lang === 'ko'
      ? "<b>He/She/It</b> ??<b>Is</b><br/><b>You/We/They</b> ??<b>Are</b><br/>Î≤ÑÌäº???åÎü¨????ÎßéÏ? ?àÏ†úÎ•??ïÏù∏?òÏÑ∏??"
    ...
    const categorySets = [
      {
        id: 'singular',
        label: lang === 'ko' ? '?ë§ ?®Ïàò Ï£ºÏñ¥' : '?ë§ Singular',
        examples: isList,
        pointer: 0,
        question: 'IS',
      },
      {
        id: 'plural',
        label: lang === 'ko' ? '?ë• Î≥µÏàò Ï£ºÏñ¥' : '?ë• Plural',
        examples: areList,
        pointer: 0,
        question: 'ARE',
      }
  ...
  function renderSortingStep(stepEl, progress) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = (lang === 'ko')
      ? 'Ï£ºÏñ¥Î•??åÎü¨ <b>Is</b> ?êÎäî <b>Are</b> Î∞îÍµ¨?àÏóê ?£Ïñ¥ Î≥¥ÏÑ∏?? Î™®Îëê ÎßûÏúºÎ©??§Ïùå ?®Í≥ÑÎ°??òÏñ¥Í∞????àÏñ¥??'
    ...
    const pool = makeBucket('pool', lang === 'ko' ? 'Ï£ºÏñ¥ Î™®Ïùå' : 'Subject Pool');
    const isBucket = makeBucket('is', 'Is ?ë§');
    const areBucket = makeBucket('are', 'Are ?ë•');
    ...
    [isBucket.wrap, areBucket.wrap].forEach((wrap) => {
    ...
    checkBtn.onclick = () => {
      const answers = {
        is: new Set(sortingPool.filter((item) => item.answer === 'is').map((item) => item.id)),
        are: new Set(sortingPool.filter((item) => item.answer === 'are').map((item) => item.id)),
      };
      ...
      markBucket(isBucket.body, 'is');
      markBucket(areBucket.body, 'are');
      ...
      if (allCorrect) {
        ...
        message.textContent = lang === 'ko' ? '?ÑÎ≤Ω?¥Ïöî! is/areÎ•???Í≥®Îûê?¥Ïöî.' : 'Perfect! You used is/are correctly.';
        ...
        message.textContent = lang === 'ko' ? 'Îπ®Í∞Ñ Ï£ºÏñ¥Î•??§Ïãú ??≤® Î≥¥ÏÑ∏??' : 'Try again! Fix the red subjects.';
        stepEl.insertBefore(message, stepEl.firstChild);
      }
    };
  }
  ...
  function renderFinishStep(stepEl) {
    ...
    body.innerHTML = (lang === 'ko')
      ? '<div style="font-weight:800;color:#19777e">?¥Ï†ú is/are ÏßàÎ¨∏??Î∞îÎ•¥Í≤?ÎßåÎì§ ???àÏñ¥??</div><div class="stars">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>'
      : '<div style="font-weight:800;color:#19777e">You now know when to use is or are in questions!</div><div class="stars">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>';
    ...
    if (!sessionClosed) {
      ...
      try {
        ...
        console.debug('[IsAreQuestionsLesson] endSession failed', err?.message);
      }

      try {
        const ev = new CustomEvent('wa:session-ended', { detail: { summary: { correct: 1, total: 1, grammarName: grammarName || 'Is vs Are Questions', category: 'grammar' } } });
...
function ensureIsAreStyles() {
  if (document.getElementById('wa-lesson-isare-styles')) return;
  const st = document.createElement('style');
  st.id = 'wa-lesson-isare-styles';
  st.textContent = `
    #gameArea .isare-subject-row{display:flex;flex-wrap:wrap;gap:10px;justify-content:center;margin:24px 0 12px}
    #gameArea .isare-subject-row button{border:2px solid #21b3be;background:#ffffff;color:#21b3be;font-weight:700;padding:10px 18px;border-radius:999px;cursor:pointer;transition:transform .15s ease, box-shadow .15s ease, background .15s ease}
    #gameArea .isare-subject-row button:hover{transform:translateY(-2px);box-shadow:0 8px 16px rgba(33,179,190,0.22)}
    #gameArea .isare-subject-row button.active{background:#21b3be;color:#ffffff;box-shadow:0 10px 22px rgba(33,179,190,0.28)}
    #gameArea .isare-highlight-card{background:#f7feff;border:2px solid rgba(33,179,190,0.28);border-radius:18px;padding:22px 20px;display:flex;flex-direction:column;align-items:center;gap:12px;max-width:420px;margin:0 auto;box-shadow:0 14px 40px -22px rgba(20,126,130,0.45)}
    #gameArea .isare-highlight-card .question-label{font-size:0.95rem;font-weight:800;color:#19777e;letter-spacing:0.08em;text-transform:uppercase}
    #gameArea .isare-highlight-card .question-emoji{font-size:2.6rem}
    #gameArea .isare-highlight-card .question-sentence{font-size:1.12rem;line-height:1.6;color:#27323a;font-weight:600}
    #gameArea .isare-highlight-card .question-sentence-ko{font-size:1rem;color:#546070;margin-top:-4px}
    #gameArea .isare-highlight-card .question-tip{font-size:0.95rem;color:#546070}
  `;
  document.head.appendChild(st);
}
...
function buildSortingPool(isList, areList) {
  const isPool = shuffle(isList).slice(0, 5).map((item, idx) => ({
    id: item.id || `is_${idx}`,
    answer: 'is',
    text: item.word || 'he',
  }));
  const arePool = shuffle(areList).slice(0, 5).map((item, idx) => ({
    id: item.id || `are_${idx}`,
    answer: 'are',
    text: item.word || 'they',
  }));
  return shuffle([...isPool, ...arePool]);
}
...
function buildQuestionSets(isList, areList) {
  ...
  return [
    {
      id: 'is',
      question: 'is',
      emoji: isExample.emoji || '?ë§',
      sentenceEn: isExample.exampleSentence || 'Is he your brother?',
      sentenceKo: isExample.exampleSentenceKo || 'Í∑∏Îäî ???ïÏù¥??',
      tipEn: "Use 'Is' with he, she, it.",
      tipKo: "he, she, it???Ä??'Is'Î•??®Ïöî.",
      label: '?ë§ Is'
    },
    {
      id: 'are',
      question: 'are',
      emoji: areExample.emoji || '?ë•',
      sentenceEn: areExample.exampleSentence || 'Are they your friends?',
      sentenceKo: areExample.exampleSentenceKo || 'Í∑∏Îì§?Ä ??ÏπúÍµ¨?§Ïù¥??',
      tipEn: "Use 'Are' with you, we, they.",
      tipKo: "you, we, they???Ä??'Are'Î•??®Ïöî.",
      label: '?ë• Are'
    }
  ];
}
...
function buildNavRow(onBack, onNext, lang) {
  ...
  const nextBtn = buildPrimaryButton(lang === 'ko' ? '?§Ïùå' : 'Next');
...
function displayStep(stepIndex, lang) {
  ...
  const stepsKo = ['?∏Ïñ¥ ?†ÌÉù', '1?®Í≥Ñ', '2?®Í≥Ñ', '3?®Í≥Ñ', '?ÑÎ£å'];
...
const fallbackIs = [
  { id: 'fb_is_he', word: 'he', exampleSentence: 'Is he your brother?', exampleSentenceKo: 'Í∑∏Îäî ???ïÏù¥??', emoji: '?ë®' },
  { id: 'fb_is_she', word: 'she', exampleSentence: 'Is she okay?', exampleSentenceKo: 'Í∑∏Î???Í¥úÏ∞Æ??', emoji: '?ë©' },
  { id: 'fb_is_it', word: 'it', exampleSentence: 'Is it a cat?', exampleSentenceKo: 'Í∑∏Í≤É?Ä Í≥†Ïñë?¥Îãà?', emoji: '?ê±' },
  { id: 'fb_is_this', word: 'this', exampleSentence: 'Is this correct?', exampleSentenceKo: '?¥Í≤å ÎßûÎãà?', emoji: '?? }
];

const fallbackAre = [
  { id: 'fb_are_you', word: 'you', exampleSentence: 'Are you ready?', exampleSentenceKo: 'Ï§ÄÎπÑÎêê??', emoji: 'Ôø? },
  { id: 'fb_are_they', word: 'they', exampleSentence: 'Are they your friends?', exampleSentenceKo: 'Í∑∏Îì§?Ä ??ÏπúÍµ¨?§Ïù¥??', emoji: '?ë•' },
  { id: 'fb_are_we', word: 'we', exampleSentence: 'Are we late?', exampleSentenceKo: '?∞Î¶¨????óà??', emoji: '?? },
  { id: 'fb_are_books', word: 'books', exampleSentence: 'Are there any books?', exampleSentenceKo: 'Ï±ÖÎì§???àÎãà?', emoji: '?ìö' }
];
