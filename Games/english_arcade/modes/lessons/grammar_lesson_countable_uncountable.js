...
export async function runGrammarLessonCountableUncountable(ctx = {}) {
  ...
  function renderIntroStep(stage, prog, stepEl) {
    ...
    intro.innerHTML = lang === 'ko'
      ? "어떤 명사는 셀 수 있어요(예: 사과), 어떤 명사는 셀 수 없어요(예: 물). 셀 수 없는 명사는 보통 액체나 반고체이거나 너무 작아서 세기 어려운 것들입니다!"
    ...
    countableHtml += `<div class="section-label">${lang === 'ko' ? '셀 수 있는 명사' : 'Countable'}</div>`;
    ...
    uncountableHtml += `<div class="section-label">${lang === 'ko' ? '셀 수 없는 명사' : 'Uncountable'}</div>`;
  ...
  function renderExamplesStep(stage, prog, stepEl) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = lang === 'ko'
      ? "왼쪽은 셀 수 있는 명사, 오른쪽은 셀 수 없는 명사입니다. 소리 내어 읽어보세요!"
  ...
  function renderSortingStep(stage, prog, stepEl) {
    ...
    body.innerHTML = lang === 'ko'
      ? "각 단어를 탭하여 셀 수 있는지, 셀 수 없는지 바구니에 넣으세요. 모두 맞히면 다음 단계로 넘어갑니다!"
    ...
    const bucketCountable = makeBucket('countable', lang === 'ko' ? '셀 수 있는 명사' : 'Countable');
    const bucketUncountable = makeBucket('uncountable', lang === 'ko' ? '셀 수 없는 명사' : 'Uncountable');
    ...
    checkBtn.onclick = () => {
      ...
      if (existingMsg) existingMsg.remove();
        ...
        message.textContent = lang === 'ko' ? '완벽해요! 모든 명사를 올바르게 분류했어요!' : "Perfect! You sorted all the nouns correctly!";
        ...
        message.textContent = lang === 'ko' ? '틀렸어요. 빨간 카드를 올바른 바구니로 옮기세요.' : 'Move the red cards to the correct basket.';
        stepEl.insertBefore(message, stepEl.firstChild);
      }
    };
  }
  ...
  function renderFinishStep(stage, prog, stepEl) {
    ...
    body.style.gap = '30px';
    body.innerHTML = lang === 'ko'
      ? "<div style=\"font-weight:800;color:#19777e\">축하합니다! 셀 수 있는 명사와 셀 수 없는 명사를 배웠어요!</div><div class=\"stars\">⭐⭐⭐⭐⭐</div>"
  ...
  function buildExampleColumn(kind, list, currentLang) {
    ...
    titleWrap.textContent = kind === 'countable'
      ? (currentLang === 'ko' ? '셀 수 있는 명사' : 'Countable')
      : (currentLang === 'ko' ? '셀 수 없는 명사' : 'Uncountable');
...
const fallbackCountable = [
  { id: 'fb_count_apple', word: 'apple', article: 'countable', emoji: '🍎', exampleSentence: 'I have an apple.', exampleSentenceKo: '저는 사과가 있어요.' },
  { id: 'fb_count_book', word: 'book', article: 'countable', emoji: '📚', exampleSentence: 'She has a book.', exampleSentenceKo: '그녀는 책을 가지고 있어요.' },
  { id: 'fb_count_chair', word: 'chair', article: 'countable', emoji: '🪑', exampleSentence: 'A chair is in the room.', exampleSentenceKo: '방에 의자가 있어요.' },
  { id: 'fb_count_dog', word: 'dog', article: 'countable', emoji: '🐶', exampleSentence: 'A dog is friendly.', exampleSentenceKo: '강아지는 친근해요.' }
];
