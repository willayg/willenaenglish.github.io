...
export async function runGrammarLessonCountableUncountable(ctx = {}) {
  ...
  function renderIntroStep(stage, prog, stepEl) {
    ...
    intro.innerHTML = lang === 'ko'
      ? "?¥Îñ§ ?®Ïñ¥???Ä ???àÍ≥†, ?¥Îñ§ ?®Ïñ¥???Ä ???ÜÏñ¥?? Î¨? ?∞Ïú†, Î∞•Ï≤ò???°Ï≤¥???àÎ¨¥ ?ëÏïÑ???∏Í∏∞ ?¥Î†§??Í≤ÉÎì§?Ä ?Ä ???ÜÏñ¥??"
    ...
    countableHtml += `<div class="section-label">${lang === 'ko' ? '?Ä ???àÏñ¥?? : 'Countable'}</div>`;
    countableHtml += '<div class="examples-list">';
    countableExamples.forEach((item) => {
      countableHtml += `<div class="example-item"><span class="ex-emoji">${escapeHtml(item.emoji || '?çé')}</span><span class="ex-word">${escapeHtml(item.word)}</span></div>`;
    });
    countableHtml += '</div></div>';

    let uncountableHtml = '<div class="examples-section">';
    uncountableHtml += `<div class="section-label">${lang === 'ko' ? '?Ä ???ÜÏñ¥?? : 'Uncountable'}</div>`;
    uncountableHtml += '<div class="examples-list">';
    uncountableExamples.forEach((item) => {
      uncountableHtml += `<div class="example-item"><span class="ex-emoji">${escapeHtml(item.emoji || '?íß')}</span><span class="ex-word">${escapeHtml(item.word)}</span></div>`;
    });
  ...
  function renderExamplesStep(stage, prog, stepEl) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = lang === 'ko'
      ? "?ºÏ™Ω?Ä ?Ä ???àÎäî Î™ÖÏÇ¨?? ?§Î•∏Ï™ΩÏ? ?Ä ???ÜÎäî Î™ÖÏÇ¨?§Ïù¥?êÏöî. ?åÎ¶¨ ?¥Ïñ¥ ?ΩÏñ¥ Î≥¥ÏÑ∏??"
  ...
  function renderSortingStep(stage, prog, stepEl) {
    ...
    body.innerHTML = lang === 'ko'
      ? "Í∞??®Ïñ¥Î•??åÎü¨ '?Ä ???àÏñ¥?? ?êÎäî '?Ä ???ÜÏñ¥?? Î∞îÍµ¨?àÏóê ?£Ïñ¥ Î≥¥ÏÑ∏?? Î™®Îëê ÎßûÌûàÎ©??§Ïùå?ºÎ°ú Í∞????àÏñ¥??"
    ...
    const bucketCountable = makeBucket('countable', lang === 'ko' ? '?Ä ???àÏñ¥?? : 'Countable');
    const bucketUncountable = makeBucket('uncountable', lang === 'ko' ? '?Ä ???ÜÏñ¥?? : 'Uncountable');
    ...
    checkBtn.onclick = () => {
      ...
      if (poolCount === 0 && wrongCount === 0) {
        ...
        message.textContent = lang === 'ko' ? "?ÑÎ≤Ω?¥Ïöî! ?Ä ???àÎäî Í≤ÉÍ≥º ?ÜÎäî Í≤ÉÏùÑ Î™®Îëê Ï∞æÏïò?¥Ïöî." : "Perfect! You sorted all the nouns correctly!";
  ...
  function renderFinishStep(stage, prog, stepEl) {
    ...
    body.style.gap = '30px';
    body.innerHTML = lang === 'ko'
      ? "<div style=\"font-weight:800;color:#19777e\">?Ä ???àÎäî Í≤ÉÍ≥º ?ÜÎäî Í≤ÉÏùÑ Î∞∞Ïõ†?¥Ïöî!</div><div class=\"stars\">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>"
      : "<div style=\"font-weight:800;color:#19777e\">You learned countable and uncountable nouns!</div><div class=\"stars\">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>";
    stepEl.appendChild(body);
  ...
  function buildExampleColumn(kind, list, currentLang) {
    ...
    titleWrap.textContent = kind === 'countable'
      ? (currentLang === 'ko' ? '?Ä ???àÏñ¥?? : 'Countable')
      : (currentLang === 'ko' ? "?Ä ???ÜÏñ¥?? : "Uncountable");
...
const fallbackCountable = [
  { id: 'fb_count_apple', word: 'apple', article: 'countable', emoji: '?çé', exampleSentence: 'I have an apple.', exampleSentenceKo: '?òÎäî ?¨Í≥ºÎ•???Í∞?Í∞ÄÏßÄÍ≥??àÏñ¥??' },
  { id: 'fb_count_book', word: 'book', article: 'countable', emoji: '?ìö', exampleSentence: 'She has a book.', exampleSentenceKo: 'Í∑∏Î???Ï±???Í∂åÏù¥ ?àÏñ¥??' },
  { id: 'fb_count_chair', word: 'chair', article: 'countable', emoji: '?™ë', exampleSentence: 'A chair is in the room.', exampleSentenceKo: 'Î∞©Ïóê ?òÏûê ??Í∞úÍ? ?àÏñ¥??' },
  { id: 'fb_count_dog', word: 'dog', article: 'countable', emoji: '?ê∂', exampleSentence: 'A dog is friendly.', exampleSentenceKo: 'Í∞úÎäî ÏπúÏ†à?¥Ïöî.' }
];

const fallbackUncountable = [
  { id: 'fb_uncount_water', word: 'water', article: 'uncountable', emoji: '?íß', exampleSentence: 'There is water in the bottle.', exampleSentenceKo: 'Î≥ëÏóê Î¨ºÏù¥ ?àÏñ¥??' },
  { id: 'fb_uncount_milk', word: 'milk', article: 'uncountable', emoji: '?•õ', exampleSentence: 'Milk is in the fridge.', exampleSentenceKo: '?∞Ïú†Í∞Ä ?âÏû•Í≥†Ïóê ?àÏñ¥??' },
  { id: 'fb_uncount_rice', word: 'rice', article: 'uncountable', emoji: '?çö', exampleSentence: 'We need more rice.', exampleSentenceKo: '?∞Î¶¨???Ä?????ÑÏöî?¥Ïöî.' },
  { id: 'fb_uncount_honey', word: 'honey', article: 'uncountable', emoji: '?çØ', exampleSentence: 'Honey tastes sweet.', exampleSentenceKo: 'ÍøÄ?Ä ?¨ÏΩ§?¥Ïöî.' }
];
