...
export async function runGrammarLessonPluralsIes(ctx = {}) {
  ...
  function renderLanguageStep(stage, prog, stepEl) {
    ...
    title.textContent = lang === 'ko' ? '?¸ì–´ë¥?? íƒ?˜ì„¸?? : 'Choose your language';
  ...
  function renderIntroStep(stage, prog, stepEl) {
    ...
    highlight.innerHTML = `
      <div class="card-heading">${lang === 'ko' ? '?ˆì‹œë¡?ê·œì¹™???´í´ë´ìš”!' : 'See how the rule works!'}</div>
      <div class="card-rule">${lang === 'ko' ? "?ìŒ + y ??<strong>yë¥?iesë¡?ë°”ê¿”??/strong>" : "Consonant + y ??change y to <strong>ies</strong>"}</div>
      <div class="card-row">
        <div class="card-side">
          <div class="card-label">${lang === 'ko' ? '?¨ìˆ˜ (1ê°?' : 'Singular (1)'}</div>
        ...
        <div class="card-side">
          <div class="card-label">${lang === 'ko' ? 'ë³µìˆ˜ (?¬ëŸ¬ ê°?' : 'Plural (many)'}</div>
      ...
      <div class="card-tip">${lang === 'ko' ? "y???„ì¹˜ë¥?ì°¾ê³ , 'ies'ë¡?ë°”ê¿”??" : "Find the y, then change it to ies!"}</div>
    `;
  ...
  function renderExamplesStep(stage, prog, stepEl) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = lang === 'ko'
      ? "?¼ìª½?€ ?¨ìˆ˜(?˜ë‚˜), ?¤ë¥¸ìª½ì? yë¥?'ies'ë¡?ë°”ê¾¼ ë³µìˆ˜ ?ˆë¬¸?´ì—?? ?Œë¦¬ ?´ì–´ ?½ì–´ ë³´ì„¸??"
  ...
  function renderSortingStep(stage, prog, stepEl) {
    ...
    body.innerHTML = lang === 'ko'
      ? "?¨ì–´ë¥??ŒëŸ¬ ?¨ìˆ˜ ?ëŠ” ë³µìˆ˜ ë°”êµ¬?ˆì— ?£ì–´ ë³´ì„¸?? ëª¨ë‘ ë§íˆë©??¤ìŒ?¼ë¡œ ê°????ˆì–´??"
    ...
    const pool = makeBucket('pool', lang === 'ko' ? '?¨ì–´ ëª¨ìŒ' : 'Word Pool');
    const bucketSing = makeBucket('singular', lang === 'ko' ? '?¨ìˆ˜' : 'Singular');
    const bucketPlu = makeBucket('plural', lang === 'ko' ? "ë³µìˆ˜ (y?’ies)" : "Plural (y?’ies)");
    ...
    checkBtn.onclick = () => {
      ...
      if (poolCount === 0 && wrongCount === 0) {
        ...
        message.textContent = lang === 'ko' ? "?„ë²½?´ìš”! y?’ies ê·œì¹™????ì°¾ì•˜?´ìš”." : "Awesome! You spotted every y?’ies plural.";
  ...
  function renderFinishStep(stage, prog, stepEl) {
    ...
    body.style.gap = '30px';
    body.innerHTML = lang === 'ko'
      ? "<div style=\"font-weight:800;color:#19777e\">y?’ies ê·œì¹™??ë°°ì› ?´ìš”!</div><div class=\"stars\">â­â­â­â­â­?/div>"
      : "<div style=\"font-weight:800;color:#19777e\">You learned the y?’ies rule!</div><div class=\"stars\">â­â­â­â­â­?/div>";
    stepEl.appendChild(body);
  ...
  function displayStep(index) {
    const steps = [lang === 'ko' ? '?¸ì–´ ? íƒ' : 'Choose Language', lang === 'ko' ? '1?¨ê³„' : 'Step 1', lang === 'ko' ? '2?¨ê³„' : 'Step 2', lang === 'ko' ? '3?¨ê³„' : 'Step 3', lang === 'ko' ? '?„ë£Œ' : 'Complete'];
    return steps[index] || '';
  }
  ...
  function buildExampleColumn(kind, list, currentLang) {
    ...
    titleWrap.textContent = kind === 'singular'
      ? (currentLang === 'ko' ? '?¨ìˆ˜ (?˜ë‚˜)' : 'Singular (one)')
      : (currentLang === 'ko' ? "ë³µìˆ˜ (y?’ies)" : "Plural (y?’ies)");
  ...
  function buildNavRow(onBack, onNext, currentLang, sfx) {
    ...
    const back = buildSecondaryButton(currentLang === 'ko' ? '?¤ë¡œ' : 'Back');
    back.onclick = () => { sfx?.('click'); onBack(); };
    const next = buildPrimaryButton(currentLang === 'ko' ? '?¤ìŒ' : 'Next');
    next.onclick = () => { onNext(); };
...
const fallbackSingular = [
  { id: 'fb_ies_baby', word: 'baby', article: 'singular', emoji: '?‘¶', exampleSentence: 'One baby is sleeping.', exampleSentenceKo: '?„ê¸° ??ëª…ì´ ?ê³  ?ˆì–´??' },
  { id: 'fb_ies_city', word: 'city', article: 'singular', emoji: '?™ï¸?, exampleSentence: 'One city is large.', exampleSentenceKo: '?„ì‹œ ?˜ë‚˜ê°€ ì»¤ìš”.' },
  { id: 'fb_ies_party', word: 'party', article: 'singular', emoji: 'ï¿?, exampleSentence: 'One party is fun.', exampleSentenceKo: '?Œí‹° ?˜ë‚˜ê°€ ?¬ë??ˆì–´??' },
  { id: 'fb_ies_fly', word: 'fly', article: 'singular', emoji: '?ª°', exampleSentence: 'One fly is on the table.', exampleSentenceKo: '?Œë¦¬ ??ë§ˆë¦¬ê°€ ?ì ?„ì— ?ˆì–´??' }
];

const fallbackPlural = [
  { id: 'fb_ies_babies', word: 'babies', article: 'plural', emoji: '?‘¶?‘¶', exampleSentence: 'Many babies are sleeping.', exampleSentenceKo: '?„ê¸°?¤ì´ ?ê³  ?ˆì–´??' },
  { id: 'fb_ies_cities', word: 'cities', article: 'plural', emoji: '?™ï¸ğŸ™ï¸', exampleSentence: 'Many cities are large.', exampleSentenceKo: 'ë§ì? ?„ì‹œ?¤ì´ ì»¤ìš”.' },
  { id: 'fb_ies_parties', word: 'parties', article: 'plural', emoji: 'ï¿½ï¿½', exampleSentence: 'Many parties are fun.', exampleSentenceKo: '?Œí‹°?¤ì? ?¬ë??ˆì–´??' },
  { id: 'fb_ies_flies', word: 'flies', article: 'plural', emoji: '?ª°?ª°', exampleSentence: 'Many flies are on the table.', exampleSentenceKo: '?Œë¦¬?¤ì´ ?ì ?„ì— ?ˆì–´??' }
];
