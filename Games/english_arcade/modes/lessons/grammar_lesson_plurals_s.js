...
export async function runGrammarLessonPluralsS(ctx = {}) {
  ...
  function renderIntroStep(stage, prog, stepEl) {
    ...
    intro.innerHTML = lang === 'ko'
      ? "?òÎÇò???åÎäî ?®Ïàò, ???¥ÏÉÅ?¥Î©¥ Î≥µÏàò?êÏöî. Î≥¥ÌÜµ Î≥µÏàò???ùÏóê 's'Î•?Î∂ôÏó¨??"
    ...
    const singularExample = singularList[0] || {
      ...
      exampleSentenceKo: 'Í≥∞Ïù¥ ??ÎßàÎ¶¨ ?àÏñ¥??',
      emoji: '?êª',
    };
    ...
    exampleCard.innerHTML = `
      <div class="card-heading">${lang === 'ko' ? '?àÏãúÎ°??ïÏù∏?¥Ïöî!' : 'Let\'s peek at an example!'}</div>
      <div class="card-row">
        <div class="card-side">
          <div class="card-label">${lang === 'ko' ? '?®Ïàò (1Í∞?' : 'Singular (1)'}</div>
          <div class="card-emoji">${escapeHtml(singularExample.emoji || '?êª')}</div>
        ...
        <div class="card-side">
          ...
          <div class="card-emoji">${escapeHtml(pluralExample.emoji || '?êª?êª')}</div>
      ...
      <div class="card-tip">${lang === 'ko' ? "???¥ÏÉÅ?¥Î©¥ ?ùÏóê 's'Î•?Î∂ôÏó¨??" : "Add an 's' when you have more than one!"}</div>
    `;
  ...
  function renderExamplesStep(stage, prog, stepEl) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = lang === 'ko'
      ? "?ºÏ™Ω?Ä ?®Ïàò(?òÎÇò), ?§Î•∏Ï™ΩÏ? Î≥µÏàò(?¨Îü¨ Í∞? ?àÎ¨∏?¥Ïóê?? ?åÎ¶¨ ?¥Ïñ¥ ?ΩÏñ¥ Î≥¥ÏÑ∏??"
  ...
  function renderSortingStep(stage, prog, stepEl) {
    ...
    body.innerHTML = lang === 'ko'
      ? "?®Ïàò ?êÎäî Î≥µÏàò Î∞îÍµ¨?àÏóê ?®Ïñ¥Î•??£Ïñ¥ Î≥¥ÏÑ∏?? Î™®Îëê ÎßûÌûàÎ©??§Ïùå?ºÎ°ú Í∞????àÏñ¥??"
    ...
    const bucketPlu = makeBucket('plural', lang === 'ko' ? 'Î≥µÏàò' : 'Plural');
    ...
    checkBtn.onclick = () => {
      ...
      if (poolCount === 0 && wrongCount === 0) {
        ...
        message.textContent = lang === 'ko' ? '?ïÎãµ?¥Ïóê?? ?®Ïàò?Ä Î≥µÏàòÎ•???Íµ¨Î∂Ñ?àÏñ¥??' : 'Great! You could spot singular and plural words.';
  ...
  function renderFinishStep(stage, prog, stepEl) {
    ...
    body.style.gap = '30px';
    body.innerHTML = lang === 'ko'
      ? '<div style="font-weight:800;color:#19777e">?¥Ï†ú ?®Ïàò?Ä Î≥µÏàòÎ•???Íµ¨Î≥Ñ?????àÏñ¥??</div><div class="stars">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>'
      : '<div style="font-weight:800;color:#19777e">You can now tell singular from plural ??awesome!</div><div class="stars">‚≠ê‚≠ê‚≠ê‚≠ê‚≠?/div>';
    stepEl.appendChild(body);
  ...
  function buildExampleColumn(kind, list, lang) {
    ...
    titleWrap.textContent = kind === 'singular'
      ? (lang === 'ko' ? '?®Ïàò (?òÎÇò)' : 'Singular (one)')
      : (lang === 'ko' ? 'Î≥µÏàò (?¨Îüø)' : 'Plural (more)');
