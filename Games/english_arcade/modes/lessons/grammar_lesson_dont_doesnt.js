...
export async function runGrammarLessonDontDoesnt(ctx = {}) {
  ...
  function renderExplainStep(stepEl, progress) {
    ...
    intro.innerHTML = lang === 'ko'
      ? "ë¶€?•ë¬¸??ë§Œë“¤ ??<b>don't</b> (I/you/we/they), <b>doesn't</b> (he/she/it)ë¥??¨ìš”. ë²„íŠ¼???ŒëŸ¬???ˆì œë¥??•ì¸??ë³´ì„¸??"
  ...
  function renderExamplesStep(stepEl, progress) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = lang === 'ko'
      ? "<b>I/You/We/They</b> ??<b>Don't</b><br/><b>He/She/It</b> ??<b>Doesn't</b><br/>ë²„íŠ¼???ŒëŸ¬????ë§ì? ?ˆì œë¥??•ì¸?˜ì„¸??"
    ...
    const categorySets = [
      {
        id: 'plural',
        label: lang === 'ko' ? '?‘¥ ë³µìˆ˜ ì£¼ì–´' : '?‘¥ Plural',
        examples: dontList,
        pointer: 0,
        negative: "DON'T",
      },
  ...
  function renderSortingStep(stepEl, progress) {
    ...
    intro.className = 'lesson-body';
    intro.innerHTML = (lang === 'ko')
      ? 'ì£¼ì–´ë¥??ŒëŸ¬ <b>Don\'t</b> ?ëŠ” <b>Doesn\'t</b> ë°”êµ¬?ˆì— ?£ì–´ ë³´ì„¸?? ëª¨ë‘ ë§ìœ¼ë©??¤ìŒ ?¨ê³„ë¡??˜ì–´ê°????ˆì–´??'
    ...
    const pool = makeBucket('pool', lang === 'ko' ? 'ì£¼ì–´ ëª¨ìŒ' : 'Subject Pool');
    ...
    checkBtn.onclick = () => {
      ...
      if (allCorrect) {
        ...
        message.textContent = lang === 'ko' ? '?„ë²½?´ìš”! don\'t/doesn\'të¥???ê³¨ë?´ìš”.' : 'Perfect! You used don\'t/doesn\'t correctly.';
        ...
        message.textContent = lang === 'ko' ? 'ë¹¨ê°„ ì£¼ì–´ë¥??¤ì‹œ ??²¨ ë³´ì„¸??' : 'Try again! Fix the red subjects.';
        stepEl.insertBefore(message, stepEl.firstChild);
      }
    };
  }
  ...
  function renderFinishStep(stepEl) {
    ...
    body.innerHTML = (lang === 'ko')
      ? '<div style="font-weight:800;color:#19777e">?´ì œ don\'t/doesn\'të¥?ë°”ë¥´ê²??¬ìš©?????ˆì–´??</div><div class="stars">â­â­â­â­â­?/div>'
      : '<div style="font-weight:800;color:#19777e">You now know when to use don\'t or doesn\'t!</div><div class="stars">â­â­â­â­â­?/div>';
  ...
  return [
    {
      ...
      sentenceKo: dontExample.exampleSentenceKo || '?˜ëŠ” ë¸Œë¡œì½œë¦¬ë¥?ì¢‹ì•„?˜ì? ?Šì•„??',
      tipEn: "Use 'don't' with I, you, we, they.",
      tipKo: "I, you, we, they???€??'don't'ë¥??¨ìš”.",
      label: '?‘¥ Don\'t'
    },
    {
      id: 'doesnt',
      negative: 'doesn\'t',
      emoji: doesntExample.emoji || '?“º',
      sentenceEn: doesntExample.exampleSentence || 'He doesn\'t watch TV.',
      sentenceKo: doesntExample.exampleSentenceKo || 'ê·¸ëŠ” TVë¥?ë³´ì? ?Šì•„??',
      tipEn: "Use 'doesn't' with he, she, it.",
      tipKo: "he, she, it???€??'doesn't'ë¥??¨ìš”.",
      label: '?‘¤ Doesn\'t'
    }
  ];
}
...
const fallbackDont = [
  { id: 'fb_dont_like', word: 'I', exampleSentence: 'I don\'t like broccoli.', exampleSentenceKo: '?˜ëŠ” ë¸Œë¡œì½œë¦¬ë¥?ì¢‹ì•„?˜ì? ?Šì•„??', emoji: '?¥¦' },
  { id: 'fb_dont_need', word: 'You', exampleSentence: 'You don\'t need to hurry.', exampleSentenceKo: '?œë‘ë¥??„ìš” ?†ì–´??', emoji: '?ƒ' },
  { id: 'fb_dont_want', word: 'We', exampleSentence: 'We don\'t need help.', exampleSentenceKo: '?°ë¦¬???„ì????„ìš” ?†ì–´??', emoji: '?™' },
  { id: 'fb_dont_play', word: 'They', exampleSentence: 'They don\'t play soccer.', exampleSentenceKo: 'ê·¸ë“¤?€ ì¶•êµ¬ë¥??˜ì? ?Šì•„??', emoji: '?? }
];

const fallbackDoesnt = [
  { id: 'fb_doesnt_watch', word: 'He', exampleSentence: 'He doesn\'t watch TV.', exampleSentenceKo: 'ê·¸ëŠ” TVë¥?ë³´ì? ?Šì•„??', emoji: '?“º' },
  { id: 'fb_doesnt_want', word: 'She', exampleSentence: 'She doesn\'t want that.', exampleSentenceKo: 'ê·¸ë???ê·¸ê²ƒ???í•˜ì§€ ?Šì•„??', emoji: '?‘©' },
  { id: 'fb_doesnt_work', word: 'It', exampleSentence: 'It doesn\'t work.', exampleSentenceKo: 'ê·¸ê²ƒ?€ ?‘ë™?˜ì? ?Šì•„??', emoji: '?™ï¸' },
  { id: 'fb_doesnt_like', word: 'John', exampleSentence: 'John doesn\'t like vegetables.', exampleSentenceKo: 'ì¡´ì? ì±„ì†Œë¥?ì¢‹ì•„?˜ì? ?Šì•„??', emoji: '?¥—' }
];
