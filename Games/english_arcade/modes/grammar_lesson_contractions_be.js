...
export async function runGrammarLessonContractionsBe(ctx = {}) {
  ...
  try {
    sessionId = startSession({
      ...
      listName: grammarName || 'Contractions (Be)',
  ...
  function render() {
    ...
    title.textContent = grammarName || (lang === 'ko' ? 'Be 동사 축약형' : 'Contractions (Be)');
  ...
  progress.textContent = formatStepLabel(stepIndex);
    ...
    body.innerHTML = lang === 'ko'
      ? '<b>축약형</b>은 두 단어를 밀어서 한 단어로 만드는 거예요! 예: <b>I am → I\'m</b>'
  ...
  progress.textContent = formatStepLabel(stepIndex);
    ...
    contractionTitle.textContent = lang === 'ko' ? '축약형 선택' : 'Choose Ending';
    ...
    const handlePronounClick = (btn) => {
      ...
      setStatus(lang === 'ko' ? '축약형을 골라 보세요!' : 'Now pick the ending!');
    };

    const handleOptionClick = (btn) => {
      if (btn.disabled) return;
      if (!activePronoun) {
        setStatus(lang === 'ko' ? '먼저 주어를 골라야 해요.' : 'Pick a subject first.', 'error');
        return;
      }
      ...
      if (chosen === expected) {
        ...
        setStatus(lang === 'ko' ? '다시 시도해 봐요!' : 'Oops, try again!', 'error');
        setTimeout(() => {
  ...
  progress.textContent = formatStepLabel(stepIndex);
    ...
    body.style.gap = '18px';
    body.innerHTML = lang === 'ko'
      ? '<div style="font-weight:800;color:#19777e">축약형을 완벽하게 배웠어요!</div><div class="stars">⭐⭐⭐⭐⭐</div>'
  ...
  function makeNavButton(kind, currentLang, onClick) {
    ...
    if (kind === 'back') {
      ...
      btn.textContent = currentLang === 'ko' ? '게임 시작' : "Let's Play";
      btn.onclick = () => {
        onClick();
      };
    } else {
  ...
  function makeSlider(row) {
    ...
    label.textContent = lang === 'ko'
      ? `${row.pronoun} + ${row.fullForm} 를 합쳐봐요`
  ...
  contractionSpan.style.transform = 'scale(0.9)';
    ...
    tip.textContent = lang === 'ko'
      ? `슬라이더를 움직이면 ${row.contraction} 이(가) 나타나요`
...
const fallbackItems = [
  { id: 'i_am', word: 'I', pronoun: 'I', fullForm: 'am', contraction: "I'm", exampleSentence: "I'm ready to go!", exampleSentenceKo: "난 출발할 준비가 됐어요!", explanation: "I am becomes I'm." },
  { id: 'you_are', word: 'You', pronoun: 'You', fullForm: 'are', contraction: "You're", exampleSentence: "You're a great friend.", exampleSentenceKo: "넌 정말 좋은 친구야.", explanation: "You are becomes You're." },
  { id: 'she_is', word: 'She', pronoun: 'She', fullForm: 'is', contraction: "She's", exampleSentence: "She's playing piano.", exampleSentenceKo: "그녀는 피아노를 치고 있어요.", explanation: "She is becomes She's." },
  { id: 'he_is', word: 'He', pronoun: 'He', fullForm: 'is', contraction: "He's", exampleSentence: "He's my teacher.", exampleSentenceKo: "그는 내 선생님이에요.", explanation: "He is becomes He's." },
  { id: 'it_is', word: 'It', pronoun: 'It', fullForm: 'is', contraction: "It's", exampleSentence: "It's sunny outside.", exampleSentenceKo: "밖에는 해가 나요.", explanation: "It is becomes It's." },
  { id: 'we_are', word: 'We', pronoun: 'We', fullForm: 'are', contraction: "We're", exampleSentence: "We're ready to learn.", exampleSentenceKo: "우리는 배울 준비가 됐어요.", explanation: "We are becomes We're." },
