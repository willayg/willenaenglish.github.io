...
export async function showGrammarModeSelector({ grammarFile, grammarName, onModeChosen, onClose }) {
  ...
  try { window.__WA_IS_GRAMMAR__ = true; window.__WA_LAST_GRAMMAR__ = { grammarFile: currentGrammarFile, grammarName: currentGrammarName }; } catch {}
  try {
    if (!historyManager.isBackNavigation) {
      historyManager.navigateToGrammarModeSelector({ grammar: { grammarFile: currentGrammarFile, grammarName: currentGrammarName } });
    }
  } catch {}
  ...
  const modes = [
    { mode: 'lesson', title: 'How To Win', svgPath: JIGSAW_ICON_SRC, styleMode: 'meaning', showStars: false },
    { mode: 'choose', title: 'Choose', svgPath: './ui/rainbow.svg', styleMode: 'multi_choice', sessionMode: 'grammar_mode' },
    { mode: 'fill_gap', title: 'Fill the Gap', svgPath: './assets/Images/icons/translate-and-spell.png?v=20250910a', styleMode: 'missing_letter', sessionMode: 'grammar_fill_gap' },
    { mode: 'unscramble', title: 'Unscramble', svgPath: './assets/Images/icons/reading.png?v=20250910a', styleMode: 'sentence', sessionMode: 'grammar_sentence_unscramble' }
  ];
  ...
  headerDiv.innerHTML = `<button type="button" class="grammar-rule-btn" aria-label="${currentGrammarName}: rules and tips">${currentGrammarName}</button>`;
  ...
  titleDiv.textContent = m.title;
    ...
    const borderColor = BORDER_COLORS[borderKey] || '#40D4DE';
    btn.style.border = `3px solid ${borderColor}`;
    btn.style.boxShadow = '0 6px 14px rgba(166, 234, 246, 0.35)';

    btn.addEventListener('click', () => {
  if (onModeChosen) onModeChosen({ mode: m.mode, grammarFile: currentGrammarFile, grammarName: currentGrammarName });
    });
