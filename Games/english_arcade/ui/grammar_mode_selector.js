...
export async function showGrammarModeSelector({ grammarFile, grammarName, grammarConfig, onModeChosen, onClose }) {
  ...
  const modes = [
    { mode: 'lesson', title: 'How To Win', svgPath: './assets/Images/icons/win.svg', styleMode: 'meaning', sessionMode: ['grammar_lesson', 'lesson', 'grammar_lesson_it_vs_they', 'grammar_lesson_am_are_is', 'grammar_lesson_this_that', 'grammar_lesson_these_those'] },
  ...
  try {
    headerDiv.querySelector('.grammar-rule-btn')?.addEventListener('click', () => {
      const message = currentGrammarConfig?.ruleHint || 'Rules & tips coming soon';
      try {
        (window.WordArcade && window.WordArcade.inlineToast)
          ? window.WordArcade.inlineToast(message)
          : alert(message);
      } catch { alert(message); }
    });
  } catch {}
