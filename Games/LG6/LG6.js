import lg6Sentences from './LG6_contents.js';
...
function startGame() {
  ...
  questions = shuffle(lg6Sentences);
  current = 0;
  score = 0;
  timeLeft = 90;
...
async function displayHighScores() {
  ...
  try {
    const response = await WillenaAPI.fetch('/.netlify/functions/submit_score?game=LG6'); // <-- FIXED
    if (!response.ok) {
...
async function submitScore() {
  ...
  await WillenaAPI.fetch('/.netlify/functions/submit_score', {
    ...
    body: JSON.stringify({ name, score, game: "LG6" }) // <-- FIXED
  });
...
function startPractice() {
  ...
  endScreen.style.display = "none";
  timerDiv.style.display = "none";
  questions = shuffle(lg6Sentences).slice(0, 10);
  current = 0;
  practiceResults = [];
  practiceRight = [];
  practiceWrong = [];
