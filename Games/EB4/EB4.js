import eb4Sentences from './EB4_contents.js';
...
function startGame() {
  ...
  questions = shuffle(eb4Sentences);
  current = 0;
  score = 0;
  timeLeft = 45;
...
async function displayHighScores() {
  ...
  try {
    const response = await WillenaAPI.fetch('/.netlify/functions/submit_score?game=EB4');
    if (!response.ok) {
...
function startPractice() {
  ...
  endScreen.style.display = "none";
  timerDiv.style.display = "none";
  questions = shuffle(eb4Sentences).slice(0, 10);
  current = 0;
  practiceResults = [];
  practiceRight = [];
  practiceWrong = [];
