...
export async function onRequestPost(context) {
  ...
  if (!key) {
    return new Response(JSON.stringify({ error: 'OpenAI secret unavailable in Pages environment' }), {
      status: 500,
  ...
  if (mode === 'classify') {
    ...
    if (!words.length) {
      return new Response(JSON.stringify({ error:'Missing words' }), { status:400, headers:{'Content-Type':'application/json'} });
    }
    ...
    if (!resp.ok) {
      return new Response(JSON.stringify({ error:'OpenAI classification failed', status:resp.status, detail:(await resp.text()).slice(0,500) }), {
  ...
  return new Response(resp.body,{status:200,headers:{'Content-Type':'audio/mpeg','Cache-Control':'no-store'}});
}
