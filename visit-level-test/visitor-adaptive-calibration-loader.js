...
fetch(sourceUrl,{cache:'no-store'}).then(function(response){
 ...
 source=replaceOnce(source,
  ...
  'function startAbility(years,grade){var visitor=window.WillenaVisitorV2Config,teacher=visitor&&Number(visitor.teacherStartLevel);if(visitor&&visitor.enabled&&Number.isFinite(teacher))return clamp(teacher,1,12);var matrix={1:{0:1,1:1.5,2:2,4:2.5,6:3},2:{0:1,1:2,2:3,4:4,6:5},4:{0:1.5,1:2.5,2:4,4:5.5,6:6.5},6:{0:2,1:3,2:4.5,4:6,6:7.5},8:{0:2.5,1:3.5,2:5,4:7,6:8.5},9:{0:3,1:4,2:5.5,4:7.5,6:9}},stage=matrix[Number(grade)]||matrix[1],value=stage[Number(years)];return clamp(value==null?1:value,1,12)}',
  'visitor teacher starting level');
 ...
 source=replaceOnce(source,
  ...
  'function visitorV2(){var v=window.WillenaVisitorV2Config;return !!(v&&v.enabled&&v.version==="adaptive-2026-09-v2")}'+
  ...
  'function adaptiveTarget(){var rows=sectionRows(),n=rows.length,base=S.fullMode?S.sectionBaseAbility:S.startAbility;if(!n)return base;var last=rows[rows.length-1],v2=visitorV2();if(n===1)return clamp(base+(last.correct?(v2?.35:1):(v2?-.35:-1)),1,12);if(n===2){var firstTwoRight=rows[0].correct&&rows[1].correct,firstTwoWrong=!rows[0].correct&&!rows[1].correct;return clamp(base+(firstTwoRight?(v2?.9:2):firstTwoWrong?(v2?-.9:-2):0),1,12)}var target=estimateAbility(rows,S.ability),recent=rows.slice(-3),lastTwo=recent.slice(-2);if(lastTwo.every(function(x){return x.correct}))target=Math.max(target,(Number(last.level)||target)+(v2?.75:1));if(recent.every(function(x){return x.correct}))target=Math.max(target,(Number(last.level)||target)+(v2?1:1.5));if(lastTwo.every(function(x){return !x.correct}))target=Math.min(target,(Number(last.level)||target)-(v2?.75:1));return clamp(target,1,12)}function finalEstimate(){if(!S.fullMode)return estimateAbility(S.answers,S.startAbility);var sections=fullSections(),scores=[];sections.forEach(function(section){var rows=S.answers.filter(function(a){return section.types.indexOf(a.type)>=0});if(rows.length)scores.push(estimateAbility(rows,S.startAbility))});if(!scores.length)return estimateAbility(S.answers,S.startAbility);return clamp(scores.reduce(function(sum,x){return sum+x},0)/scores.length,1,12)}'+
 ...
 script.text=source+'\n//# sourceURL=app-classic-calibrated-visitor-v2.js';
 document.head.appendChild(script);
 script.remove();
}).catch(function(error){
 console.error(error);
 var app=document.querySelector('#app');
 if(app)app.innerHTML='<section class="screen"><h2>Could not load the visitor test</h2><p class="error">Please refresh the page.</p></section>';
});
})();