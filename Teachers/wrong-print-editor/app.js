...
function answerKeyHtml(entries){const groups=[];const vocab=entries.filter(x=>isVocab(x.r));if(vocab.length)groups.push(['Vocabulary',vocab]);const seen=new Map();for(const x of entries.filter(x=>!isVocab(x.r))){const k=skillKey(x.r);if(!seen.has(k))seen.set(k,[]);seen.get(k).push(x)}for(const [k,rows] of seen)groups.push([skillLabel(k),rows]);return groups.map(([label,rows])=>`<section class="answer-skill"><h3>${esc(label)}</h3><div class="answer-grid">${rows.map((x,i)=>`<div><b>${i+1}.</b> ${esc(printAnswer(x.r))}</div>`).join('')}</div></section>`).join('')}
