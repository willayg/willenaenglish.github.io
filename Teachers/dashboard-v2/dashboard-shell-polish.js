...
<a class="app-card utility-card" href="/students/question-render-lab/"><span class="chip">RENDER</span><h3>내신 Render Lab</h3><p>Browse and inspect live 내신 and test-prep questions.</p></a>
<a class="app-card utility-card" href="/Teachers/tools/level-test-render/"><span class="chip">RENDER</span><h3>Level Test Render Lab</h3><p>Browse the published level-test bank using the live visitor renderer.</p></a>
...
function boot(){mountRailToggle();mountUtilities();refreshRailIcons();normalizeNavOrder();openDefaultApps();const rail=$('.rail'),mobile=$('.mobile-tabs');if(rail)new MutationObserver(()=>{refreshRailIcons();normalizeNavOrder()}).observe(rail,{childList:true,subtree:true});if(mobile)new MutationObserver(normalizeNavOrder).observe(mobile,{childList:true,subtree:true})}
