...
function rebuildThreeKpis(detail){const body=$('#naDiagBody'),overview=$('.na-diag-view[data-view="overview"]',body);if(!overview)return;const kpis=$('.na-kpis',overview);if(!kpis)return;const old=$$('.na-kpi',kpis),acc=old.find(x=>x.classList.contains('accuracy')),wrong=old.find(x=>x.classList.contains('wrong')),activity=old.find(x=>x.classList.contains('activity'));if(!acc||!wrong||!activity)return;const allTime=$('.na-kpi-value',acc)?.textContent||'—';const wrongVal=$('.na-kpi-value',wrong)?.textContent||'0';const attempts=$('.na-kpi-value',activity)?.textContent||'0';const mini=$$('.na-kpi-mini span',activity);const coverage=mini[0]?.querySelector('b')?.textContent||'—';const days=mini[1]?.querySelector('b')?.textContent||'0';const recent=detail?.summary?.recent_accuracy_3d==null?'—':`${detail.summary.recent_accuracy_3d}%`;const recentAttempts=detail?.summary?.recent_attempts_3d||0;kpis.dataset.signed='1';kpis.className='na-kpis na-signed-three';kpis.innerHTML=`<button class="na-kpi accuracy" data-local="accuracy"><div class="na-kpi-label">Accuracy</div><div class="na-dual-accuracy"><div class="metric"><strong>${allTime}</strong><span>All time</span></div><div class="metric"><strong>${recent}</strong><span>Last 3 days${recentAttempts?` · ${recentAttempts} Q`:''}</span></div></div><div class="na-kpi-open">Analyse ›</div></button><button class="na-kpi wrong" data-local="wrong"><div class="na-kpi-label">오답</div><div class="na-kpi-value">${wrongVal}</div><div class="na-kpi-note">Unresolved</div><div class="na-kpi-open">View wrong answers ›</div></button><button class="na-kpi activity" data-local="activity"><div class="na-kpi-label">Activity</div><div class="na-activity-stats"><div class="na-activity-stat"><strong>${attempts}</strong><span>Questions</span></div><div class="na-activity-stat"><strong>${coverage}</strong><span>Coverage</span></div><div class="na-activity-stat"><strong>${days}</strong><span>Active days</span></div></div><div class="na-kpi-open">View activity ›</div></button>`;$$('[data-local]',kpis).forEach(b=>b.onclick=()=>showDiag(b.dataset.local))}
async function fetchDetailForRow(row){const t=token();if(!t)return null;const studentId=row.dataset.student||'';const groupId=row.closest('.na-exam')?.dataset.group||'';if(!studentId||!groupId)return null;try{const gr=await fetch(`${GROUP_API}?action=teacher_groups`,{headers:{Authorization:`Bearer ${t}`,apikey:KEY},credentials:'omit',cache:'no-store'});const gj=await gr.json();const group=(gj.groups||[]).find(x=>String(x.group?.id)===String(groupId));const member=(group?.members||[]).find(m=>String(m.student?.id||m.plan?.student_id||'')===String(studentId));const planId=member?.plan?.id||member?.plan_id||'';const q=new URLSearchParams({action:'student_detail',student_id:studentId});if(planId)q.set('plan_id',planId);const r=await fetch(`${INSIGHTS_API}?${q}`,{headers:{Authorization:`Bearer ${t}`,apikey:KEY},credentials:'omit',cache:'no-store'});const j=await r.json();return r.ok&&j.success!==false?j:null}catch{return null}}
function init(){addStyles();const groups=$('#naFreshGroups');if(groups){new MutationObserver(styleExamCards).observe(groups,{childList:true,subtree:true});styleExamCards()}const diag=$('#naDiagBody');if(diag){new MutationObserver(()=>{const overview=$('.na-diag-view[data-view="overview"]',diag);if(overview&&!$('.na-kpis.na-signed-three',overview))rebuildThreeKpis(pendingDetail)}).observe(diag,{childList:true,subtree:true})}document.addEventListener('click',e=>{if(e.target.closest('[data-view="naesin"]'))queueMicrotask(styleExamCards);const row=e.target.closest('.na-member');if(row){pendingDetail=null;fetchDetailForRow(row).then(d=>{pendingDetail=d;rebuildThreeKpis(d)});setTimeout(()=>rebuildThreeKpis(null),80)}},true)}
