...
function init(){addStyles();const groups=$('#naFreshGroups');if(groups){new MutationObserver(styleExamCards).observe(groups,{childList:true,subtree:true});styleExamCards()}const diag=$('#naDiagBody');if(diag){new MutationObserver(rebuildFiveKpis).observe(diag,{childList:true,subtree:true});rebuildFiveKpis()}document.addEventListener('click',e=>{if(e.target.closest('[data-view="naesin"]'))queueMicrotask(styleExamCards);if(e.target.closest('.na-member'))queueMicrotask(rebuildFiveKpis)},true)}
