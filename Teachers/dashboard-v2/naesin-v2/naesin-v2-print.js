...
function urlFor(ctx={},autoPdf=false){const q=new URLSearchParams();if(ctx.studentId)q.set('student_id',ctx.studentId);if(ctx.planId)q.set('plan_id',ctx.planId);if(ctx.student)q.set('student',ctx.student);if(ctx.exam)q.set('exam',ctx.exam);if(ctx.wrongFilter==='today')q.set('wrong_filter','today');if(autoPdf){q.set('auto_pdf','1');q.set('auto_close','1')}return `${BASE}?${q.toString()}`}
...
window.NaesinV2Print={openEditor,makePdf,version:'r9.2-p8'};
