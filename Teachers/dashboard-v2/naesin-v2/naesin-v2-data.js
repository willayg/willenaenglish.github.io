...
const GROUP_EDGE='https://fiieuiktlsivwfgyivai.supabase.co/functions/v1/test-prep-groups-v2';
...
const diag={requests:0,cacheHits:0,cacheMisses:0,lastMs:null,source:'shared-naesin-v2-data-r12'};
...
async function authedJson(url,opts={}){let access=await accessToken();if(!access)throw new Error('AUTH_REQUIRED');const run=t=>fetch(url,{...opts,headers:{apikey:API_KEY,Authorization:`Bearer ${t}`,...(opts.headers||{})},cache:'no-store',credentials:'omit'}),started=performance.now();diag.requests++;let r=await run(access);if(r.status===401){access=await refreshToken();if(access)r=await run(access)}const payload=await r.json().catch(()=>({}));diag.lastMs=performance.now()-started;if(r.status===401)throw new Error('AUTH_REQUIRED');if(!r.ok||payload?.success===false)throw new Error(payload?.error||`Naesin data request failed (${r.status})`);return payload}
...
async function loadGroups({force=false}={}){if(force){groupsPromise=null;latestGroups=[]}if(groupsPromise){diag.cacheHits++;return groupsPromise}diag.cacheMisses++;groupsPromise=authedJson(`${GROUP_EDGE}?action=list_groups`).then(payload=>{const rows=Array.isArray(payload.groups)?payload.groups:[];latestGroups=rows.filter(item=>(item?.group||item)?.active!==false);return latestGroups}).catch(error=>{groupsPromise=null;latestGroups=[];throw error});return groupsPromise}
