...
function scoreClass(v){const n=num(v);return n==null?'':n>=80?'score-good':n>=50?'score-mid':'score-low'}
function examType(v){return v==='final'?'기말고사':'중간고사'}
function dday(date){if(!date)return '—';const end=new Date(`${String(date).slice(0,10)}T00:00:00`),now=new Date();now.setHours(0,0,0,0);const n=Math.ceil((end-now)/86400000);if(Number.isNaN(n))return '—';return n===0?'D-DAY':n>0?`D-${n}`:`D+${Math.abs(n)}`}
...
function renderLoading(){const box=q('#na2Tests');if(box&&!state.loaded)box.innerHTML='<div class="na2-empty">시험 정보를 불러오는 중…</div>'}
function renderError(message){const box=q('#na2Tests');if(!box)return;box.innerHTML=`<div class="na2-empty"><b>내신 정보를 불러오지 못했습니다.</b><span>${esc(message||'Unknown error')}</span><button type="button" class="na2-retry">다시 불러오기</button></div>`;q('.na2-retry',box)?.addEventListener('click',()=>ensureLoaded({force:true}))}
function renderEmpty(){const box=q('#na2Tests');if(!box)return;box.innerHTML=`<div class="na2-empty"><b>아직 진행 중인 시험이 없습니다.</b><span>시험 대비를 추가하면 여기에 학생 매트릭스가 표시됩니다.</span><button class="na2-add" type="button" data-empty-add>+ 시험 대비 추가</button></div>`;q('[data-empty-add]',box)?.addEventListener('click',()=>window.NaesinV2Editor?.openCreate?.())}
...
function todayCell(today){const questions=Math.max(0,Number(today?.questions)||0),accuracy=num(today?.accuracy);return `<td class="na2-stat na2-today"><strong>${esc(questions)}</strong><small>${accuracy==null?'—':esc(pct(accuracy))} 정확도</small></td>`}
function reviewCell(review){const r=review||{},active=Math.max(0,Number(r.active)||0),waiting=Math.max(0,Number(r.waiting)||0),total=r.total==null?active+waiting:Math.max(0,Number(r.total)||0);return `<td class="na2-stat na2-review-cell"><strong>${esc(total)}</strong><small><span class="na2-review-active">활성 ${esc(active)}</span> · 대기 ${esc(waiting)}</small></td>`}
function studentRow(member){const skills=skillMap(member),s=member?.student||{},summary=member?.summary||{};return `<tr data-student-id="${esc(member.student_id||s.id||'')}" data-plan-id="${esc(member.plan_id||'')}"><td class="na2-student-cell"><button type="button" class="na2-student-btn" data-student><span>${esc(nameOf(s))}</span><i>›</i></button></td><td class="na2-stat na2-overall"><strong class="${scoreClass(summary.recent150_accuracy)}">${esc(pct(summary.recent150_accuracy))}</strong><small>${esc(summary.recent150_count||0)}문항</small></td>${PRACTICES.map(([k])=>statCell(skills.get(k))).join('')}${todayCell(member?.today)}${reviewCell(member?.review)}</tr>`}
function matrixBody(item){const members=Array.isArray(item?.members)?item.members:[];return members.length?members.map(studentRow).join(''):'<tr><td colspan="11" class="na2-no-students">학생이 없습니다.</td></tr>'}
function testShell(item){const g=groupOf(item),memberCount=Array.isArray(item?.members)?item.members.length:null,range=[...(g.scope?.lessons||[]).map(x=>x.lesson),...(g.scope?.external_passages||[]).map(x=>x.label||x.lesson||x.unit_label)].filter(Boolean).join(' · ');return `<article class="na2-test" data-group-id="${esc(g.id)}"><header class="na2-test-head"><div class="na2-test-copy"><div class="na2-title-row"><h2>${esc(g.school||'학교')}</h2><span class="na2-dday">${esc(dday(g.end_date))}</span></div><div class="na2-test-meta"><span>${esc(g.term||'')}학기 ${esc(examType(g.exam_type))}</span><span>${esc(g.book_label||g.book_key||'교재 미설정')}</span><span>${esc(formatDate(g.end_date))}</span>${memberCount!=null?`<span>${memberCount}명</span>`:''}</div>${range?`<div class="na2-range">${esc(range)}</div>`:''}</div><div class="na2-test-actions"><button type="button" class="na2-assign-open" data-assign>과제 보내기</button><div class="na2-menu-wrap"><button type="button" class="na2-more" data-more aria-label="시험 메뉴">•••</button><div class="na2-menu" data-menu hidden><button type="button" data-edit>수정</button><button type="button" data-archive>보관</button></div></div></div></header><div class="na2-matrix-wrap"><table class="na2-matrix"><thead><tr><th>학생</th><th class="overall">전체 최근</th>${PRACTICES.map(([,label])=>`<th>${label}</th>`).join('')}<th class="today">오늘 활동</th><th class="na2-review-head">오답</th></tr></thead><tbody>${matrixBody(item)}</tbody></table></div></article>`}
function bindMatrixRows(test,groupId){qa('[data-student]',test).forEach(btn=>btn.addEventListener('click',()=>{const tr=btn.closest('tr');window.NaesinV2StudentDetail?.open?.(tr?.dataset.studentId,tr?.dataset.planId,groupId)}))}
function bindTest(test){const id=test.dataset.groupId,more=q('[data-more]',test),menu=q('[data-menu]',test);q('[data-assign]',test)?.addEventListener('click',()=>window.NaesinV2Assignment?.open?.(id));more?.addEventListener('click',e=>{e.stopPropagation();qa('.na2-menu').forEach(m=>{if(m!==menu)m.hidden=true});menu.hidden=!menu.hidden});q('[data-edit]',test)?.addEventListener('click',()=>{menu.hidden=true;window.NaesinV2Editor?.openEdit?.(id)});q('[data-archive]',test)?.addEventListener('click',()=>{menu.hidden=true;window.NaesinV2Editor?.archive?.(id)});bindMatrixRows(test,id)}
function renderGroups(){const box=q('#na2Tests');if(!box)return;if(!state.groups.length){renderEmpty();return}box.innerHTML=state.groups.map(testShell).join('');qa('.na2-test',box).forEach(bindTest)}
