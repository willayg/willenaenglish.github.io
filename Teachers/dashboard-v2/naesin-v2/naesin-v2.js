...
function mountView(){const ws=q('.workspace');if(!ws||q(`#view-${VIEW}`))return;const sec=document.createElement('section');sec.className='view na2-view';sec.id=`view-${VIEW}`;sec.innerHTML=`<div class="na2-shell"><div class="na2-head"><div><div class="na2-title-with-rev"><h1>내신 V2</h1><span class="na2-rev-badge">${REV}</span></div><p>진행 중인 시험</p></div><button class="na2-add" id="na2Add" type="button">+ 시험 대비 추가</button></div><div class="na2-tests" id="na2Tests"><div class="na2-empty">불러오는 중…</div></div></div>`;ws.appendChild(sec);q('#na2Add',sec)?.addEventListener('click',()=>window.NaesinV2Editor?.openCreate?.())}
function renderLoading(){const box=q('#na2Tests');if(box)box.innerHTML='<div class="na2-empty">시험 정보를 불러오는 중…</div>'}
...
function testShell(item){const g=groupOf(item),memberCount=Array.isArray(item?.members)?item.members.length:null,range=(g.scope?.lessons||[]).map(x=>x.lesson).filter(Boolean).join(' · ');return `<article class="na2-test" data-group-id="${esc(g.id)}"><header class="na2-test-head"><div class="na2-test-copy"><div class="na2-title-row"><h2>${esc(g.school||'학교')}</h2><span class="na2-dday">${esc(dday(g.end_date))}</span></div><div class="na2-test-meta"><span>${esc(g.term||'')}학기 ${esc(examType(g.exam_type))}</span><span>${esc(g.book_label||g.book_key||'교재 미설정')}</span><span>${esc(formatDate(g.end_date))}</span>${memberCount!=null?`<span>${memberCount}명</span>`:''}</div>${range?`<div class="na2-range">${esc(range)}</div>`:''}</div><div class="na2-menu-wrap"><button type="button" class="na2-more" data-more aria-label="시험 메뉴">•••</button><div class="na2-menu" data-menu hidden><button type="button" data-edit>수정</button><button type="button" data-archive>보관</button></div></div></header><div class="na2-matrix-wrap"><table class="na2-matrix"><thead><tr><th>학생</th><th class="overall">전체 최근</th>${PRACTICES.map(([,label])=>`<th>${label}</th>`).join('')}</tr></thead><tbody><tr class="na2-matrix-loading"><td colspan="9">학생 통계를 불러오는 중…</td></tr></tbody></table></div></article>`}
function renderGroups(){const box=q('#na2Tests');if(!box)return;if(!state.groups.length){renderEmpty();return}box.innerHTML=state.groups.map(testShell).join('');qa('.na2-test',box).forEach(test=>{const id=test.dataset.groupId,more=q('[data-more]',test),menu=q('[data-menu]',test);more?.addEventListener('click',e=>{e.stopPropagation();qa('.na2-menu',box).forEach(m=>{if(m!==menu)m.hidden=true});menu.hidden=!menu.hidden});q('[data-edit]',test)?.addEventListener('click',()=>{menu.hidden=true;window.NaesinV2Editor?.openEdit?.(id)});q('[data-archive]',test)?.addEventListener('click',()=>{menu.hidden=true;window.NaesinV2Editor?.archive?.(id)});loadMatrixInto(test,id)})}
function skillMap(member){const map=new Map();(member?.skills||[]).forEach(s=>map.set(String(s.practice_type||''),s));return map}
function statCell(row){if(!row||!Number(row.recent_count))return '<td class="na2-stat empty"><strong>—</strong><small>0문항</small></td>';return `<td class="na2-stat"><strong>${esc(pct(row.recent_accuracy))}</strong><small>${esc(row.recent_count)}문항</small></td>`}
function studentRow(member){const skills=skillMap(member),s=member?.student||{},summary=member?.summary||{};return `<tr data-student-id="${esc(member.student_id||s.id||'')}" data-plan-id="${esc(member.plan_id||'')}"><td class="na2-student-cell"><button type="button" class="na2-student-btn" data-student><span>${esc(nameOf(s))}</span><i>›</i></button></td><td class="na2-stat na2-overall"><strong>${esc(pct(summary.recent150_accuracy))}</strong><small>${esc(summary.recent150_count||0)}문항</small></td>${PRACTICES.map(([k])=>statCell(skills.get(k))).join('')}</tr>`}
...
function mount(){mountDesktopNav();mountMobileNav();mountView();document.addEventListener('click',e=>{if(!e.target.closest('.na2-menu-wrap'))qa('.na2-menu').forEach(m=>m.hidden=true)});window.NaesinV2={show,mount,refresh:()=>ensureLoaded({force:true}),version:'r8.2'};console.info(`[Naesin V2] ${REV} mounted`)}
