(()=>{
  ...
  let userId='',classRows=[],classesLoaded=false,classesLoading=null,classOrderLoaded=false;
  ...
  async function loadClasses(){try{const d=await edge(CLASS_API);classRows=d.classes||[];window.__adminClassRows=classRows;const names=classRows.map(c=>c.name).filter(Boolean);if(names.length){canonical.splice(0,canonical.length,...names);buildFilters();renderClasses()}classesLoaded=true}catch(e){console.error(e);showToast(e.message,{error:true});throw e}}
  async function ensureClassesLoaded(){
    if(classesLoaded)return classRows;
    if(classesLoading)return classesLoading;
    classesLoading=loadClasses().finally(()=>{classesLoading=null});
    if(!classOrderLoaded){classOrderLoaded=true;import('/Teachers/admin/admin-class-order.js?v=20260901-compact1').catch(e=>console.warn('[Admin class order] failed to load',e))}
  ...
  window.AdminClassEdge={edge,resolveUserId,CLASS_API,CONTENT_API,refresh:loadClasses,ensureLoaded:ensureClassesLoaded};
  button.onclick=async()=>{await ensureClassesLoaded().catch(()=>{});open()};modal.onclick=e=>{if(e.target===modal)close()};document.addEventListener('click',e=>{if(!e.target.closest('.book-autocomplete'))closeMenus()});document.addEventListener('willena-language-change',applyLanguage);applyLanguage();
  document.querySelectorAll('[data-page="classes"]').forEach(btn=>btn.addEventListener('click',()=>{void ensureClassesLoaded()}));
})();
