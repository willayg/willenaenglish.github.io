...
function openAdd(){const cls=[['','No class'],...classes().map(c=>[c,c])];modal('Add student',`<div class="admin-form-grid">${input('English name','mName')}${input('Korean name','mKorean')}${input('Username','mUsername','','text','autocomplete="off"')}${input('Temporary password','mPassword','','text','autocomplete="new-password"')}${selectField('Class','mClass',cls,'')}${input('Grade','mGrade')}${input('School','mSchool')}${input('Phone','mPhone')}</div><p class="admin-form-note">Username uses lowercase letters, numbers, dots, dashes or underscores.</p>`,async()=>{const username=$('#mUsername').value.trim().toLowerCase(),password=$('#mPassword').value;if(!username||!/^[a-z0-9._-]{3,40}$/.test(username))throw new Error('Enter a valid username of at least 3 characters');if(password.length<6)throw new Error('Password must be at least 6 characters');if(state.students.some(s=>s.username.toLowerCase()===username))throw new Error('That username is already in the roster');const body={name:$('#mName').value.trim(),korean_name:$('#mKorean').value.trim(),username,password,class:$('#mClass').value,grade:$('#mGrade').value.trim()||null,school:$('#mSchool').value.trim()||null,phone:$('#mPhone').value.trim()||null,approved:true};if(!body.name&&!body.korean_name)throw new Error('Enter an English or Korean name');const result=await adminApi('create_student',body);state.students.unshift({id:result.user_id,name:body.name,korean_name:body.korean_name,username,email:`${username}@stu.willena`,class:body.class,grade:body.grade||'',school:body.school||'',phone:body.phone||'',role:'student',approved:true});syncRoster();toast('Student created')},{saveLabel:'Create student'})}
...
async function loadStudents(force=false){
  ...
  try{const roster=await loadStudentRoster({force,repairSession:true,onRefresh:fresh=>{state.students=fresh.students||[];buildFilters();applyFilters()}});state.students=roster.students||[];buildFilters();applyFilters()}
