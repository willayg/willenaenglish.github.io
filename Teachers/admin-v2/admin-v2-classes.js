...
function renderCards(){
  ...
  grid.innerHTML=rows.map(row=>{
    ...
    return `<div class="class-card admin-class-card" data-class-id="${esc(row.id||'')}" data-class-name="${esc(row.name||'')}"><div class="class-top"><div class="class-name">${esc(row.name||'')}</div><div class="class-count">${countFor(row.name)} students</div></div>${meta}<div class="class-stats"><div class="class-stat"><b>${countFor(row.name)}</b><span>students</span></div><div class="class-stat"><b>${books.length||'—'}</b><span>books</span></div></div><div class="class-link">Manage class →</div></div>`;
  }).join('');
  $$('.admin-class-card',grid).forEach(card=>card.onclick=()=>openEdit(card.dataset.className));
}
...
function renderDrawer(row,mode='edit'){
  ...
  [1,2,3].forEach(n=>bindBook(`classBook${n}`));
  syncLevel();
  $('#classSaveV2').onclick=saveDrawer;
  bg.classList.add('open');drawer.classList.add('open');document.body.style.overflow='hidden';
  setTimeout(()=>$(isAdd?'#classNameV2':'#classBook1')?.focus(),0);
}
...
function showMenu(id,books,message=''){
  ...
  $$('[data-i]',menu).forEach(btn=>btn.onclick=()=>{const b=books[Number(btn.dataset.i)];selectedBooks.set(id,b);input.value=displayBook(b);menu.classList.remove('show');bookStatus(id);syncLevel()});
  $('[data-manual]',menu)?.addEventListener('click',()=>{selectedBooks.delete(id);menu.classList.remove('show');bookStatus(id);syncLevel()});
}
async function searchBooks(id){
  const input=$('#'+id);if(!input)return;const q=input.value.trim();selectedBooks.delete(id);bookStatus(id);syncLevel();
  if(q.length<2){showMenu(id,[],'Type at least 2 characters');return}
  const key=q.toLowerCase(),cached=searchCache.get(key);if(cached&&Date.now()-cached.time<60000){showMenu(id,cached.books);return}
  controllers.get(id)?.abort();const c=new AbortController();controllers.set(id,c);showMenu(id,[],'Searching…');
  try{const d=await edge(CONTENT_API,{method:'POST',signal:c.signal,body:JSON.stringify({action:'search_books',q})});if(input.value.trim()!==q)return;const books=d.data||[];searchCache.set(key,{time:Date.now(),books});showMenu(id,books,books.length?'':'No matching books')}catch(e){if(e.name!=='AbortError')showMenu(id,[],'Search failed')}
}
...
async function saveDrawer(){
  const error=$('#classDrawerErrorV2');error.textContent='';const btn=$('#classSaveV2');btn.disabled=true;
  const books=[1,2,3].map(n=>bookPayload(`classBook${n}`)).filter(Boolean);const level=books.length?null:$('#classLevelV2').value;
  try{
    if(drawerMode==='add'){
      const name=$('#classNameV2').value.trim().replace(/\s+/g,' ');if(!name)throw new Error('Enter a class name');if(rows.some(r=>String(r.name).toLowerCase()===name.toLowerCase()))throw new Error('That class already exists');
      const d=await edge(CLASS_API,{method:'POST',body:JSON.stringify({action:'create_class',name,books,level})});rows.push(d.class);rows.sort((a,b)=>String(a.name).localeCompare(String(b.name),undefined,{numeric:true}));
    }else{
      const d=await edge(CLASS_API,{method:'POST',body:JSON.stringify({action:'update_class',class_id:activeClass.id,books,level})});const i=rows.findIndex(r=>r.id===d.class.id);if(i>=0)rows[i]=d.class;
    }
    renderCards();closeDrawer();
  }catch(e){error.textContent=e.message;btn.disabled=false}
}

export async function mountClasses(){
  if($('#classAddV2')?.dataset.bound!=='1'){$('#classAddV2').dataset.bound='1';$('#classAddV2').onclick=openAdd;$('#classRefreshV2').onclick=()=>loadClasses(true);$('#classDrawerCloseV2').onclick=closeDrawer;$('#classDrawerBgV2').onclick=e=>{if(e.target===$('#classDrawerBgV2'))closeDrawer()}}
