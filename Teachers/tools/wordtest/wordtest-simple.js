...
function generateWorksheetHTML(wordPairs, title = 'Word Test', layout = 'picture-matching', fontFamily = 'Arial, sans-serif', fontSize = '12px') {
    const baseStyles = `
        <style>
            @page { margin: 0.5in; }
            body { 
    ...
    if (layout === '4col') {
        ...
        `;
    } else if (layout === 'picture-matching') {
        // Picture Matching layout with circles
        const shuffledKorean = [...wordPairs].sort(() => Math.random() - 0.5);
        
        tableHTML = `
            <div class="matching-container">
                <div class="matching-column">
                    <h3 style="text-align: center; margin-bottom: 20px;">English</h3>
                    ${wordPairs.map((pair, i) => `
                        <div class="matching-item">
                            <div class="matching-circle"></div>
                            <div class="matching-text">${pair.eng || ''}</div>
                        </div>
                    `).join('')}
                </div>
                ...
                <div class="matching-column">
                    <h3 style="text-align: center; margin-bottom: 20px;">Korean</h3>
                    ${shuffledKorean.map((pair, i) => `
                        <div class="matching-item">
                            <div class="matching-circle"></div>
                            <div class="matching-text">${pair.kor || ''}</div>
                        </div>
                    `).join('')}
                </div>
            </div>
...
function generatePreviewHTML(wordPairs, title = 'Word Test', layout = 'picture-matching', fontFamily = 'Arial, sans-serif', fontSize = '12px') {
    let tableHTML = '';
    
    const baseStyle = `font-family: ${fontFamily}; font-size: ${fontSize};`;
    
    if (layout === 'picture-matching') {
        // Picture Matching layout with circles
        tableHTML = `
            <div style="padding: 20px; ${baseStyle}">
                <h2 style="text-align: center; margin-bottom: 20px; border-bottom: 2px solid #333; padding-bottom: 10px; ${baseStyle}">
                    ${title}
                    <div style="font-size: 12px; color: #666; margin-top: 5px;">Name: _________________ Date: _____________</div>
                </h2>
                <div style="display: flex; justify-content: space-between; margin: 20px 0;">
                    <div style="width: 45%;">
                        <h3 style="text-align: center; margin-bottom: 20px; ${baseStyle}">English</h3>
                        ${wordPairs.map((pair, i) => `
                            <div style="display: flex; align-items: center; margin: 15px 0; padding: 10px; border: 1px solid #ddd; border-radius: 5px;">
                                <div style="width: 25px; height: 25px; border: 2px solid #333; border-radius: 50%; margin-right: 15px; flex-shrink: 0;"></div>
                                <div style="${baseStyle} flex-grow: 1;">${pair.eng || ''}</div>
                            </div>
                        `).join('')}
                    </div>
                    
                    <div style="width: 10%; text-align: center; display: flex; flex-direction: column; justify-content: center; align-items: center;">
                        ${wordPairs.map((pair, i) => `
                            <div style="width: 15px; height: 15px; border: 2px solid #333; border-radius: 50%; margin: 10px 0;"></div>
                        `).join('')}
                    </div>
                    
                    <div style="width: 45%;">
                        <h3 style="text-align: center; margin-bottom: 20px; ${baseStyle}">Korean</h3>
                        ${[...wordPairs].sort(() => Math.random() - 0.5).map((pair, i) => `
                            <div style="display: flex; align-items: center; margin: 15px 0; padding: 10px; border: 1px solid #ddd; border-radius: 5px;">
                                <div style="width: 25px; height: 25px; border: 2px solid #333; border-radius: 50%; margin-right: 15px; flex-shrink: 0;"></div>
                                <div style="${baseStyle} flex-grow: 1;">${pair.kor || ''}</div>
                            </div>
                        `).join('')}
                    </div>
                </div>
...
// Ensure valid word pairs are entered
function validateWordPairsInput() {
...
function updatePreview() {
    ...
    const fontSizeSelect = document.getElementById('wordTestFontSize');

    if (!wordsTextarea || !previewArea) return;

    const words = wordsTextarea.value.trim();
    const title = titleInput ? titleInput.value.trim() || 'Word Test' : 'Word Test';
    const layout = layoutSelect ? layoutSelect.value : 'picture-matching';
    ...
    const fontSizeMultiplier = fontSizeSelect ? parseFloat(fontSizeSelect.value) : 1.0;
    const fontSize = Math.round(16 * fontSizeMultiplier) + 'px';
    ...
    const html = generatePreviewHTML(maskedPairs, title, layout, fontFamily, fontSize);
...
function printWorksheet() {
    ...
    const worksheetHTML = generateWorksheetHTML(maskedPairs, title, layout, fontFamily, fontSize);
