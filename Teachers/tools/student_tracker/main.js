          ...
          const modeChartHtml = modeCharts.map((mode, mIdx) => {
            ...
            return `
              <div class="game-chart mode">
                ...
                <div class="mode-caption">${metaBits.join(' 路 ')}</div>
              </div>
            `;
          }).join('');
            ...
            <div class="game-card" data-chart-id="${chartId}" data-completion="${completionPct}" data-best-stars="${bestStars}">
              <div class="game-header">
                <div>
                  ...
                  <div class="game-meta">${metaLine.join(' 路 ')}</div>
                </div>
              </div>
...
function renderAssignmentsList(assignments, showEnded, className, displayName) {
  ...
  assignmentList.querySelectorAll('.hw-assignment-card').forEach(card => {
    card.addEventListener('click', async (e) => {
      ...
      if (e.target.classList.contains('hw-end-btn') || e.target.classList.contains('hw-delete-btn')) return;
    ...
    hwAssignBtn.addEventListener('click', () => {
      ...
      if (!selected) {
        ...
        if (subtitle && subtitle.textContent) {
          const match = subtitle.textContent.match(/^(.+?)\s*[-?撯�?]?\s*Homework/i);
          if (match && match[1]) {
...
function showHomeworkStudentModal(row, totalModes, assignmentId) {
  ...
  if (meta) meta.textContent = `Completion: ${displayCompletion}% 路 Stars: ${row.stars || 0} 路 Overall Accuracy: ${overallAcc}% (Best: ${bestAcc}%) 路 Modes attempted: ${displayModesAttempted}/${displayTotalModes}${totalWasComputed ? ' (computed)' : ''}`;
  if (modesWrap) {
