...
<head>
  ...
  <script src="/js/api-config.js?v=20260329c"></script>
  <link rel="stylesheet" href="/Teachers/tools/wordtest/style.css?v=20251015-rc1" />
  <link rel="stylesheet" href="./report-card.css?v=20251015-rc4" />
  <style>
    :root { --header-h: 64px; }
  body { background:#f5f7fb; font-family: 'Poppins', ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, "Apple Color Emoji", "Segoe UI Emoji"; }
  ...
  .header h1 { color:#fff; font-size:18px; font-weight:700; }
    ...
    /* New toolbar layout */
    .top-actions-bar { display:grid; grid-template-columns: 1fr auto; gap:10px; padding:10px; align-items:center; border-bottom:1px solid #e5e7eb; background:linear-gradient(180deg,#e0f2fe,#eaf6ff); }
    .ta-left { display:flex; gap:10px; align-items:center; min-width:0; }
    .ta-right { display:flex; gap:10px; justify-content:flex-end; align-items:center; }
    .ta-row2 { grid-column:1 / -1; display:flex; gap:10px; align-items:center; flex-wrap:wrap; }
    .chip { border-radius:999px; padding:4px 10px; background:#eef2ff; color:#3730a3; border:1px solid #c7d2fe; font-weight:600; font-size:12px; white-space:nowrap; }
  .btn { padding:8px 12px; border-radius:12px; background:#fff; border:1px solid #e5e7eb; box-shadow:0 1px 0 rgba(0,0,0,0.03); cursor:pointer; }
  .btn:hover { background:#f8fafc; }
  .btn-primary { background:#a7f3d0; border-color:#34d399; color:#064e3b; font-weight:700; }
  .btn-primary:hover { background:#86efac; }
  /* Text-style buttons */
  .btn-link { background:transparent; border:none; box-shadow:none; color:#0f172a; font-weight:600; padding:6px 8px; }
  .btn-link:hover { background:#f1f5f9; }
    .btn-ghost { background:transparent; border:1px dashed #cbd5e1; }
    .select { appearance:none; padding:8px 12px; border-radius:12px; border:1px solid #e5e7eb; background:#fff; box-shadow:0 1px 0 rgba(0,0,0,0.03); }
    /* Overflow menu */
    details.more { position:relative; }
    details.more > summary { list-style:none; }
    details.more[open] > summary::marker { display:none; }
    details.more .more-menu { position:absolute; right:0; top:110%; background:#fff; border:1px solid #e5e7eb; border-radius:12px; box-shadow:0 8px 20px rgba(0,0,0,0.12); padding:8px; display:flex; flex-direction:column; gap:6px; min-width:180px; z-index:3; }
  /* Hand cursor on clickable elements */
  a[href], button, [role="button"], .btn, .btn-link, .layout-btn, .clear-btn, .extract-btn, details.more > summary,
  #gridMenu div[data-act], #colMenu div[data-act], #campaignSelectAll, #campaignClear { cursor: pointer; }
    @media (max-width: 900px) {
      .top-actions-bar { grid-template-columns: 1fr; }
      .ta-right { justify-content:stretch; }
    }
  ...
  #classComment { min-height:260px; font-size:13px; resize:vertical; }
    ...
    /* Keep burger menu inside header on this page (avoid floating over top action buttons) */
    #burger-menu-mount { position: relative; z-index: 2; }
    #burger-menu-mount .burger-menu { position: static !important; top: auto !important; right: auto !important; }
    #burger-menu-mount .burger-dropdown { top: calc(100% + 8px) !important; right: 0 !important; z-index: 1200; }
  </style>
</head>
<body>
  <div class="container">
    <div class="app-card">
      ...
      <div class="top-actions top-actions-bar">
        <div class="ta-left">
          <input id="search" class="input-field" placeholder="Search students" style="max-width:320px;" />
          <span id="testMeta" class="chip" title="Current test and student count">No test loaded</span>
          <!-- Month filter hidden per feedback; date will be automatic -->
          <input id="testMonthFilter" type="month" class="input-field" style="max-width:160px; display:none;" />
        </div>
        <div class="ta-right">
          <button id="openReportCardsBtn" class="btn-link" title="Open report cards">Report Cards</button>
          <details class="more">
            <summary id="rowActionsBtn" class="btn-link" title="Row actions">Rows</summary>
            <div class="more-menu">
              <button id="addRowBtn" class="btn-link" type="button">Add Row</button>
              <button id="deleteRowBtn" class="btn-link" type="button">Delete Row</button>
            </div>
          </details>
          <button id="openLoadBtn" class="btn-link">Load Test</button>
          <button id="saveBtn" class="btn-link" title="Save (Ctrl+S)">Save</button>
        </div>
        <div class="ta-row2">
          <input id="testName" class="input-field" placeholder="Test name (e.g., Brown Midterm)" style="flex:0 0 200px; width:200px; max-width:200px;" />
          <!-- Term period (start/end month) -->
          <input id="termFrom" type="month" class="input-field" title="Term start month" style="flex:0 0 140px; max-width:140px;" />
          <input id="termTo" type="month" class="input-field" title="Term end month" style="flex:0 0 140px; max-width:140px;" />
          <select id="testClassFilter" class="select"><option value="">All Classes</option></select>
          <button id="newTestBtn" class="btn-link">New Test</button>
          <button id="importTableBtn" class="btn-link">Import</button>
          <details class="more">
            <summary class="btn-link">More</summary>
            <div class="more-menu">
              <button id="exportCsvBtn" class="btn-link">Export CSV</button>
              <button id="refreshBtn" class="btn-link">Refresh</button>
            </div>
          </details>
        </div>
      </div>
  ...
  <script type="module" src="./test_input.js?v=20260306d"></script>
  <style>
    /* Selection styling */
  ...
  <div id="campaignModalBg" style="display:none; position:fixed; inset:0; background:rgba(15,23,42,0.35); z-index:3000; align-items:center; justify-content:center;">
    <div style="background:#fff; width:96vw; max-width:640px; border-radius:16px; box-shadow:0 10px 30px rgba(0,0,0,0.2); padding:16px 16px 12px;">
      ...
      <div style="display:flex; gap:10px; align-items:center; margin-bottom:8px;">
        <input id="campaignName" class="input-field" placeholder="Test name (e.g., October 2025 Test)" style="flex:1 1 auto; min-width:0;" />
        <input id="campaignDate" type="date" class="input-field" style="flex:0 0 150px; max-width:150px;" />
      </div>
  ...
  <!-- Load Test Modal -->
  <div id="loadModalBg" style="display:none; position:fixed; inset:0; background:rgba(15,23,42,0.35); z-index:3002; align-items:center; justify-content:center;">
    <div style="background:#fff; width:min(96vw, 900px); max-height:90vh; overflow:auto; border-radius:16px; box-shadow:0 10px 30px rgba(0,0,0,0.2); padding:14px; display:flex; flex-direction:column; gap:10px;">
      <div style="display:flex; align-items:center; gap:10px;">
        <strong style="flex:1;">Load Test</strong>
        <button id="loadCloseBtn" class="clear-btn">Close</button>
      </div>
      <input id="loadSearch" class="input-field" placeholder="Search by name, date, or class" />
      <div id="loadList" style="display:grid; grid-template-columns:1fr; gap:8px;">
        <!-- populated by script -->
      </div>
  ...
  <div style="background:#fff; width:min(96vw, 980px); border-radius:16px; box-shadow:0 10px 30px rgba(0,0,0,0.2); padding:16px 16px 12px; display:flex; flex-direction:column; gap:10px; position:relative;">
      ...
      <div id="importBusy" style="display:none; position:absolute; inset:0; background:rgba(255,255,255,0.75); border-radius:16px; align-items:center; justify-content:center;">
        <div style="display:flex; flex-direction:column; align-items:center; gap:8px;">
          <div style="width:36px; height:36px; border-radius:50%; border:3px solid #93c5fd; border-top-color:#2563eb; animation: spin 0.8s linear infinite;"></div>
    ...
    <div id="rcModal" style="background:#fff; width:min(96vw, 1100px); max-height:92vh; overflow:auto; border-radius:16px; box-shadow:0 10px 30px rgba(0,0,0,0.2);">
      <div style="position:sticky; top:0; background:#f8fafc; border-bottom:1px solid #e5e7eb; display:flex; align-items:center; justify-content:space-between; gap:8px; padding:10px 12px; z-index:1;">
        <div style="display:flex; align-items:center; gap:10px;">
          ...
          <select id="rcTotalMode" class="select" title="Total calculation mode" style="height:32px; padding:4px 10px;">
            <option value="weighted">Total % (points-based)</option>
            <option value="average">Average % (skills)</option>
          </select>
          <span id="rcStatus" class="note"></span>
        </div>
        <div style="display:flex; gap:8px;">
          <button id="rcPdfBtn" class="layout-btn">Export PDF</button>
