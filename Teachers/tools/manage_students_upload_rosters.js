...
const langMap = {
  ...
  ko: {
    ...
    addStudent: '?™ìƒ ì¶”ê?',
    addFullClass: '?„ì²´ ë°?ì¶”ê?',
    moveClassUp: 'ë°??´ë™',
    koreanEnglish: 'English | ?œêµ­??,
    username: '?„ì´??,
    password: 'ë¹„ë?ë²ˆí˜¸',
    name: '?´ë¦„ (? íƒ)',
    koreanName: '?œêµ­ ?´ë¦„',
    ...
    createBtn: '?™ìƒ ì¶”ê?',
    bulkClass: 'ë°??´ë¦„',
    bulkList: '??ë¶™ì—¬?£ê¸° (?œíŠ¸?ì„œ ë³µì‚¬)',
    bulkSubmit: '?™ìƒ ì¶”ê?',
    editStudent: '?™ìƒ ?•ë³´ ?˜ì •',
    editName: "?´ë¦„",
    editUsername: "?„ì´??,
    editKoreanName: "?œêµ­ ?´ë¦„",
    editClass: "ë°?,
    editGrade: "?™ë…„",
    editSubmit: "?˜ì •",
    moveClassModal: "ë°??´ë™",
    selectCurrentClass: "?„ìž¬ ë°?? íƒ",
    newClassName: "??ë°??´ë¦„",
    moveBtn: "?´ë™",
    cancel: "ì·¨ì†Œ",
    search: "?„ì´???ëŠ” ?´ë¦„?¼ë¡œ ê²€??,
    ...
    exportCsv: "CSV ?´ë³´?´ê¸°",
    classLabel: "ë°?",
  tableHeaders: ["?„ì´??, "?´ë¦„", "?œêµ­ ?´ë¦„", "ë°?, "?™ë…„", "?™êµ", "?„í™”ë²ˆí˜¸", "?¹ì¸??, "?„êµ¬"],
    addAll: '?„ì²´ ì¶”ê?',
    addAllTitle: '?„ì²´ ?™ìƒ ì¶”ê?',
    addAllPaste: 'ëª…ë‹¨ ?ìŠ¤??ë¶™ì—¬?£ê¸°',
    addAllPreview: 'ë¯¸ë¦¬ë³´ê¸°',
    addAllUpload: '?…ë¡œ??
    ,
    school: '?™êµ',
    phone: '?„í™”ë²ˆí˜¸',
...
function setLanguage(lang) {
  ...
  const testBtn = document.getElementById('openTestInput'); if (testBtn) testBtn.textContent = (lang==='ko' ? '?œí—˜ ?…ë ¥' : 'Test Input');
...
function parseBulkSheet(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (c0 === 'no' || c0 === '#' || c0 === 'ë²ˆí˜¸' || c1 === 'school' || c1 === '?™êµ') continue;
    if (c0 === 'school' || c0 === '?™êµ') continue;
