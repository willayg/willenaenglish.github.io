/**
 * Vercel Serverless Function: /api/analyze-sentence
 *
 * Body: JSON { transcript: string, language?: string }
 * Returns: JSON { corrected_sentence: string, teacher_note: string }
 *
 * Set env var in Vercel:
 * - OPENAI_API_KEY (preferred)
 * - or OPENAI_API (compat)
 */
...
If no changes needed, set teacher_note to "Perfect! Your sentence is correct!"`;
  ...
  try {
    ...
    if (!content) {
      return res.status(200).json({ corrected_sentence: transcript, teacher_note: 'Good job!' });
    }
