-- Fix Daily Study reward mirroring: progress_attempts.word is NOT NULL.
-- Use the answered Daily Study key as the legacy/global attempt identifier.
...
begin
  ...
  v_answer_key := coalesce(
    nullif(new.plan -> old.cursor ->> 'daily_key', ''),
    nullif(new.plan -> old.cursor ->> 'id', ''),
    'daily-study-question-' || new.cursor::text
  );
