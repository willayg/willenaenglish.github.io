...
Deno.serve(async(req:Request)=>{if(req.method==='OPTIONS')return new Response('',{status:200,headers:cors});try{
 ...
 if(action==='me'&&req.method==='GET'){
   const [fast,pointsRes]=await Promise.all([
     admin.rpc('test_prep_student_me_fast_payload_v1',{p_student_id:uid}),
     admin.rpc('sum_points_for_user',{uid})
   ]);
   ...
   const global_points=pointsRes.error?null:Number(pointsRes.data)||0;
   return out(200,{success:true,user:profile,plans,tasks,global_points,source:'fast-snapshot-v1'});
 }
