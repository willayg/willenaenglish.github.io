    ...
    if (event.path.endsWith('/upload_teacher_file') && event.httpMethod === 'POST') {
      ...
      try {
        ...
        if (body.action === 'insert_game_data' && body.data) {
          const gd = body.data;
          // Resolve authenticated user from cookie (consistent with update path)
          ...
          // Ownership check removed – any authenticated teacher may update any game.
          ...
          const { error: upErr } = await supabase
            ...
            .update(updateRow)
            .eq('id', body.id);
          if (upErr) return { statusCode: 400, body: JSON.stringify({ success:false, error: upErr.message }) };
          return { statusCode: 200, body: JSON.stringify({ success:true }) };
