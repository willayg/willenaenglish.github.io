...
function enhance(){
 installStyle();const input=activeInput();if(!input||input.dataset.hybridKb==='1')return;
 input.dataset.hybridKb='1';input.readOnly=true;input.setAttribute('readonly','');input.setAttribute('inputmode','none');input.setAttribute('aria-label','Spelling answer');
 ...
 const fine=matchMedia?.('(pointer:fine)')?.matches&&!matchMedia?.('(pointer:coarse)')?.matches;if(fine)hideKeyboard(input);
}
...
function boot(){installStyle();new MutationObserver(()=>queueMicrotask(enhance)).observe(document.getElementById('assignedQuizPane')||document.body,{childList:true,subtree:true});document.addEventListener('keydown',hardwareKey,true);enhance()}
