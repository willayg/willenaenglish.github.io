...
function boot(){addStyle();installKeyboard();const card=$('#card')||document.body;new MutationObserver(()=>queueMicrotask(installKeyboard)).observe(card,{childList:true,subtree:true});document.addEventListener('focusin',e=>rememberField(e.target),true);document.addEventListener('pointerdown',e=>rememberField(e.target),true);document.addEventListener('click',e=>{if(e.target.closest('#seosulCheck'))normalizeBeforeCheck()},true)}
