...
async function ensureContext(){
 ...
 const key=`${sel.plan.id}|${sel.lesson}|${sel.unitId}`;if(ctx.key===key&&ctx.items.length)return true;if(ctx.loading)return false;
 ctx={key,loading:true,items:[],byWord:new Map(),seen:new Set(),completed:false,unknown:new Set(),repeatPhase:false,repeatStarted:false,lastPrompt:'',shownAt:0};
 try{
  ...
  ctx.seen=new Set([...latest.keys()].filter(k=>ctx.byWord.has(k)));ctx.completed=items.length>0&&items.every(x=>latest.get(norm(x.canonical_text))===true);
...
async function answerSelfCheck(root,knew){const prompt=root.querySelector('.vp-prompt')?.textContent?.trim()||'',word=norm(prompt),item=ctx.byWord.get(word);const buttons=root.querySelectorAll('.vp-selfcheck-btn');buttons.forEach(b=>b.disabled=true);if(item){ctx.seen.add(word);if(!knew&&!ctx.repeatPhase)ctx.unknown.add(String(item.id));try{await saveCheck(item,knew,ctx.repeatPhase,Date.now()-ctx.shownAt)}catch(e){console.warn('[test-prep] self-check save failed',e);toast('자기 확인 기록 저장에 실패했습니다.')}if(knew&&ctx.repeatPhase)ctx.unknown.delete(String(item.id))}const next=root.querySelector('#vpNext');if(next){next.disabled=false;next.click()}}
