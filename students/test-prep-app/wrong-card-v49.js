...
function installStyles(){
 if(document.getElementById('tpWrong49mStyles'))return;
 const s=document.createElement('style');s.id='tpWrong49mStyles';s.textContent=`
 ...
 .tp49-wrong-copy{display:block;text-align:center}.tp49-wrong-copy b{display:block;font-size:24px;line-height:1.15;font-weight:800;color:#fff}.tp49-wrong-copy small{display:block;margin-top:7px;color:rgba(255,255,255,.84);font-size:12px;font-weight:600}
 ...
 @media(max-width:600px){#tpWrongCardMount{margin-bottom:38px!important}.tp49-wrong-card{padding:21px 19px 20px}.tp49-wrong-copy b{font-size:21px}.tp49-wrong-copy small{font-size:11px}.tp49-wrong-stats{padding-top:15px;margin-top:16px}.tp49-wrong-stat{padding:0 9px}.tp49-wrong-stat strong{font-size:25px}.tp49-wrong-stat small{font-size:10px}}
...
function loadingCard(){return`<button class="tp49-wrong-card tp49-wrong-loading" type="button" disabled><span class="tp49-wrong-copy"><b>오답</b><small>복습할 문제를 확인하고 있어요…</small></span>${statsHtml('—','—','—')}</button>`}
function card(rows){
 ...
 return`<button class="tp49-wrong-card" type="button" data-tp49-open ${total?'':'disabled'}><span class="tp49-wrong-copy"><b>오답</b><small>${message}</small></span>${statsHtml(ready,later,total)}</button>`;
}
function errorCard(msg){return`<button class="tp49-wrong-card tp49-wrong-error" type="button" data-tp49-retry><span class="tp49-wrong-copy"><b>오답</b><small>${String(msg||'오답을 불러오지 못했습니다.')} · 눌러서 다시 시도</small></span>${statsHtml('—','—','—')}</button>`}
...
console.log('[REV49m] centered wrong-answer title with no icon');
})();