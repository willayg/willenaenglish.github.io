...
function renderQuestion(){
  ...
  root.innerHTML=`<div class="real-mock-head"><div><button class="real-mock-back" id="exitMock">← 시험 대비</button><h1>실전모의고사</h1><p>${esc(manifest.bookLabel||'')} · 현재 시험 범위 · V2 renderer experiment</p></div><strong>${index+1} / ${manifest.questions.length}</strong></div><div class="real-mock-meta"><span class="real-mock-pill">${esc(LABEL[section]||section)}</span>${sourceBadge(q)}<span class="real-mock-pill">${esc(q.__lesson||'')}</span></div><div class="real-mock-progress"><i style="width:${Math.round(index/manifest.questions.length*100)}%"></i></div><div class="real-mock-card"><div id="questionHost"></div></div>`;
