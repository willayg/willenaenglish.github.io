...
function renderQuestion(){
  ...
  root.innerHTML=`<div class="real-mock-head"><div><button class="real-mock-back" id="exitMock">← 시험 대비</button><h1>실전모의고사</h1><p>${esc(manifest.bookLabel||'')} · 현재 시험 범위 · V2 renderer experiment</p></div><div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;justify-content:flex-end"><button class="real-mock-back" id="resetMock">RESET TEST</button><strong>${index+1} / ${manifest.questions.length}</strong></div></div><div class="real-mock-meta"><span class="real-mock-pill">${esc(LABEL[section]||section)}</span>${sourceBadge(q)}<span class="real-mock-pill">${esc(q.__lesson||'')}</span><span class="real-mock-pill">REV 52e</span></div><div class="real-mock-progress"><i style="width:${Math.round(index/manifest.questions.length*100)}%"></i></div><div class="real-mock-card"><div id="questionHost"></div></div>`;
  ...
  document.getElementById('exitMock').onclick=()=>{if(confirm('실전모의고사를 종료할까요?'))location.replace('./')};document.getElementById('resetMock').onclick=resetTest;
