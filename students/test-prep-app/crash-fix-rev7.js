...
function removeCrashBadges(){['tp-crash-fix-rev1','tp-crash-fix-rev2','tp-crash-fix-rev3','tp-crash-fix-rev4','tp-crash-fix-rev5','tp-crash-fix-rev6','tp-crash-fix-rev7','tp-crash-fix-rev8'].forEach(id=>document.getElementById(id)?.remove())}
...
function boot(){addStyles();removeCrashBadges();document.addEventListener('click',intercept,true);console.info('[Test Prep] Crash Fix Rev7 active: lightweight journey + real mastery + clearly visible simple width bars + tablet text sizing')}
