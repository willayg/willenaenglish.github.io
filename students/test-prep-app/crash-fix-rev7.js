...
function removeCrashBadges(){['tp-crash-fix-rev1','tp-crash-fix-rev2','tp-crash-fix-rev3','tp-crash-fix-rev4','tp-crash-fix-rev5','tp-crash-fix-rev6','tp-crash-fix-rev7','tp-crash-fix-rev8','tp-students-rev1','tp-students-rev2','tp-students-rev3','tp-students-rev4'].forEach(id=>document.getElementById(id)?.remove())}
function addStudentsBadge(){removeCrashBadges();const b=document.createElement('div');b.id='tp-students-rev4';b.textContent='Rev4';Object.assign(b.style,{position:'fixed',right:'8px',bottom:'8px',zIndex:'2147483647',padding:'4px 8px',borderRadius:'999px',background:'rgba(20,20,24,.82)',color:'#fff',font:'700 11px/1.2 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',boxShadow:'0 2px 8px rgba(0,0,0,.18)',pointerEvents:'none',opacity:'.9'});document.body.appendChild(b)}
...
function boot(){addStyles();addStudentsBadge();document.addEventListener('click',intercept,true);console.info('[Test Prep] Students Rev4 active')}
window.WillenaStudentsRev4={renderJourney};
window.WillenaStudentsRev2=window.WillenaStudentsRev4;
