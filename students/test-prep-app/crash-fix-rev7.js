...
function renderJourney(planId,lesson){if(window.WillenaAssignedTestPrep?.isPracticeOpen?.())return false;const home=document.getElementById('assignmentHome'),state=window.WillenaTestPrepAuth?.state,plan=state?.plans?.find(p=>String(p.id)===String(planId));if(!home||!plan)return false;const l=scopeFor(plan).find(x=>String(x.lesson)===String(lesson));if(!l)return false;const shown=STATIONS.filter(st=>available(plan,l,st));home.innerHTML=`<div class="tp-r7-wrap"><button type="button" class="tp-r7-back">← 시험 대비</button><div class="tp-r7-head"><h1>${esc(lesson)}</h1><p>${esc(plan.book_label||'')} · 학습 지도</p></div><div class="tp-r7-route">${shown.map((s,i)=>`<button type="button" class="tp-r7-stop" data-r7-skill="${esc(s.k)}"><span class="tp-r7-dot">${i+1}</span><span class="tp-r7-copy"><b>${esc(s.label)}</b><small>${esc(s.desc)}</small><span class="tp-r7-bar" aria-hidden="true"><i></i></span></span><span class="tp-r7-pct">—</span><span class="tp-r7-arrow">›</span></button>`).join('')}</div></div>`;requestAnimationFrame(()=>window.scrollTo({top:0,left:0,behavior:'auto'}));home.querySelector('.tp-r7-back').onclick=()=>{window.WillenaTestPrepNavigation?.toHome?.({replaceEntry:true})||window.WillenaTestPrepUX?.renderHome?.()};home.querySelectorAll('[data-r7-skill]').forEach(btn=>btn.onclick=()=>window.WillenaTestPrepUX?.launchSkill?.(plan.id,lesson,btn.dataset.r7Skill));return true}
function intercept(e){const card=e.target instanceof Element?e.target.closest('.tp-lesson-card'):null;if(!card||window.WillenaAssignedTestPrep?.isPracticeOpen?.())return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();renderJourney(card.dataset.lessonPlan,card.dataset.lesson)}
