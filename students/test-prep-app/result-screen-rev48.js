...
async function restart(opts={}){const sel=current?.selection;if(!sel?.plan?.id||!sel.lesson)return;hideResult();try{await window.WillenaAssignedTestPrep?.startSelection?.(sel.plan.id,sel.lesson,sel.section,opts)}catch(e){console.error('[RESULT48] restart failed',e)}}
...
function renderResult(){
 ...
 const retry=wrong?'<button class="tp48r-primary" data-tp48r-action="retry" disabled>오답 다시 풀기</button>':'';
 ...
 el.querySelector('[data-tp48r-action="retry"]')?.addEventListener('click',()=>restart({reviewMode:true,reviewIds:[...current.wrongIds]}));el.querySelector('[data-tp48r-action="again"]')?.addEventListener('click',()=>restart({}));el.querySelector('[data-tp48r-action="lesson"]')?.addEventListener('click',leaveResult);el.querySelector('[data-tp48r-retry-save]')?.addEventListener('click',retrySave)
}
...
console.log('[RESULT48] normal-study result owner active; standalone review ignored');
})();
