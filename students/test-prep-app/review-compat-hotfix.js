...
function cleanQuestion(row){
 ...
 if(choiceMode(row)){
  ...
  delete row.metadata.constructed_response;
  delete row.metadata.authored_constructed_response;
  delete row.metadata.constructed_response_authored;
 }
...
function cleanReviewItem(item){
 ...
 const a={...(item.attempt_metadata||{})},m={...(item.metadata||{})};
