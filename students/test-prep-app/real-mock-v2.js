...
function diversePick(rows,count){const unique=[],seen=new Set();for(const q of shuffle(rows||[])){const id=idOf(q);if(!id||seen.has(id))continue;seen.add(id);unique.push(q)}const out=[],used=new Set(),byLesson=new Map();for(const q of unique){const k=String(q.__lesson||'');if(!byLesson.has(k))byLesson.set(k,[]);byLesson.get(k).push(q)}for(const lesson of shuffle([...byLesson.keys()])){if(out.length>=count)break;const pool=byLesson.get(lesson)||[];if(!pool.length)continue;const q=pool.shift();out.push(q);used.add(idOf(q))}const buckets=new Map();for(const q of unique){if(used.has(idOf(q)))continue;const k=signature(q);if(!buckets.has(k))buckets.set(k,[]);buckets.get(k).push(q)}for(const [k,v] of buckets)buckets.set(k,shuffle(v));let moved=true;const keys=shuffle([...buckets.keys()]);while(out.length<count&&moved){moved=false;for(const k of keys){const b=buckets.get(k);if(b?.length&&out.length<count){const q=b.shift();if(used.has(idOf(q)))continue;used.add(idOf(q));out.push(q);moved=true}}}return out}
