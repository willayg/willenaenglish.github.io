...
async function saveAnswer(item,q,spec,ok,selected,testMode=false){
 ...
 try{
  ...
  const r=await auth.recordAttempt({practice_type:constructed?'constructed_response':String(item.practice_type||q.section||'reading').toLowerCase(),question_id:String(item.question_id),selected_answer:selected,correct_answer:spec.answers,is_correct:!!ok,question_type:item.question_type||q.question_type||null,targets:Array.isArray(item.targets)&&item.targets.length?item.targets:(Array.isArray(q.targets)?q.targets:[]),source_label:q.student_source_label||base.source_label||'오답 복습',metadata:{...base,...qm,review_mode:true,source:'wrong-review',review_state_id:String(item.question_id),source_question_id:q.id||base.source_question_id||null,review_runner:'v49e',staging_auto_correct:!!testMode,canonical_text:canonicalText(item,q)||base.canonical_text||null}});
  if(!r)throw new Error('오답 기록 저장에 실패했습니다.');await auth.flushAttemptBatch?.('review-v49e');const bs=auth.getAttemptBatchState?.();if(bs&&Number(bs.queued)>0)throw new Error('오답 기록이 아직 저장되지 않았습니다.');done++;if(ok)correct++;else wrong.push(String(item.question_id));return true;
...
console.log('[REV49e] review queue dedupes context and does not own revision badge');
})();