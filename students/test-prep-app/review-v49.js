...
function parseCorrection(v){const m=String(v||'').trim().match(/^(.+?)\s*→\s*(.+)$/);return m?{raw:`${m[1].trim()} → ${m[2].trim()}`,wrong:m[1].trim(),right:m[2].trim()}:null}
function isCorrection(item,q){const t=qType(item,q),m={...meta(item),...(q?.metadata||{})};return t==='grammar_correction_text'||t.includes('correction_text')||(m.constructed_response===true&&!!parseCorrection(canonicalText(item,q)))}
...
function installStyles(){
 ...
 .tp49-input,.tp49-correction-input{width:100%;box-sizing:border-box;border:2px solid #8edcdf;border-radius:15px;padding:14px 15px;font:700 17px Poppins,sans-serif;outline:none}.tp49-input:focus,.tp49-correction-input:focus{border-color:#19777e}.tp49-actions{margin-top:18px}.tp49-feedback{display:none;border-radius:13px;padding:12px 14px;margin-top:14px;font-size:13px;font-weight:800}.tp49-feedback.ok{display:block;background:#eefaf2;color:#176b35}.tp49-feedback.bad{display:block;background:#fff1f1;color:#953039}.tp49-source{font-size:10px;color:#8a999e;font-weight:700;margin-top:12px}
 .tp49-correction{display:grid;grid-template-columns:1fr 46px 1fr;gap:10px;align-items:end;margin-top:18px}.tp49-correction-cell{display:grid;gap:7px}.tp49-correction-label{font-size:13px;font-weight:800;color:#19777e}.tp49-correction-arrow{font-size:28px;font-weight:800;text-align:center;color:#7b8790;padding-bottom:10px}
 ...
 @media(max-width:600px){.tp49-page{width:calc(100% - 18px);margin-top:10px}.tp49-body{padding:20px 16px}.tp49-prompt{font-size:18px}.tp49-stats{gap:7px}.tp49-stat{padding:12px 6px}.tp49-stat b{font-size:21px}.tp49-correction{grid-template-columns:1fr 30px 1fr;gap:7px}.tp49-correction-arrow{font-size:24px}}
...
function resolveSpec(item,q){
 ...
 if(isCorrection(item,q)){
  const p=parseCorrection(answer||arr(q?.correct_answer)[0]||arr(item?.correct_answer_snapshot)[0]);
  if(p)return{mode:'correction',answers:[p.raw],display:[p.raw],wrong:p.wrong,right:p.right};
 }
 ...
 if(choiceMode){let correctIndices=numericSource;if(!correctIndices.length){const texts=(snap.length?snap:qAns).map(norm);correctIndices=choices.map((c,i)=>texts.includes(norm(choiceText(c)))?String(i+1):null).filter(Boolean)}return{mode:correctIndices.length>1||mode.includes('multi')?'multi':'choice',answers:correctIndices,display:correctIndices.map(n=>choiceText(choices[Number(n)-1])).filter(Boolean)}}
 let answers=[];if(answer)answers=[answer];if(!answers.length)answers=qAns.filter(x=>!numeric(x));if(!answers.length)answers=snap.filter(x=>!numeric(x));return{mode:'text',answers,display:answers};
}
...
function renderContext(c={}){const order=[['korean','우리말'],['definition','정의'],['sentence','문장'],['passage','지문'],['dialogue','대화'],['source_passage','원문'],['rewritten','바꿔 쓴 글'],['given_sentence','보기'],['bank','보기'],['items',''],['statements',''],['pairs',''],['clues','문제 단서'],['segments','A–D'],['claims','설명'],['table','표'],['source_sentence',''],['original','원문']];const used=new Set();let out='';for(const[k,l]of order){if(c?.[k]!=null&&c[k]!==''){out+=ctx(l,c[k]);used.add(k)}}for(const[k,v]of Object.entries(c||{})){if(used.has(k)||v==null||v===''||['underlined','underlined_spans'].includes(k))continue;if(typeof v==='string'||Array.isArray(v))out+=ctx(k,v)}return out}
function skillLabel(item,q){const m={...meta(item),...(q?.metadata||{})};if(m.constructed_response===true||m.authored_constructed_response===true||m.constructed_response_authored===true)return'서술형';return({vocabulary:'Vocabulary',vocab_test:'Vocab Test',communication:'Communication',grammar:'Grammar',sentences:'본문외우기',reading:'Reading',constructed_response:'서술형'}[String(item?.practice_type||'').toLowerCase()]||String(item?.practice_type||'Review'))}
...
function setButtons(on){document.querySelectorAll('.tp49-choice,.tp49-input,.tp49-correction-input,.tp49-next,.tp49-test,.tp49-word').forEach(el=>{if('disabled'in el)el.disabled=!on})}
...
async function saveAnswer(item,q,spec,ok,selected,testMode=false){
 ...
 try{
  await ensurePlan(item);const auth=window.WillenaTestPrepAuth;if(!auth?.recordAttempt)throw new Error('기록 저장기를 찾지 못했습니다.');const base=meta(item),qm=q?.metadata||{},constructed=qm.constructed_response===true||qm.authored_constructed_response===true||qm.constructed_response_authored===true;
  const r=await auth.recordAttempt({practice_type:constructed?'constructed_response':String(item.practice_type||q.section||'reading').toLowerCase(),question_id:String(item.question_id),selected_answer:selected,correct_answer:spec.answers,is_correct:!!ok,question_type:item.question_type||q.question_type||null,targets:Array.isArray(item.targets)&&item.targets.length?item.targets:(Array.isArray(q.targets)?q.targets:[]),source_label:q.student_source_label||base.source_label||'오답 복습',metadata:{...base,...qm,review_mode:true,source:'wrong-review',review_state_id:String(item.question_id),source_question_id:q.id||base.source_question_id||null,review_runner:'v49b',staging_auto_correct:!!testMode,canonical_text:canonicalText(item,q)||base.canonical_text||null}});
  if(!r)throw new Error('오답 기록 저장에 실패했습니다.');await auth.flushAttemptBatch?.('review-v49b');const bs=auth.getAttemptBatchState?.();if(bs&&Number(bs.queued)>0)throw new Error('오답 기록이 아직 저장되지 않았습니다.');done++;if(ok)correct++;else wrong.push(String(item.question_id));return true;
...
async function renderCurrent(){
 ...
 if(spec.mode==='correction')modeHTML=`<div class="tp49-correction"><div class="tp49-correction-cell"><div class="tp49-correction-label">틀린 부분</div><input class="tp49-correction-input" id="tp49WrongPart" autocomplete="off" spellcheck="false" placeholder="찾아 쓰기"></div><div class="tp49-correction-arrow">→</div><div class="tp49-correction-cell"><div class="tp49-correction-label">고친 부분</div><input class="tp49-correction-input" id="tp49RightPart" autocomplete="off" spellcheck="false" placeholder="바르게 고치기"></div></div>`;
 else if(spec.mode==='text')modeHTML='<input class="tp49-input" id="tp49Input" autocomplete="off" spellcheck="false" placeholder="답을 입력하세요">';
 ...
 let selected=new Set(),answered=false,built=[];const check=$('#tp49Check',h),input=$('#tp49Input',h),wrongPart=$('#tp49WrongPart',h),rightPart=$('#tp49RightPart',h),next=()=>{index++;renderCurrent()};
 function reveal(ok){if(spec.mode==='choice'||spec.mode==='multi'){$$('.tp49-choice',h).forEach(b=>{const n=String(b.dataset.i);if(spec.answers.includes(n))b.classList.add('correct');else if(selected.has(n))b.classList.add('wrong');b.disabled=true})}if(input)input.disabled=true;if(wrongPart)wrongPart.disabled=true;if(rightPart)rightPart.disabled=true;$$('.tp49-word',h).forEach(b=>b.disabled=true);feedback(ok,ok?'정답입니다!':'모범 답안: '+spec.display.join(', '));check.disabled=false;check.textContent=index===run.length-1?'복습 끝내기':'다음 문제';answered=true}
 ...
 async function submit(testMode=false){
  ...
  else if(spec.mode==='correction'){const a=String(wrongPart?.value||'').trim(),b=String(rightPart?.value||'').trim();if(!a||!b)return;sel=`${a} → ${b}`;ok=norm(a)===norm(spec.wrong)&&norm(b)===norm(spec.right)}
 ...
 if(spec.mode==='correction'){const sync=()=>{if(!answered)check.disabled=!(wrongPart?.value.trim()&&rightPart?.value.trim())};wrongPart?.addEventListener('input',sync);rightPart?.addEventListener('input',sync);wrongPart?.focus()}
 else if(spec.mode==='text'){input?.focus();input?.addEventListener('input',()=>{if(!answered)check.disabled=!input.value.trim()});input?.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();submit(false)}})}
...
function install(){installStyles();const ux=window.WillenaTestPrepUX;if(!ux){setTimeout(install,50);return}ux.showWrongCenter=show;window.WillenaReviewV49={show,load,start};const badge=$('#tp-rev-badge');if(badge)badge.textContent='REV 49b'}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();
console.log('[REV49b] review runner preserves structured grammar_correction_text 서술형 rendering');
})();