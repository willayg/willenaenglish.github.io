...
async function saveAnswer(item,q,spec,ok,selected,testMode=false){
 ...
 try{
  ...
  const r=await auth.recordAttempt({practice_type:constructed?'constructed_response':String(item.practice_type||q.section||'reading').toLowerCase(),question_id:String(item.question_id),selected_answer:selected,correct_answer:spec.answers,is_correct:!!ok,question_type:item.question_type||q.question_type||null,targets:Array.isArray(item.targets)&&item.targets.length?item.targets:(Array.isArray(q.targets)?q.targets:[]),source_label:q.student_source_label||base.source_label||'오답 복습',metadata:{...base,...qm,review_mode:true,source:'wrong-review',review_state_id:String(item.question_id),source_question_id:q.id||base.source_question_id||null,review_runner:'v49c',staging_auto_correct:!!testMode,canonical_text:canonicalText(item,q)||base.canonical_text||null}});
  if(!r)throw new Error('오답 기록 저장에 실패했습니다.');await auth.flushAttemptBatch?.('review-v49c');const bs=auth.getAttemptBatchState?.();if(bs&&Number(bs.queued)>0)throw new Error('오답 기록이 아직 저장되지 않았습니다.');done++;if(ok)correct++;else wrong.push(String(item.question_id));return true;
...
async function renderCurrent(){
 ...
 if(spec.mode==='constructed'){
  ...
  constructedControl=api.mount(host,spec.answers,{onChange:value=>{constructedValue=value;if(!answered)check.disabled=!String(value||'').trim()}});
 }
...
function install(){installStyles();const ux=window.WillenaTestPrepUX;if(!ux){setTimeout(install,50);return}ux.showWrongCenter=show;window.WillenaReviewV49={show,load,start};const badge=$('#tp-rev-badge');if(badge)badge.textContent='REV 49c'}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();
console.log('[REV49c] review queue delegates constructed-response input shape to shared component');
})();