...
async function runCurrent(runId){
 ...
 }catch(e){if(String(e?.message||e)==='QUESTION_RUNTIME_CANCELLED')return;console.error('[REV46l] exam question failed',e);showRuntimeError(item,e)}
}
...
async function finishPhase(){const state=active;if(!state)return;stopDecorator();runtime().cancel();state.finishedAt=Date.now();try{await closeTrackingGroup(state)}catch(e){console.warn('[REV46l] final tracking block close failed',e)}if(!active||active!==state)return;if(!state.correction){state.firstPassResults=state.results.slice();renderFirstPassResult();return}const remaining=correctionItems(state.results);if(remaining.length){renderCorrectionRetry(remaining.length);return}renderCorrectionComplete()}
...
function cleanupState(old){runtime().cancel();stopDecorator();document.querySelector('.app')?.classList.remove('tp-exam46-active');window.WillenaVocabTestPractice?.restore?.();window.WillenaExamControls?.stop?.();Promise.resolve(closeTrackingGroup(old)).catch(e=>console.warn('[REV46l] exit tracking close failed',e))}
...
console.log('[REV46l] ExamSession manifest tracking lock + replacements ready');
})();