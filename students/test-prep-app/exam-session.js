...
function practiceForItem(item){return String(item?.trackingPractice||practiceFor(item?.question)||'reading').toLowerCase()}
function trackingKey(item){return`${String(item?.lesson||'')}|${practiceForItem(item)}`}
function loadingStyles(){if($('#examLoadingStyles'))return;const s=document.createElement('style');s.id='examLoadingStyles';s.textContent=`.exam-loading{min-height:230px;display:grid;place-items:center;text-align:center;padding:28px}.exam-loading-inner{display:grid;justify-items:center;gap:12px;color:#52666e;font:700 13px/1.45 Poppins,'Noto Sans KR',sans-serif}.exam-spinner{width:36px;height:36px;border:4px solid #e3ecee;border-top-color:#19777e;border-radius:50%;animation:examSpin .7s linear infinite}.exam-dots{display:flex;gap:5px}.exam-dots i{display:block;width:6px;height:6px;border-radius:50%;background:#78949c;animation:examPulse .9s ease-in-out infinite}.exam-dots i:nth-child(2){animation-delay:.12s}.exam-dots i:nth-child(3){animation-delay:.24s}@keyframes examSpin{to{transform:rotate(360deg)}}@keyframes examPulse{0%,100%{opacity:.3;transform:translateY(0)}50%{opacity:1;transform:translateY(-3px)}}@media(prefers-reduced-motion:reduce){.exam-spinner,.exam-dots i{animation:none}}`;document.head.appendChild(s)}
function loadingMarkup(text){loadingStyles();return`<div class="exam-loading"><div class="exam-loading-inner"><div class="exam-spinner" aria-hidden="true"></div><div>${esc(text)}</div><div class="exam-dots" aria-hidden="true"><i></i><i></i><i></i></div></div></div>`}
...
async function prepareTracking(state,item){const key=trackingKey(item),practice=practiceForItem(item),lesson=String(item.lesson||state.lesson||'');if(state.trackingKey===key&&auth()?.state?.session)return;if(state.trackingKey)await closeTrackingGroup(state);const a=auth();a?.setActivePlan?.(state.plan,lesson||null);await a?.ensureSession?.(practice);a?.beginStudyActivity?.();state.trackingKey=key;state.trackingPractice=practice;state.trackingLesson=lesson;state.trackingGroupResults=[]}
...
async function runCurrent(runId){
 ...
 }catch(e){if(String(e?.message||e)==='QUESTION_RUNTIME_CANCELLED')return;console.error('[exam] question failed',e);showRuntimeError(item,e)}
}
function showRuntimeError(item,e){const c=card();if(!c)return;c.innerHTML=`<div class="empty"><b>이 문제를 실행하지 못했습니다.</b><br><small>${esc(item?.question?.question_type||'unknown')} · ${esc(e?.message||e)}</small><div class="actions" style="margin-top:16px"><button class="secondary" id="examRetry">다시 시도</button><button class="secondary" id="examExit">모의고사 종료</button></div></div>`;$('#examRetry')?.addEventListener('click',()=>runCurrent(active?.serial));$('#examExit')?.addEventListener('click',()=>exit(true));decorate()}
...
function itemForResult(r){return active?.manifest?.items?.find(i=>String(i.question?.id)===String(r.question?.id||r.questionId)&&String(i.lesson)===String(r.lesson))||{position:r.manifestPosition||0,lesson:r.lesson||'',unitId:r.question?.__unitId||'',engine:runtime().engineFor(r.question),trackingPractice:practiceFor(r.question),question:r.question}}
...
async function finishPhase(){const state=active;if(!state)return;stopDecorator();runtime().cancel();state.finishedAt=Date.now();try{await closeTrackingGroup(state)}catch(e){console.warn('[exam] final tracking block close failed',e)}if(!active||active!==state)return;if(!state.correction){state.firstPassResults=state.results.slice();renderFirstPassResult();return}const remaining=correctionItems(state.results);if(remaining.length){renderCorrectionRetry(remaining.length);return}renderCorrectionComplete()}
function renderFirstPassResult(){if(!active)return;const stats=originalStats(),wrongItems=correctionItems(active.firstPassResults),c=card(),b=bar();if(b)b.style.width='100%';if(c)c.innerHTML=`<div class="result exam-result"><div class="score">${stats.correct}/${stats.total}</div><h2>모의고사 완료</h2><p>정답률 ${stats.pct}% · 오답 ${stats.wrong}개</p><div class="actions">${wrongItems.length?'<button class="primary" id="examCorrections">오답 복습 시작</button>':'<button class="primary" id="examDone">완료</button>'}</div></div>`;$('#examCorrections')?.addEventListener('click',()=>startCorrections(wrongItems));$('#examDone')?.addEventListener('click',()=>exit(false));window.dispatchEvent(new CustomEvent('exam46:complete',{detail:{manifest:active.manifest,results:active.firstPassResults,correct:stats.correct,total:stats.total,wrong:stats.wrong,replacementsUsed:active.replacementsUsed}}));window.WillenaExamControls?.decorate?.()}
function renderCorrectionRetry(count){if(!active)return;const stats=originalStats(),c=card(),b=bar();if(b)b.style.width='100%';if(c)c.innerHTML=`<div class="result exam-result"><div class="score">${count}</div><h2>아직 ${count}문제가 남았어요.</h2><p>틀린 문제를 다시 풀어 모두 맞혀 보세요.</p><div class="actions"><button class="primary" id="examRetryWrong">틀린 문제 다시 풀기</button></div></div>`;$('#examRetryWrong')?.addEventListener('click',()=>startCorrections(correctionItems(active.results)));window.dispatchEvent(new CustomEvent('exam46:correction-round',{detail:{remaining:count,original:stats}}));window.WillenaExamControls?.decorate?.()}
function renderCorrectionComplete(){if(!active)return;const stats=originalStats(),c=card(),b=bar();if(b)b.style.width='100%';if(c)c.innerHTML=`<div class="result exam-result"><div class="score">✓</div><h2>오답 복습 완료</h2><p>처음 점수 ${stats.correct}/${stats.total} · 모든 오답을 다시 맞혔어요.</p><div class="actions"><button class="primary" id="examDone">완료</button></div></div>`;$('#examDone')?.addEventListener('click',()=>exit(false));window.dispatchEvent(new CustomEvent('exam46:correction-complete',{detail:{manifest:active.manifest,firstPassResults:active.firstPassResults,original:stats}}));window.WillenaExamControls?.decorate?.()}
...
function cleanupState(old){runtime().cancel();stopDecorator();document.querySelector('.app')?.classList.remove('tp-exam46-active');window.WillenaVocabTestPractice?.restore?.();window.WillenaExamControls?.stop?.();Promise.resolve(closeTrackingGroup(old)).catch(e=>console.warn('[exam] exit tracking close failed',e))}
