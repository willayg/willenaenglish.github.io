...
async function begin(plan,manifest,scope,lesson){if(!manifest?.items?.length)throw new Error('모의고사에 사용할 문제가 없습니다.');serial++;active={serial,plan,manifest,scope,lesson,queue:manifest.items,index:0,results:[],firstPassResults:null,correction:false,startedAt:Date.now(),phaseStartedAt:Date.now(),finishedAt:null,trackingKey:null,trackingPractice:null,trackingLesson:null,trackingGroupResults:[]};ensureView();startDecorator();window.dispatchEvent(new CustomEvent('exam46:start',{detail:{manifest,scope,lesson}}));await runCurrent(active.serial);return active}

async function closeTrackingGroup(state){
 if(!state)return;
 const rows=Array.isArray(state.trackingGroupResults)?state.trackingGroupResults.slice():[];
 if(!state.trackingKey||!rows.length){state.trackingKey=null;state.trackingPractice=null;state.trackingLesson=null;state.trackingGroupResults=[];return}
 const correct=rows.filter(r=>r.correct).length,wrongIds=rows.filter(r=>!r.correct).map(r=>String(r.questionId||r.question?.id||'')).filter(Boolean);
 await auth()?.completeSession?.(correct,rows.length,wrongIds);
 state.trackingKey=null;state.trackingPractice=null;state.trackingLesson=null;state.trackingGroupResults=[];
}
async function prepareTracking(state,item){
 const key=trackingKey(item),practice=practiceFor(item.question),lesson=String(item.lesson||state.lesson||'');
 if(state.trackingKey===key&&auth()?.state?.session)return;
 if(state.trackingKey)await closeTrackingGroup(state);
 const a=auth();a?.setActivePlan?.(state.plan,lesson||null);await a?.ensureSession?.(practice);a?.beginStudyActivity?.();
 state.trackingKey=key;state.trackingPractice=practice;state.trackingLesson=lesson;state.trackingGroupResults=[];
}
...
async function runCurrent(runId){
 ...
 const state=active,item=state.queue[state.index],c=card();if(c)c.innerHTML='<div class="loading">문제를 불러오는 중...</div>';decorate();
 try{
  ...
  if(!active||active!==state||state.serial!==runId)return;
 ...
 }catch(e){console.error('[REV46e] exam question failed',e);showRuntimeError(item,e)}
}
...
function cleanupState(old){runtime().cancel();stopDecorator();document.querySelector('.app')?.classList.remove('tp-exam46-active');window.WillenaVocabTestPractice?.restore?.();window.WillenaExamControls?.stop?.();Promise.resolve(closeTrackingGroup(old)).catch(e=>console.warn('[REV46e] exit tracking close failed',e))}
...
console.log('[REV46e] ExamSession canonical tracking blocks ready');
})();