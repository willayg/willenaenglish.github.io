(function(){
  ...
  function installDebugPanel(){const qs=new URLSearchParams(location.search);if(qs.get('debug')!=='tracking'||document.getElementById('trackingDebug'))return;const panel=document.createElement('div');panel.id='trackingDebug';panel.innerHTML='<strong>Phase 1 tracking test</strong><pre>Answer a question to see the event here.</pre>';document.body.appendChild(panel);window.addEventListener('testprep:tracking',e=>{const pre=panel.querySelector('pre');if(pre)pre.textContent=JSON.stringify(e.detail,null,2)});window.addEventListener('testprep:vocab-attempt',e=>{const pre=panel.querySelector('pre');if(pre)pre.textContent=JSON.stringify({type:'vocab-attempt',...e.detail},null,2)});}
  function renderHome(){
  ...
  async function waitForVocab(){for(let i=0;i<60;i++){if(window.WillenaVocabPractice)return window.WillenaVocabPractice;await new Promise(r=>setTimeout(r,50));}throw new Error('Vocabulary practice module did not load.');}
  ...
  async function start(btn){
    ...
    try{
      ...
      if(selection.section==='vocabulary'){
        back.querySelector('button').onclick=()=>{window.WillenaVocabPractice?.restore?.();renderHome()};
        const mod=await waitForVocab();await mod.start({quiz,unitId:selection.unitId,lesson:selection.lesson,bookLabel:plan.book_label});return;
      }
      window.WillenaVocabPractice?.restore?.();window.WillenaTestPrepAuth.setActivePlan(plan,selection.lesson);window.WillenaTestPrepAuth.beginStudyActivity?.();
