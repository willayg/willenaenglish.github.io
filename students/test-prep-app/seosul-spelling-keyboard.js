...
function isTablet(){return window.matchMedia?.('(min-width:700px) and (pointer:coarse)')?.matches||false}
...
#seosulKbTools{display:flex;justify-content:flex-end;margin-top:8px}
#seosulKbShow{border:1px solid #a9e4e7;background:#fff;color:#19777e;border-radius:999px;padding:8px 12px;font-weight:800;font-size:13px}
#tpSeosulAppKeyboard{box-sizing:border-box;background:#eef3f5;border-top:1px solid #d7e0e3;padding:8px 10px 10px;z-index:11900}
#tpSeosulAppKeyboard .vp-kb-top{display:flex;gap:8px;align-items:center;margin-bottom:6px}
#tpSeosulAppKeyboard .vp-kb-answer{min-height:36px;flex:1;background:#fff;border:1px solid #cad7db;border-radius:10px;padding:7px 10px;font-size:16px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis}
#tpSeosulAppKeyboard .vp-kb-hide{border:0;background:transparent;color:#4e656d;font-weight:800;padding:8px 6px;font-size:12px}
...
#tpSeosulAppKeyboard .vp-kb-hint{display:none}
#tpSeosulAppKeyboard[hidden]{display:none!important}
@media (min-width:700px) and (pointer:coarse){
 #tpSeosulAppKeyboard{position:fixed;left:50%;bottom:14px;transform:translateX(-50%);width:min(760px,calc(100vw - 28px));border:1px solid #bdeff1;border-radius:18px;box-shadow:0 14px 40px rgba(31,59,66,.18);padding:10px 12px 12px}
 ...
 #tpSeosulAppKeyboard .vp-kb-answer{min-height:34px;font-size:15px}
}
...
function ensure(){addStyle();const inputs=candidates();if(!inputs.length){cleanup();return}inputs.forEach(lock);if(!active||!inputs.includes(active))active=inputs[0];if(kb()){sync();return}
 const tools=document.createElement('div');tools.id='seosulKbTools';tools.className='vp-spell-tools';tools.innerHTML='<button type="button" class="vp-spell-showkb" id="seosulKbShow">⌨ 키보드 열기</button>';
