...
function ensure(){addStyle();const inputs=candidates();if(!inputs.length){cleanup();return}inputs.forEach(lock);if(!active||!inputs.includes(active))active=inputs[0];if(kb())return;
 ...
 k.addEventListener('click',e=>{const b=e.target.closest('[data-key]');if(!b)return;if(b.dataset.downHandled==='1'){delete b.dataset.downHandled;e.preventDefault();e.stopImmediatePropagation();return}const key=b.dataset.key;if(key==='enter')hide()});
