...
function practiceKey(q){if(q?.__examVocabularyGenerated)return'vocab_test';const s=norm(q?.section);if(s==='vocab_test'||s==='vocabulary')return'vocab_test';const u=norm(q?.__sourceSection||q?.section);return u==='vocabulary'?'vocab_test':u}
...
async function vocabPool(bookId,unitId,lesson){
 const [generated,stored]=await Promise.all([generatedVocabPool(unitId,lesson),fetchSection(bookId,unitId,'vocabulary',lesson)]),
 generatedForExam=generated.map(q=>({...q,__sourceSection:q.__sourceSection||q.section||'vocabulary',section:'vocab_test'})),
 storedForExam=stored.map(q=>({...q,__sourceSection:'vocabulary',section:'vocab_test'})),seenIds=new Set(),out=[];
 for(const q of [...generatedForExam,...storedForExam]){const k=String(q.id||'');if(!k||seenIds.has(k))continue;seenIds.add(k);out.push(q)}
 return out
}
...
function finishManifest({plan,scope,lesson,items,unsupported,hp}){const rt=runtime(),recentUsed=items.filter(q=>hp.recent.has(String(q.id))).length,unseenUsed=items.filter(q=>!hp.byId.has(String(q.id))).length,vocab=items.filter(q=>practiceKey(q)==='vocab_test'),vocabWriting=vocab.filter(isVocabText).length;const manifest={id:id(),version:'46l',createdAt:new Date().toISOString(),planId:String(plan.id),bookLabel:plan.book_label||'',scope,lesson:lesson||null,total:items.length,replacedQuestionIds:[],items:items.map((q,i)=>({position:i+1,lesson:String(q.__lesson||lesson||''),unitId:String(q.__unitId||''),engine:rt.engineFor(q),trackingPractice:practiceKey(q),question:q})),unsupported,selectionDiagnostics:{historyQuestions:hp.total,recentWindow:RECENT_WINDOW,recentReused:recentUsed,unseenSelected:unseenUsed,vocabQuestions:vocab.length,vocabWriting,vocabChoice:vocab.length-vocabWriting,vocabUniqueTargets:new Set(vocab.map(vocabTargetKey)).size,vocabFormats:vocab.reduce((m,q)=>(m[vocabFormat(q)]=(m[vocabFormat(q)]||0)+1,m),{})}};if(unsupported.length)console.warn('[REV46j] unsupported usable questions excluded',unsupported);console.info('[REV46l] manifest',{scope,lesson,total:manifest.total,history:hp.total,recentReused:recentUsed,unseenSelected:unseenUsed,vocab:{total:vocab.length,writing:vocabWriting,choice:vocab.length-vocabWriting,uniqueTargets:new Set(vocab.map(vocabTargetKey)).size},engines:manifest.items.reduce((m,x)=>(m[x.engine]=(m[x.engine]||0)+1,m),{})});return manifest}
...
async function replacementFor(plan,manifest,item){
 ...
 return{position:item.position,lesson,unitId,engine:runtime().engineFor(replacement),trackingPractice:practiceKey(replacement),question:replacement}
}

window.WillenaExamBuilder={buildLesson,buildAll,replacementFor,resolveBook,lessonsFor,blueprint:{...BLUEPRINT},vocabTargetKey,vocabFormat,practiceKey};
console.log('[REV46l] ExamBuilder canonical manifest tracking ready');
})();