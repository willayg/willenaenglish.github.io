...
function hasHangul(rawAnswers){return /[가-힣]/.test(answers(rawAnswers).join(' '))}
function prefersAppKeyboard(rawAnswers){const text=answers(rawAnswers).join(' ');return /[A-Za-z]/.test(text)&&!/[가-힣]/.test(text)}
...
function mount(host,rawAnswers,opts={}){
 ...
 const useAppKeyboard=opts.useAppKeyboard===false?false:prefersAppKeyboard(rawAnswers),korean=hasHangul(rawAnswers);
 const keyboardMode=els=>{for(const el of els){if(!el)continue;el.setAttribute('autocomplete','off');el.setAttribute('autocorrect','off');el.setAttribute('spellcheck','false');if(useAppKeyboard){el.dataset.willenaKeyboard='app';el.setAttribute('inputmode','none');el.setAttribute('autocapitalize','off')}else{el.dataset.willenaKeyboard='native';el.setAttribute('inputmode','text');el.removeAttribute('autocapitalize');if(korean)el.setAttribute('lang','ko')}}};
 if(spec.kind==='multi_correction'){
  const box=document.createElement('div');box.className='wcri-multi-corrections';const fallback=['ⓐ','ⓑ','ⓒ','ⓓ'];
  spec.parsed.forEach((p,i)=>{const row=document.createElement('div');row.className='wcri-multi-row';row.innerHTML=`<div class="wcri-label">${esc(p.label||fallback[i]||i+1)}</div><input class="wcri-input wcri-wrong" autocomplete="off" spellcheck="false" placeholder="틀린 부분"><div class="wcri-multi-arrow">→</div><input class="wcri-input wcri-right" autocomplete="off" spellcheck="false" placeholder="고친 부분">`;box.appendChild(row)});host.appendChild(box);keyboardMode($$('.wcri-input',box));
  ...
  const p=spec.parsed[0],box=document.createElement('div');box.className='wcri-correction';box.innerHTML=`<div class="wcri-cell"><div class="wcri-label">틀린 부분</div><input class="wcri-input wcri-wrong" autocomplete="off" spellcheck="false" placeholder="찾아 쓰기"></div><div class="wcri-arrow">→</div><div class="wcri-cell"><div class="wcri-label">고친 부분</div><input class="wcri-input wcri-right" autocomplete="off" spellcheck="false" placeholder="바르게 고치기"></div>`;host.appendChild(box);const a=$('.wcri-wrong',box),b=$('.wcri-right',box);keyboardMode([a,b]);const sync=()=>{value=serializeCorrection(p,a.value,b.value);emit()};a.addEventListener('input',sync);b.addEventListener('input',sync);a.focus();
 }else if(spec.kind==='multi'){
  const box=document.createElement('div');box.className='wcri-multi';const marks=['ⓐ','ⓑ','ⓒ','ⓓ'];spec.answers.forEach((_,i)=>{const row=document.createElement('div');row.className='wcri-row';row.innerHTML=`<label>${marks[i]||i+1}</label><input class="wcri-input" autocomplete="off" spellcheck="false" placeholder="답 ${i+1}">`;box.appendChild(row)});host.appendChild(box);keyboardMode($$('.wcri-input',box));const sync=()=>{value=$$('.wcri-input',box).map(x=>x.value.trim()).join('\n');emit()};$$('.wcri-input',box).forEach(x=>x.addEventListener('input',sync));$('.wcri-input',box)?.focus();
 }else{
  const ta=document.createElement('textarea');ta.className='wcri-textarea';ta.autocomplete='off';ta.spellcheck=false;ta.placeholder=opts.placeholder||'답을 입력하세요';keyboardMode([ta]);host.appendChild(ta);ta.addEventListener('input',()=>{value=ta.value;emit()});ta.focus();
 }
 return{spec,useAppKeyboard,getValue:()=>value,setDisabled(disabled){$$('input,textarea',host).forEach(x=>x.disabled=!!disabled)},focus(){host.querySelector('input,textarea')?.focus()}};
}
function mountTextarea(textarea,rawAnswers,opts={}){if(!textarea||textarea.dataset.wcriMounted==='1')return null;textarea.dataset.wcriMounted='1';textarea.style.display='none';const host=document.createElement('div');host.className='wcri-host';textarea.parentNode.insertBefore(host,textarea);return mount(host,rawAnswers,{...opts,onChange:(value,spec)=>{textarea.value=value;textarea.dispatchEvent(new Event('input',{bubbles:true}));opts.onChange?.(value,spec)}})}
...
window.WillenaConstructedResponseInput={answers,hasHangul,prefersAppKeyboard,parseCorrection,serializeCorrection,detect,mount,mountTextarea,matches,ensureStyles};
console.log('[REV49h] constructed responses choose Willena or native keyboard by answer language');
})();