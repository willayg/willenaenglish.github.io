...
function mount(host,rawAnswers,opts={}){
 ...
 const appKeyboard=els=>{if(!opts.useAppKeyboard)return;for(const el of els){el.setAttribute('inputmode','none');el.setAttribute('autocomplete','off');el.setAttribute('autocorrect','off');el.setAttribute('autocapitalize','off');el.setAttribute('spellcheck','false')}};
 if(spec.kind==='multi_correction'){
  const box=document.createElement('div');box.className='wcri-multi-corrections';const fallback=['ⓐ','ⓑ','ⓒ','ⓓ'];
  spec.parsed.forEach((p,i)=>{const row=document.createElement('div');row.className='wcri-multi-row';row.innerHTML=`<div class="wcri-label">${esc(p.label||fallback[i]||i+1)}</div><input class="wcri-input wcri-wrong" autocomplete="off" spellcheck="false" placeholder="틀린 부분"><div class="wcri-multi-arrow">→</div><input class="wcri-input wcri-right" autocomplete="off" spellcheck="false" placeholder="고친 부분">`;box.appendChild(row)});host.appendChild(box);appKeyboard($$('.wcri-input',box));
  ...
  const p=spec.parsed[0],box=document.createElement('div');box.className='wcri-correction';box.innerHTML=`<div class="wcri-cell"><div class="wcri-label">틀린 부분</div><input class="wcri-input wcri-wrong" autocomplete="off" spellcheck="false" placeholder="찾아 쓰기"></div><div class="wcri-arrow">→</div><div class="wcri-cell"><div class="wcri-label">고친 부분</div><input class="wcri-input wcri-right" autocomplete="off" spellcheck="false" placeholder="바르게 고치기"></div>`;host.appendChild(box);const a=$('.wcri-wrong',box),b=$('.wcri-right',box);appKeyboard([a,b]);const sync=()=>{value=serializeCorrection(p,a.value,b.value);emit()};a.addEventListener('input',sync);b.addEventListener('input',sync);a.focus();
 }else if(spec.kind==='multi'){
  const box=document.createElement('div');box.className='wcri-multi';const marks=['ⓐ','ⓑ','ⓒ','ⓓ'];spec.answers.forEach((_,i)=>{const row=document.createElement('div');row.className='wcri-row';row.innerHTML=`<label>${marks[i]||i+1}</label><input class="wcri-input" autocomplete="off" spellcheck="false" placeholder="답 ${i+1}">`;box.appendChild(row)});host.appendChild(box);appKeyboard($$('.wcri-input',box));const sync=()=>{value=$$('.wcri-input',box).map(x=>x.value.trim()).join('\n');emit()};$$('.wcri-input',box).forEach(x=>x.addEventListener('input',sync));$('.wcri-input',box)?.focus();
 }else{
  const ta=document.createElement('textarea');ta.className='wcri-textarea';ta.autocomplete='off';ta.spellcheck=false;ta.placeholder=opts.placeholder||'답을 입력하세요';appKeyboard([ta]);host.appendChild(ta);ta.addEventListener('input',()=>{value=ta.value;emit()});ta.focus();
 }
...
console.log('[REV49f] shared constructed-response component can opt into Willena keyboard');
})();