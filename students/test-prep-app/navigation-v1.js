...
function push(state){
 ...
 // Never stack lesson menus on top of other lesson menus.
...
function clickCapture(e){const target=e.target instanceof Element?e.target:null;if(!target)return;
 ...
 if(back){stopEvent(e);smartBack();return}
...
function onPop(e){
 const from=currentState||{tp:'home'};
 let state=e.state?.tp?e.state:{tp:'home'};
 // Clean up old polluted history: leaving one lesson must never reveal another lesson.
 if(from.tp==='lesson'&&state.tp==='lesson'){
  state={tp:'home'};
  history.replaceState(state,'',location.href);
 }
 remember(state);
 let tries=0;
 const go=()=>{if(renderState(state))return;if(++tries<80)setTimeout(go,25)};
 go();
}
