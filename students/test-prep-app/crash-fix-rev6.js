...
function masteryFor(plan,lesson,practice){const v=plan?.summary?.by_lesson_practice?.[`${lesson}||${practice}`]||{},target=Number(CONTENT_TOTALS[plan?.book_label]?.[lesson]?.[practice])||0,done=Math.max(0,Number(v.unique)||0),accuracy=done?Math.max(0,Math.min(100,Number(v.accuracy)||0)):0,coverage=target?Math.min(100,done/target*100):0;return target?Math.round(coverage*accuracy/100):0}
...
function renderJourney(planId,lesson){const home=document.getElementById('assignmentHome'),state=window.WillenaTestPrepAuth?.state,plan=state?.plans?.find(p=>String(p.id)===String(planId));if(!home||!plan)return;const l=scopeFor(plan).find(x=>String(x.lesson)===String(lesson));if(!l)return;const shown=STATIONS.filter(st=>available(plan,l,st));home.style.display='block';home.innerHTML=`<div class="tp-r6-wrap"><button type="button" class="tp-r6-back">← 시험 대비</button><div class="tp-r6-head"><h1>${esc(lesson)}</h1><p>${esc(plan.book_label||'')} · 학습 지도</p></div><div class="tp-r6-route">${shown.map((s,i)=>`<button type="button" class="tp-r6-stop" data-r6-skill="${esc(s.k)}"><span class="tp-r6-dot">${i+1}</span><span class="tp-r6-copy"><b>${esc(s.label)}</b><small>${esc(s.desc)}</small></span><span class="tp-r6-pct">${masteryFor(plan,lesson,s.k)}%</span><span class="tp-r6-arrow">›</span></button>`).join('')}</div></div>`;home.querySelector('.tp-r6-back').onclick=()=>{window.WillenaTestPrepNavigation?.toHome?.({replaceEntry:true})||window.WillenaTestPrepUX?.renderHome?.()};home.querySelectorAll('[data-r6-skill]').forEach(btn=>btn.onclick=()=>window.WillenaTestPrepUX?.launchSkill?.(plan.id,lesson,btn.dataset.r6Skill))}
...
function boot(){addStyles();addBadge();document.addEventListener('click',intercept,true);console.info('[Test Prep] Crash Fix Rev6 active: lightweight journey + plain mastery percentage text')}
