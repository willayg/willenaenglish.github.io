class StudentRewardCelebration extends HTMLElement {
  static get observedAttributes(){return ['percent','stars','points','label'];}
  ...
  render(){
    ...
    this.shadowRoot.innerHTML=`
      ...
      <section class="reward" aria-label="${stars} stars and ${points} points earned">
        <div class="label">${this.escape(label)}</div>
        <div class="percent">${percent}%</div>
        <div class="stars" aria-hidden="true">${starHtml}</div>
        <div class="awards">
          <div class="award"><strong>+${stars}</strong><span>STARS</span></div>
          <div class="award"><strong>+${points}</strong><span>POINTS</span></div>
        </div>
        ${stars===0?'<div class="none">60% 이상이면 별을 받을 수 있어요.</div>':''}
      </section>`;
    this._rendered=true;
  }
