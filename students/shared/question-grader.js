import {FORMS,parseCorrection} from '../test-prep-v2/question-model.js';
...
function metadataAiAllowed(question){const v=question?.metadata?.ai_allowed;return v===true||v==='true'}
function metadataMode(question){return String(question?.metadata?.grading_mode||'')}
function effectiveAiAllowed(question){return question?.grading?.aiAllowed===true||metadataAiAllowed(question)}
function effectiveMode(question){return String(question?.grading?.mode||metadataMode(question)||'exact_normalized')}
...
export async function gradeQuestion(question,response){
  ...
  if(effectiveAiAllowed(question)&&effectiveMode(question)==='ai_semantic_strict'&&aiForm){
    ...
    catch(e){console.warn('[shared grader] AI failed closed',e);return{correct:false,message:'AI 확인에 실패했습니다.',correctAnswer:question.answer,method:'ai_failed'}}
  }
