(function(global){
'use strict';
var VERSION='coach-stage5-skill-capability-v1.1';
...
function listeningEvidence(ctx){var s=stage5(),xs=s&&typeof s.evidence==='function'?s.evidence(ctx):[];return arr(xs).filter(function(x){return x&&x.domain==='listening'&&(x.state==='recurring_weakness'||x.state==='recovering');});}
...
async function buildListening(ctx,target){
  ...
  return{type:'coach_stage5_listening',title:ko()?'듣기 집중 연습':'Listening focus',message:target&&target.state==='recovering'?(ko()?'좋아지고 있어요. 몇 문제만 더 확인해 볼게요.':'You are improving. Let’s confirm it with just a few more listening questions.'):(ko()?'최근 듣기에서 같은 종류의 실수가 반복됐어요. 다른 문제로 짧게 확인해 볼게요.':'The same kind of listening miss has repeated recently. Let’s check it with a few different listening questions.'),items:picked};
}

coach.registerCapability({
  id:'stage5_listening_weakness',
  available:function(ctx){return listeningEvidence(ctx).length>0;},
  score:function(ctx){var x=listeningEvidence(ctx)[0];if(!x)return 0;return x.state==='recovering'?205+Math.min(70,Number(x.score)||0):330+Math.min(120,Number(x.score)||0);},
  label:function(ctx){var x=listeningEvidence(ctx)[0];return x&&x.state==='recovering'?{ko:'듣기 실력 회복 확인',en:'Confirm listening recovery'}:{ko:'반복된 듣기 실수 다시 연습',en:'Practice a recurring listening miss'};},
  response:function(ctx){var x=listeningEvidence(ctx)[0],n=x&&Number(x.count)||0;if(x&&x.state==='recovering')return ko()?'최근 답이 좋아지고 있어요. 아직 완전히 끝내기 전에 몇 문제만 더 확인할 수 있어요.':'Your recent answers are improving. We can check a few more before considering this fully secure.';return ko()?('최근 듣기에서 같은 종류의 실수가 '+n+'번 반복됐어요. 발음 자체가 아니라 듣고 구별하는 부분을 다른 문제로 확인해 볼게요.'):('I found '+n+' recent misses of the same listening type. I’ll check listening discrimination with different items rather than treating it as a pronunciation problem.');},
  actions:function(ctx){var x=listeningEvidence(ctx)[0];return x?[{label:x.state==='recovering'?{ko:'몇 문제 더 확인',en:'Check a few more'}:{ko:'듣기 집중 연습 시작',en:'Start listening focus'},run:function(liveCtx){return buildListening(liveCtx||ctx,x);}}]:[];}
});
