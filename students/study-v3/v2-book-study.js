...
function rowLabel(value,fallback){var s=txt(value).toLowerCase();if(/question|ask|prompt|질문/.test(s))return ko()?'질문':'Question';if(/answer|response|reply|답/.test(s))return ko()?'답변':'Answer';if(/negative|부정/.test(s))return ko()?'부정문':'Negative';if(/statement|affirmative|positive|평서|긍정/.test(s))return ko()?'평서문':'Statement';return fallback||(ko()?'형태':'Pattern');}
...
async function loadUnit(unitId,fallbackLabel){
  ...
  }catch(e){console.warn('[StudyV3 Book Study]',e);if(token===requestToken)state(ko()?'교재 내용을 불러오지 못했어요. 잠시 후 다시 시도해 주세요.':'Could not load the book content. Please try again.');}
}
...
function grammarChart(detail,forms){
  var rows=[],seen={};
  function add(label,value){value=txt(value);if(!value)return;var key=value.toLowerCase();if(seen[key])return;seen[key]=1;rows.push('<div class="book-study-grammar-chart-row"><span>'+esc(label)+'</span><strong>'+esc(value)+'</strong></div>');}
  var prompt=txt(detail&&detail.prompt_pattern),response=txt(detail&&detail.response_pattern);
  if(prompt)add(ko()?'질문':'Question',prompt);
  if(response)add(prompt?(ko()?'답변':'Answer'):(ko()?'형태':'Pattern'),response);
  (forms||[]).forEach(function(f){add(rowLabel(f.label,ko()?'형태':'Pattern'),f.text);});
  return rows.length?'<div class="book-study-grammar-chart">'+rows.join('')+'</div>':'';
}
function grammarSection(data){
  ...
  Object.keys(groups).forEach(function(id){
    var g=groups[id],d=g.detail||{};
    var name=txt(d.name||d.grammar_category||g.label||'Grammar');
    var explanation=txt((ko()?d.explanation_ko:d.explanation_en)||d.explanation_en||d.explanation_ko);
    var uniqueExamples=[],seenExamples={};g.examples.forEach(function(v){var k=v.text.toLowerCase();if(!seenExamples[k]){seenExamples[k]=1;uniqueExamples.push(v);}});
    var uniqueForms=[],seenForms={};g.forms.forEach(function(v){var k=v.text.toLowerCase();if(!seenForms[k]){seenForms[k]=1;uniqueForms.push(v);}});
    var examples=[];for(var i=0;i<uniqueExamples.length;i+=2){var first=uniqueExamples[i],second=uniqueExamples[i+1];examples.push('<div class="book-study-grammar-pair"><div><strong>'+esc(first.text)+'</strong>'+(first.translation?'<small>'+esc(first.translation)+'</small>':'')+'</div>'+(second?'<div><strong>'+esc(second.text)+'</strong>'+(second.translation?'<small>'+esc(second.translation)+'</small>':'')+'</div>':'')+'</div>');}
    var chart=grammarChart(d,uniqueForms);
    if(!chart&&!examples.length&&!explanation)return;
    cards.push('<div class="book-study-grammar"><h5>'+esc(name)+'</h5>'+chart+(explanation?'<p>'+esc(explanation)+'</p>':'')+(examples.length?'<div class="book-study-grammar-examples">'+examples.join('')+'</div>':'')+'</div>');
  });
