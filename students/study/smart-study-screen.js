...
function renderNext(){if(!queue.length){finish();return;}current=queue.shift();answered=false;countEl.textContent=(settled.size+1)+' / '+TARGET;engine=new WillenaActivityEngine(root,{onAnswer:function(){}});engine.setActivity(current);}
...
function finish(){root.innerHTML='<div class="activity-card"><span class="eyebrow">SMART STUDY</span><h2 style="margin:8px 0">오늘 연습 완료</h2><p style="font-weight:700;color:var(--muted)">'+TARGET+'개 목표를 끝냈어요. 틀린 문제는 다시 맞힐 때까지 연습했습니다.</p></div>';countEl.textContent='완료';paint();}
