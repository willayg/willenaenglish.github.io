...
function finish(){root.innerHTML='<div class="activity-card"><span class="eyebrow">SMART STUDY</span><h2 style="margin:8px 0">오늘 연습 완료</h2><p style="font-weight:700;color:var(--muted)">'+TARGET+'개 목표를 끝냈어요. 틀린 문제는 다시 맞힐 때까지 연습했습니다.</p></div>';countEl.textContent='완료';nextBtn.disabled=true;paint();}
async function start(){if(!ready())return;showSmartScreen();root.innerHTML='<div class="study-loading">Smart Study 준비 중…</div>';countEl.textContent='— / '+TARGET;try{pool=[];await buildPool();if(!pool.length)throw new Error('No practice activities');makeSession();}catch(e){root.innerHTML='<div class="study-loading">Smart Study를 불러오지 못했습니다. '+esc(e.message||e)+'</div>';}}
if(!card||!panel)return;card.setAttribute('role','button');card.setAttribute('tabindex','0');card.setAttribute('aria-label','Start Smart Study');card.style.cursor='pointer';card.addEventListener('click',start);card.addEventListener('keydown',function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();start();}});nextBtn.addEventListener('click',renderNext);closeBtn.addEventListener('click',closeSmartScreen);global.addEventListener('willena:activity-answer',onAnswer);var tries=0,t=setInterval(function(){tries++;paint();if(ready()&&tries>5||tries>30)clearInterval(t);},350);window.addEventListener('pageshow',function(){setTimeout(paint,100);});
})(window);