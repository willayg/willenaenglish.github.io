...
function unitReady(){var b=document.getElementById('bookTitle'),u=document.getElementById('unitTitle');return !!(b&&u&&!/^Loading/i.test(b.textContent||'')&&/Unit\s*\d+/i.test(u.textContent||''));}
function updateMeter(){if(!ring)return;var pct=Math.round(dailyCount()/TARGET*100),fill=Math.min(100,pct);ring.style.background='conic-gradient(var(--pink) 0 '+fill+'%,#dff5f7 '+fill+'% 100%)';ring.querySelector('span').textContent=pct+'%';if(title)title.textContent='Smart Study';if(copy)copy.textContent='오늘 목표 '+TARGET+'문항 · 탭해서 시작';}
function claimMeterWhenReady(){var tries=0,timer=setInterval(function(){tries++;if(unitReady())updateMeter();if(unitReady()&&tries>4||tries>30)clearInterval(timer);},400);}
...
function setScreen(on){['.book-hero','.learning-map-section','.section-block','.lower-grid','#studyStats'].forEach(function(sel){var n=document.querySelector(sel);if(n)n.hidden=!!on;});var oldStudy=document.getElementById('studyPanel'),oldPractice=document.getElementById('practicePanel');if(on){if(oldStudy)oldStudy.hidden=true;if(oldPractice)oldPractice.hidden=true;}if(panel)panel.hidden=!on;if(on)panel.scrollIntoView({behavior:'smooth',block:'start'});else{if(oldStudy)oldStudy.hidden=true;if(oldPractice)oldPractice.hidden=true;document.querySelector('.book-hero').scrollIntoView({behavior:'smooth',block:'start'});}}
...
async function start(){try{if(!unitReady())throw new Error('Unit is still loading');setScreen(true);root.innerHTML='<div class="study-loading">Smart Study 준비 중…</div>';countEl.textContent='— / '+TARGET;pool=[];await buildPool();if(!pool.length)throw new Error('No practice activities');makeSession();}catch(err){root.innerHTML='<div class="study-loading">Smart Study를 불러오지 못했습니다. '+esc(err.message)+'</div>';}}
function mount(){card=document.querySelector('.unit-progress-card');ring=document.querySelector('.progress-ring');title=document.getElementById('progressTitle');copy=document.getElementById('progressCopy');panel=document.getElementById('smartPracticePanel');root=document.getElementById('smartActivityRoot');nextBtn=document.getElementById('smartNext');closeBtn=document.getElementById('smartClose');countEl=document.getElementById('smartCount');if(!card||!panel)return;card.setAttribute('role','button');card.setAttribute('tabindex','0');card.setAttribute('aria-label','Start Smart Study');card.style.cursor='pointer';card.addEventListener('click',start);card.addEventListener('keydown',function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();start();}});nextBtn.addEventListener('click',renderNext);closeBtn.addEventListener('click',function(){setScreen(false);updateMeter();});global.addEventListener('willena:activity-answer',onAnswer);claimMeterWhenReady();}
