...
function ensureHost(){var units=document.getElementById('studyHomeUnits');if(!units)return null;var box=document.getElementById('headerSkillMastery');if(box)return box;box=el('div','header-skill-mastery');box.id='headerSkillMastery';var head=el('div','header-skill-mastery-head');head.appendChild(el('strong','','Unit mastery'));head.appendChild(el('span','','Tap a skill to practice'));box.appendChild(head);var grid=el('div','header-skill-mastery-grid');SKILLS.forEach(function(def){var b=el('button','header-skill-master');b.type='button';b.dataset.skill=def[0];b.dataset.label=def[1];var top=el('div','header-skill-master-top');top.appendChild(el('strong','',def[1]));top.appendChild(el('span','header-skill-master-pct','0%'));b.appendChild(top);var track=el('div','header-skill-master-track'),fill=el('i','header-skill-master-fill');fill.style.width='0%';track.appendChild(fill);b.appendChild(track);b.addEventListener('click',function(){launch(def[0],def[1]);});grid.appendChild(b);});box.appendChild(grid);units.appendChild(box);return box;}
