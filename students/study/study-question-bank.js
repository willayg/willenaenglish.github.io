...
async function loadLevel(levelId,context){if(!levelId)return[];var select='id,level_id,difficulty_rating,item_type,prompt_text,context_text,correct_answer,metadata,choices';var path='assessment_items?select='+encodeURIComponent(select)+'&level_id=eq.'+encodeURIComponent(levelId)+'&status=eq.published&is_flagged=eq.false&order=difficulty_rating.asc,source_key.asc';var rows=await get(path);return arr(rows).map(function(r){return mapRow(r,context||{});}).filter(Boolean);}
global.WillenaStudyQuestionBank={version:'authored-bank-v2',loadLevel:loadLevel};
})(window);
