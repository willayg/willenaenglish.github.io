...
function payloadFrom(detail){var activity=detail&&detail.activity||{},result=detail&&detail.result||{},meta=activity.metadata||{},bookId=meta.book_id,unitId=meta.unit_id;if(!bookId||!unitId)return null;var contentType=String(activity.sourceType||'activity'),contentId=activity.sourceId||null;var masteryType=meta.mastery_content_type||contentType,masteryId=meta.mastery_content_id||contentId;if(activity.skill==='grammar'&&meta.pattern_id){masteryType='pattern';masteryId=meta.pattern_id;}var isPreview=previewActive();return{
 ...
 scoring_version:SCORING_VERSION,progress_version:PROGRESS_VERSION,preview_mode:isPreview
 };}
...
async function send(item){return api('?section=study_attempt&_='+Date.now(),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({payload:item.payload})});}
...
async function flush(){if(flushing)return;flushing=true;try{var q=readQueue(),remaining=[];for(var i=0;i<q.length;i++){try{await send(q[i]);}catch(error){remaining.push(q[i]);}}writeQueue(remaining);if(q.length&&!remaining.length)setStatus('Saved queued answers','#23704a');else if(remaining.length)setStatus('Queued '+remaining.length,'#a66a15');else setStatus(previewActive()?'Preview · recording':'Recorder ready','#173f46');}finally{flushing=false;}}
