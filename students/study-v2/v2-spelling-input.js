...
function answerWords(engine,tokens,wordLengths){
  ...
  var total=Array.isArray(tokens)?tokens.length:0,fromAnswer=words.map(function(w){return w.replace(/[^A-Za-zÀ-ÖØ-öø-ÿ'-]/g,'').length;}).filter(function(n){return n>0;});
  var sum=fromAnswer.reduce(function(a,b){return a+b;},0);
  return fromAnswer.length&&sum===total?fromAnswer:[total];
}
...
function install(){
  ...
  if(proto.__v2SpellingInput)return true;
  proto.__v2SpellingInput=true;
  ...
  proto.renderLetterOrder=function(card,tokens,wordLengths){
    cleanup();
    var self=this,chosen=[],bank=(tokens||[]).map(function(t,i){return{text:String(t),id:i+'-'+Math.random()};});
    bank=bank.slice().sort(function(){return Math.random()-.5;});
...
var tries=0,t=setInterval(function(){tries++;if(install()||tries>100)clearInterval(t);},20);
