...
function resolve(v,ctx){return typeof v==='function'?v(ctx||context()):v;}
function t(v,ctx){v=resolve(v,ctx);if(v==null)return'';if(typeof v==='string')return v;return ko()?text(v.ko||v.en):text(v.en||v.ko);}
...
function context(){
  var h=getHome(),books=arr(h&&h.books),wanted=h&&h.activeBookId;
  ...
  if(!unit)return null;
  var publicLevel=Number(book.public_level||book.publicLevel)||0;
  if(!publicLevel&&Number(book.internal_level_id)>2)publicLevel=Number(book.internal_level_id)-2;
  if(publicLevel<1||publicLevel>10)return null;
...
function bubble(kind,msg){var root=transcript();if(!root||!text(msg))return null;var row=document.createElement('div');row.className='study-v2-ai-chat-row is-'+kind;var b=document.createElement('div');b.className='study-v2-ai-chat-bubble';b.textContent=msg;row.appendChild(b);root.appendChild(row);while(root.children.length>8)root.removeChild(root.firstChild);try{root.scrollTop=root.scrollHeight;}catch(_){}return b;}
function renderButtons(buttons){var p=ensureChoices();if(!p)return;p.innerHTML='';var ctx=context();arr(buttons).forEach(function(def){var label=t(def.label,ctx);if(!label)return;var b=document.createElement('button');b.type='button';b.className='study-v2-ai-prompt'+(def.kind==='launch'?' is-launch':'');b.textContent=label;b.addEventListener('click',function(){if(busy)return;Promise.resolve(def.onClick&&def.onClick()).catch(function(e){console.warn('[AI Coach action]',e);bubble('coach',ko()?'문제를 불러오는 중에 문제가 생겼어요. 다시 해볼까요?':'I had trouble loading that. Please try again.');});});p.appendChild(b);});}
...
async function homeCapabilities(){var ctx=context(),ranked=[];if(!ctx)return[];for(var i=0;i<capabilities.length;i++){var cap=capabilities[i];if(!(await available(cap,ctx)))continue;var score=await relevance(cap,ctx);if(score<=0)continue;ranked.push({cap:cap,score:score,index:i});}ranked.sort(function(a,b){return(b.score-a.score)||(a.index-b.index);});return ranked.map(function(x){return x.cap;});}
async function renderHome(resetMessage){
  state={view:'home',capability:null};
  if(resetMessage){var tr=transcript();if(tr)tr.innerHTML='';bubble('coach',ko()?'지금 도움이 될 만한 연습을 골랐어요. 원하는 것을 선택해도 돼요.':'I picked the practice that looks useful right now. Choose any option you want.');}
  var caps=await homeCapabilities();
  if(!caps.length){if(resetMessage)bubble('coach',ko()?'아직 추천할 만큼 학습 정보가 충분하지 않아요.':'I do not have enough study evidence to recommend something yet.');renderButtons([]);return;}
  renderButtons(caps.map(function(cap){return{label:cap.label,onClick:function(){return openCapability(cap);}};}));
}
async function openCapability(cap){if(busy)return;setBusy(true);state={view:'capability',capability:cap.id};var ctx=context();bubble('user',t(cap.label,ctx));var intro=typeof cap.response==='function'?await cap.response(ctx):t(cap.response,ctx);if(intro)bubble('coach',intro);var actions=typeof cap.actions==='function'?await cap.actions(ctx):arr(cap.actions);var buttons=actions.map(function(action){return{label:action.label,kind:(action.provider||action.run)?'launch':null,onClick:function(){return runAction(cap,action);}};});buttons.push({label:{ko:'← 추천으로 돌아가기',en:'← Back to suggestions'},onClick:function(){return renderHome(false);}});renderButtons(buttons);setBusy(false);}
async function runAction(cap,action){if(busy)return;setBusy(true);bubble('user',t(action.label,context()));try{var result;if(typeof action.run==='function')result=await action.run(context());else if(action.provider)result=await provider(action.provider,action.args||{});if(result&&result.message)bubble('coach',t(result.message,context()));if(result&&arr(result.items).length){var plan={type:result.type||cap.id,title:t(result.title||cap.label,context()),items:result.items};renderButtons([{label:result.startLabel||{ko:result.items.length+'문제 시작하기',en:'Start '+result.items.length+' questions'},kind:'launch',onClick:function(){launch(plan);}},{label:{ko:'← 추천으로 돌아가기',en:'← Back to suggestions'},onClick:function(){return renderHome(false);}}]);}else if(result&&arr(result.actions).length){var extra=result.actions.map(function(a){return{label:a.label,kind:a.kind||((a.provider||a.run)?'launch':null),onClick:function(){return runAction(cap,a);}};});extra.push({label:{ko:'← 추천으로 돌아가기',en:'← Back to suggestions'},onClick:function(){return renderHome(false);}});renderButtons(extra);}else{renderButtons([{label:{ko:'← 추천으로 돌아가기',en:'← Back to suggestions'},onClick:function(){return renderHome(false);}}]);}if(action.after)await action.after(result,context());}catch(e){console.warn('[AI Coach]',e);bubble('coach',ko()?'지금은 그 연습을 만들 수 없어요. 다른 추천을 골라 볼까요?':'I could not build that practice right now. Try another suggestion?');await renderHome(false);}finally{setBusy(false);}}
...
function bind(){
  clearLegacy();ensureChoices();
  var hero=document.getElementById('practiceHeroBtn');
  if(hero)hero.addEventListener('click',function(){var shell=document.getElementById('aiRecommendations');if(shell)try{shell.scrollIntoView({behavior:'smooth',block:'start'});}catch(_){}renderHome(false);});
  var lang=document.getElementById('languageBtn');
  if(lang)lang.addEventListener('click',function(){setTimeout(function(){if(state.view==='home')renderHome(false);},30);},true);
  global.addEventListener('willena:study-recording',function(){if(state.view==='home')setTimeout(function(){renderHome(false);},250);});
  global.addEventListener('willena:morphology-updated',function(){if(state.view==='home')setTimeout(function(){renderHome(false);},120);});
}

if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',bind,{once:true});else bind();
global.WillenaAICoach={version:'coach-controller-v1-clean',registerCapability:registerCapability,registerProvider:registerProvider,refresh:function(){return renderHome(false);},home:renderHome,context:context,launch:launch,getSuggestions:homeCapabilities,getState:function(){return Object.assign({},state);}};
})(window);
