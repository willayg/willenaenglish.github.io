...
function renderButtons(buttons){var p=ensureChoices();if(!p)return;p.innerHTML='';arr(buttons).forEach(function(def){var b=document.createElement('button');b.type='button';b.className='study-v2-ai-prompt';b.textContent=t(def.label);b.addEventListener('click',function(){if(busy)return;Promise.resolve(def.onClick&&def.onClick()).catch(function(e){console.warn('[AI Coach action]',e);bubble('coach',ko()?'문제를 불러오는 중에 문제가 생겼어요. 다시 해볼까요?':'I had trouble loading that. Please try again.');});});p.appendChild(b);});}
