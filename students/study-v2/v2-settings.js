...
function load(){
  try{
    ...
    return {theme:THEMES[saved.theme]?saved.theme:DEFAULTS.theme,sounds:typeof saved.sounds==='boolean'?saved.sounds:DEFAULTS.sounds};
  }catch(_){return {theme:DEFAULTS.theme,sounds:DEFAULTS.sounds};}
}
...
function motifSvg(kind,color,small){
  var svg='';
  if(kind==='bolt')svg="<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><path d='M58 7 24 55h24l-8 38 36-52H52z' fill='none' stroke='"+color+"' stroke-opacity='"+(small?'.14':'.18')+"' stroke-width='7' stroke-linejoin='round'/></svg>";
  else if(kind==='rocket')svg="<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><path d='M58 14c16 4 25 13 28 29-14 3-25 10-34 20L37 78 22 63l15-15c10-9 17-20 21-34Z' fill='none' stroke='"+color+"' stroke-opacity='"+(small?'.14':'.18')+"' stroke-width='6' stroke-linejoin='round'/><circle cx='64' cy='36' r='7' fill='none' stroke='"+color+"' stroke-opacity='"+(small?'.14':'.18')+"' stroke-width='5'/><path d='m29 70-9 15 15-9M42 57l-18-3 7-10M55 70l3 18 10-8' fill='none' stroke='"+color+"' stroke-opacity='"+(small?'.14':'.18')+"' stroke-width='5' stroke-linecap='round'/></svg>";
  else if(kind==='fire')svg="<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><path d='M54 10c4 18-5 24-3 38 2-6 8-10 13-15 3 8 14 19 14 32 0 17-13 28-29 28S20 82 20 65c0-18 14-27 23-40 4-6 8-11 11-15Z' fill='none' stroke='"+color+"' stroke-opacity='"+(small?'.14':'.18')+"' stroke-width='6' stroke-linejoin='round'/><path d='M50 55c6 8 11 13 11 21 0 7-5 12-12 12S37 83 37 76c0-7 6-13 13-21Z' fill='none' stroke='"+color+"' stroke-opacity='"+(small?'.14':'.18')+"' stroke-width='5'/></svg>";
  else svg="<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><path d='M50 5 59 38 92 50 59 62 50 95 41 62 8 50 41 38Z' fill='none' stroke='"+color+"' stroke-opacity='"+(small?'.14':'.18')+"' stroke-width='4' stroke-linejoin='round'/></svg>";
  return 'url("data:image/svg+xml,'+encodeURIComponent(svg)+'")';
}
function applyTheme(){
  var t=THEMES[state.theme]||THEMES.pink;
  ...
  [shell,card].forEach(function(el){
    el.style.setProperty('--study-theme-accent',t.accent);
    el.style.setProperty('--study-theme-soft',t.soft);
    el.style.setProperty('--study-theme-ring',t.ring);
    el.style.setProperty('--study-theme-glow',t.glow);
    el.style.setProperty('--study-theme-shadow',t.shadow);
    el.style.setProperty('--study-theme-badge',t.badge);
    el.style.setProperty('--study-theme-badge-text',t.badgeText);
    el.style.setProperty('--study-theme-art-large',motifSvg(t.motif,t.accent,false));
    el.style.setProperty('--study-theme-art-small',motifSvg(t.motif,t.accent,true));
  });
  document.querySelectorAll('.study-theme-choice').forEach(function(btn){
...
function setupAudio(){
  try{
    rightAudio=new Audio('/Games/english_arcade/assets/audio/right-answer.mp3');
    wrongAudio=new Audio('/Games/english_arcade/assets/audio/wrong-answer.mp3');
    [rightAudio,wrongAudio].forEach(function(a){a.preload='auto';a.volume=.45;});
  }catch(_){}
}
function playAnswerSound(correct){
  if(!state.sounds)return;
  var a=correct?rightAudio:wrongAudio;
  if(!a)return;
  try{a.pause();a.currentTime=0;a.volume=.45;var p=a.play();if(p&&p.catch)p.catch(function(){});}catch(_){}
}
function syncCopy(){
  var ko=korean(),open=document.getElementById('studySettingsBtn'),title=document.getElementById('studySettingsTitle'),themeLabel=document.getElementById('studyThemeLabel'),soundLabel=document.getElementById('studySoundLabel'),soundState=document.getElementById('studySoundState'),toggle=document.getElementById('studySoundToggle');
  if(open)open.textContent=ko?'설정':'Settings';
  ...
  if(toggle){toggle.setAttribute('aria-label',ko?'정답 효과음 켜기/끄기':'Turn answer sounds on or off');toggle.setAttribute('aria-pressed',state.sounds?'true':'false');}
}
...
function buildUi(){
  ...
  back.querySelector('.study-settings-close').addEventListener('click',closeSettings);
  back.addEventListener('click',function(e){if(e.target===back)closeSettings();});
  document.addEventListener('keydown',function(e){if(e.key==='Escape'&&!back.hidden)closeSettings();});
  back.querySelectorAll('.study-theme-choice').forEach(function(btn){btn.addEventListener('click',function(){state.theme=btn.dataset.theme;save();applyTheme();});});
  back.querySelector('#studySoundToggle').addEventListener('click',function(){state.sounds=!state.sounds;save();syncCopy();});
  var lang=document.getElementById('languageBtn');if(lang)lang.addEventListener('click',function(){setTimeout(syncCopy,0);});
  syncCopy();applyTheme();
}
function bind(){
  buildUi();setupAudio();applyTheme();
