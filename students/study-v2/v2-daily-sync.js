...
function wrapDaily(){var d=global.WillenaStudyV2Daily;if(!d||d.__serverSyncWrapped)return false;d.__serverSyncWrapped=true;var originalOpen=d.open;d.open=async function(){await pull();return originalOpen.apply(d,arguments);};return true;}
function bind(){var tries=0,t=setInterval(function(){tries++;if(wrapDaily()||tries>100)clearInterval(t);},30);global.addEventListener('willena:activity-answer',function(){if(document.body.classList.contains('study-v2-daily-mode'))setTimeout(function(){push();},80);});document.addEventListener('click',function(e){if(e.target&&e.target.closest&&e.target.closest('#v2PracticeClose,#v2DailyHome'))setTimeout(function(){push();},50);},true);window.addEventListener('pagehide',function(){push();});setTimeout(pull,250);}
