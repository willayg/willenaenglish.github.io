...
async function loadSentences(unitId){var occ=arr(await get('source_content_occurrences?select=id,sentence_id,source_text,skill,activity_label&unit_id=eq.'+encodeURIComponent(unitId)+'&occurrence_type=eq.sentence&status=in.(review,published)'));var ids=unique(occ.map(function(o){return o.sentence_id;}));if(!ids.length)return[];var rows=arr(await get('sentences?select=id,text,translation_ko&id=in.'+encodeURIComponent('('+ids.join(',')+')')+'&status=in.(review,published)')),by={};rows.forEach(function(r){by[r.id]=r;});return occ.map(function(o){var s=by[o.sentence_id];return s?{id:s.id,occurrenceId:o.id,text:text(s.text||o.source_text),ko:text(s.translation_ko),skill:text(o.skill),label:text(o.activity_label)}:null;}).filter(Boolean);}
async function loadPatterns(unitId){var occ=arr(await get('source_content_occurrences?select=id,pattern_id&unit_id=eq.'+encodeURIComponent(unitId)+'&occurrence_type=eq.pattern&status=in.(review,published)'));var ids=unique(occ.map(function(o){return o.pattern_id;}));if(!ids.length)return[];var rows=arr(await get('patterns?select=id,prompt_pattern,response_pattern,explanation_ko&id=in.'+encodeURIComponent('('+ids.join(',')+')')+'&status=in.(review,published)')),by={};rows.forEach(function(r){by[r.id]=r;});return occ.map(function(o){var p=by[o.pattern_id];return p?Object.assign({},p,{occurrenceId:o.id}):null;}).filter(Boolean);}
async function loadVocabulary(unitId){var occ=arr(await get('source_content_occurrences?select=id,lexical_entry_id,source_text&unit_id=eq.'+encodeURIComponent(unitId)+'&occurrence_type=eq.lexical_entry&status=in.(review,published)'));var ids=unique(occ.map(function(o){return o.lexical_entry_id;}));if(!ids.length)return[];var rows=arr(await get('lexical_entries?select=id,canonical_text,translation_ko&id=in.'+encodeURIComponent('('+ids.join(',')+')')+'&status=in.(review,published)')),by={};rows.forEach(function(r){by[r.id]=r;});return occ.map(function(o){var e=by[o.lexical_entry_id];return e?{id:e.id,occurrenceId:o.id,word:text(e.canonical_text||o.source_text),ko:text(e.translation_ko)}:null;}).filter(function(x){return x&&x.word;});}
...
function ensureMinimum(items,min){if(!items.length)return items;var base=items.slice(),i=0;while(items.length<min){var source=base[i%base.length],copy=Object.assign({},source,{id:source.id+'-repeat-'+items.length,metadata:Object.assign({},source.metadata||{},{generated_repeat:true})});items.push(copy);i++;}return items;}
...
if(out.length<10)patterns.forEach(function(p){[text(p.prompt_pattern),firstAlternative(p.response_pattern)].forEach(function(value,j){if(!cleanSentence(value))return;out.push({id:'v2-sentence-pattern-'+p.occurrenceId+'-'+j,sourceType:'pattern',sourceId:p.id,skill:'sentence_building',usage:['practice'],stimulus:{type:'text',prompt:text(p.explanation_ko)||'단어를 올바른 순서로 배열하세요.',context:'단어를 올바른 순서로 배열하세요.'},response:{type:'token_order',tokens:sentenceTokens(value)},answer:value,metadata:meta(context,p.occurrenceId,'Focused unit practice')});});});
return ensureMinimum(out,10);
}
...
global.WillenaStudyQuestionBank.loadUnit=async function(publicLevel,context){
  context=context||{};
  var rows=arr(await originalLoadUnit(publicLevel,context));
