...
function weakSkills(ctx){
  ...
  base.forEach(function(x){if(x.skill)by[x.skill]={skill:x.skill,pct:x.pct,attempts:x.attempts,misses:Number(misses[x.skill])||0};});
  Object.keys(misses).forEach(function(skill){if(!by[skill])by[skill]={skill:skill,pct:100,attempts:0,misses:Number(misses[skill])||0};});
  var ranked=Object.keys(by).map(function(k){var x=by[k];x.effective=Math.min(x.pct,x.misses>=2?Math.max(25,90-x.misses*10):x.pct);x.location=locationForSkill(x.skill,ctx);return x;})
    .filter(function(x){return x.skill&&((x.attempts>0&&x.pct<80)||x.misses>=2);})
    .sort(function(a,b){return(a.effective-b.effective)||(b.misses-a.misses)||(b.attempts-a.attempts)||a.skill.localeCompare(b.skill);});
  if(ranked.length)return ranked.slice(0,3);
  var fallback=base.filter(function(x){return x.skill&&x.attempts>0;}).sort(function(a,b){return(a.pct-b.pct)||(b.attempts-a.attempts)||a.skill.localeCompare(b.skill);});
  if(!fallback.length||fallback[0].pct>=90)return[];
  return[{skill:fallback[0].skill,pct:fallback[0].pct,attempts:fallback[0].attempts,misses:0,effective:fallback[0].pct,location:locationForSkill(fallback[0].skill,ctx)}];
}
...
coach.registerCapability({
  ...
  actions:function(ctx){
    ...
    weakSkills(ctx).forEach(function(x){
      var loc=x.location,where=locationLabel(loc);
      if(loc)out.push({label:{ko:(where?where+' · ':'')+skillNameFor('ko',x.skill)+' 공부하기',en:'Study '+skillNameFor('en',x.skill)+(where?' · '+where:'')},provider:'studyNavigation',args:{bookId:loc.bookId,unitId:loc.unitId,skill:x.skill}});
