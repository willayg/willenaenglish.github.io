...
var VERSION='coach-diagnosis-v1.2';
...
function tokenize(v){return expandContractions(v).replace(/[“”"'.,!?;:()[\]{}]/g,' ').split(/\s+/).filter(Boolean);}
function words(v){if(Array.isArray(v)){var out=[];v.forEach(function(x){out=out.concat(tokenize(x));});return out;}return tokenize(v);}
...
function responseType(activity){return lower(activity&&activity.response&&activity.response.type||activityMeta(activity).response_type||activityMeta(activity).question_form||'');}
...
function structural(activity,result){
  ...
  var sw=words(selected),aw=words(answer),skill=skillOf(activity),concept=conceptText(activity),prompt=promptText(activity),rtype=responseType(activity);
  var s=sw.join(' '),a=aw.join(' ');
  if(!a)return payload();

  var orderingActivity=Array.isArray(selected)||/sentence|order|build|unscram|reorder|token/.test(skill+' '+concept+' '+rtype);
  if(orderingActivity&&sameBag(selected,answer)&&s!==a)return exact('grammar','word_order','word_order','Sentence word order',.98);
