...
function paint(){
  ...
  if(ring&&ring.style.getPropertyValue('--progress')!==String(pct))ring.style.setProperty('--progress',pct);
  var numberText=done>=TARGET?'✓':done+'/'+TARGET;
  if(num&&num.textContent!==numberText)num.textContent=numberText;
  var titleText=langKo()?'오늘의 학습':'Daily Study';
  if(title&&title.textContent!==titleText)title.textContent=titleText;
  var copyText=done>=TARGET?(langKo()?'오늘 목표 완료 ✓':'Daily goal complete ✓'):(langKo()?'오늘 목표 · '+done+'/'+TARGET:'Today · '+done+'/'+TARGET);
  if(copy&&copy.textContent!==copyText)copy.textContent=copyText;
}

function sessionFrom(data){
  if(data&&data.session){session=data.session;paint();return session;}
  return null;
}
...
async function syncCard(){
  ...
  syncing=(async function(){
    try{
      ...
      if(data&&data.session)sessionFrom(data);else{session=null;paint();}
      return data;
    }catch(e){console.warn('[Daily Study] sync',e);return null;}
...
async function adaptive(){
  try{
    if(global.WillenaStudyProgress&&typeof global.WillenaStudyProgress.getAdaptiveState==='function')return await global.WillenaStudyProgress.getAdaptiveState();
  }catch(e){console.warn('[Daily Study] adaptive state unavailable',e);}
  return{items:[]};
}
...
async function loadUnitPool(book,unit,kind){
  ...
  return arr(rows).filter(validActivity).map(function(source){
    ...
    a.metadata=Object.assign({},a.metadata||{}, {
      ...
      daily_source:kind||'current',
      current_curriculum:kind!=='review',
      adaptive_review:kind==='review'
    });
    return a;
  });
}
...
async function loadBookPool(book,state){
  ...
  var dueIds=[],seen={};
  arr(state&&state.items).filter(function(x){
    return String(x.book_id)===String(book.book_id)&&String(x.unit_id)!==String(book.currentUnit.id)&&(x.due||Number(x.lapses||0)>0);
  }).sort(function(a,b){
    var al=Number(a.lapses||0),bl=Number(b.lapses||0);
    if(al!==bl)return bl-al;
    return Number(a.mastery_score||0)-Number(b.mastery_score||0);
  }).forEach(function(x){
    var id=String(x.unit_id||'');
    if(id&&!seen[id]&&dueIds.length<2){seen[id]=true;dueIds.push(id);}
  });
  var units=arr(book.units);
  var reviewUnits=dueIds.map(function(id){return units.find(function(u){return String(u.id)===id;});}).filter(Boolean);
    ...
    for(var s=0;s<SKILLS.length&&out.length<MAX_CANDIDATES;s++){
      for(var b=0;b<books.length&&out.length<MAX_CANDIDATES;b++){
        ...
        while(bucket.length){
          ...
          used[k]=true;out.push(a);progress=true;break;
        }
      }
    }
  }
...
async function buildPlan(){
  ...
  var state=await adaptive();
  var groups=await Promise.all(books.map(function(b){return loadBookPool(b,state);}));
  groups=groups.filter(function(g){return g.length;});
  if(!groups.length)throw new Error('No Daily Study questions are available.');
  var plan=balancedCandidates(groups);
  if(plan.length<TARGET)throw new Error('Daily Study needs at least 20 unique questions.');
  return plan;
}
...
async function ensureSession(){
  ...
  var created=await request('POST',{action:'create',plan:plan});
...
function currentItem(){
  ...
  var cursor=Number(session.cursor)||0,plan=arr(session.plan);
  return cursor>=0&&cursor<plan.length?clone(plan[cursor]):null;
}
...
function sourceBadge(a){
  if(!root)return;
  var c=root.querySelector('.activity-card');if(!c)return;
  var old=c.querySelector('.activity-source-badge');if(old)old.remove();
  var m=a.metadata||{},b=document.createElement('div');b.className='activity-source-badge';
  b.textContent=(m.daily_book_title||'Study')+' · Unit '+(m.daily_unit_number||'');
  c.insertBefore(b,c.firstChild);
}

function prepareActivity(item){
  var a=clone(item),key=dailyKey(a);
  a.daily_key=key;
  a.metadata=Object.assign({},a.metadata||{}, {
    daily_mode:true,
    daily_date:dateKey(),
...
function showCurrent(){
  ...
  if(!item){
    if(root)root.innerHTML='<div class="smart-finish"><h2>'+(langKo()?'오늘의 문제를 모두 사용했어요.':'Daily question pool exhausted.')+'</h2><p>'+(langKo()?'담당 선생님에게 알려 주세요.':'Please tell your teacher.')+'</p></div>';
    return;
  }
  ...
  engine.setActivity(current);sourceBadge(current);
  if(panel)panel.scrollTop=0;
}

function replaceCheck(label,handler){
  var check=root&&root.querySelector('.activity-check');if(!check)return;
  var b=check.cloneNode(true);b.disabled=false;b.textContent=label;check.replaceWith(b);b.addEventListener('click',handler,{once:true});
}

async function submitAnswer(correct){
  var key=dailyKey(current);
  var data=await request('POST',{action:'answer',daily_key:key,correct:!!correct});
  if(data&&data.stale){sessionFrom(data);showCurrent();return;}
  if(!data||data.success===false)throw new Error(data&&data.error||'Daily answer was not saved.');
  sessionFrom(data);
}
...
async function onAnswer(e){
  ...
  try{
    ...
    console.warn('[Daily Study] answer save',error);
    answerLocked=false;
...
function finish(){
  ...
  document.body.classList.add('study-v2-practice-mode','study-v2-daily-mode');
  if(panel)panel.hidden=false;
  if(countEl)countEl.textContent=langKo()?'완료':'Done';
  if(skillEl)skillEl.textContent=langKo()?'오늘의 학습':'DAILY STUDY';
  if(titleEl)titleEl.textContent=langKo()?'오늘 목표 완료':'Daily goal complete';
  if(root)root.innerHTML='<div class="smart-finish"><div class="smart-confetti">✓</div><h2>'+(langKo()?'잘했어요!':'Great work!')+'</h2><p>'+(langKo()?'오늘의 20개 학습 목표를 모두 맞혔어요.':'You got all 20 Daily Study targets correct.')+'</p><button id="v2DailyHome" class="primary-button smart-home-button" type="button">'+(langKo()?'돌아가기':'Back to Study')+'</button></div>';
  var b=document.getElementById('v2DailyHome');if(b)b.addEventListener('click',close,{once:true});
}

function close(){
  document.body.classList.remove('study-v2-practice-mode','study-v2-daily-mode');
  if(panel)panel.hidden=true;if(root)root.innerHTML='';
  current=null;engine=null;paint();window.scrollTo({top:0,behavior:'auto'});
}

async function open(){
  if(loading)return;loading=true;
  try{await ensureSession();showCurrent();}
  catch(e){console.warn('[Daily Study] open',e);if(countEl)countEl.textContent=langKo()?'잠시 후 다시 시도하세요.':'Please try again shortly.';}
...
function bind(){
  if(card){
    ...
    var observer=new MutationObserver(function(){
      var done=resolvedCount(),expected=done>=TARGET?'✓':done+'/'+TARGET;
      var num=document.getElementById('smartDailyPct');
      if(num&&num.textContent!==expected)paint();
    });
    observer.observe(card,{childList:true,subtree:true,characterData:true,attributes:true,attributeFilter:['style']});
  }
  global.addEventListener('willena:activity-answer',onAnswer);
  global.addEventListener('willena:study-progress-updated',function(){setTimeout(paint,0);});
  ...
  document.addEventListener('click',function(e){if(e.target&&e.target.closest&&e.target.closest('[data-book-index],[data-unit-id]'))setTimeout(paint,220);},true);
