// Simple V1-derived 본문외우기 module for Test Prep V2.
// Intentionally self-contained: V1 content source + sentence unscramble + V2 tracking.
...
function splitSentences(body,title,passageId,translations=[]){
  ...
  for(let line of String(body||'').split(/\n+/).map(x=>x.trim()).filter(Boolean)){
    ...
    let speaker='';const m=line.match(/^([A-Za-z][A-Za-z .'-]{0,24}):\s*(.+)$/);
    if(m){speaker=m[1];line=m[2]}
    const parts=protectDots(line).match(/[^.!?]+[.!?]+(?:["'”’])?|[^.!?]+$/g)||[];
    parts.map(x=>restoreDots(x.trim())).filter(x=>x&&/[A-Za-z]/.test(x)).forEach((text,i)=>{
      out.push({text,ko:String(translations[sentenceIndex]||''),speaker,passageTitle:title,passageId,partIndex:i,sentenceIndex});
      sentenceIndex++;
    });
  }
  return out;
...
async function loadItems(unitId){
  ...
  return (rows||[]).flatMap(p=>splitSentences(p.body,p.title,p.id,Array.isArray(p.metadata?.sentence_translations_ko)?p.metadata.sentence_translations_ko:[]));
}
function uid(q){
  const s=`${q.passageId}|${q.partIndex}|${q.text}`;let h1=2166136261,h2=2246822519,h3=3266489917,h4=668265263;
  for(let i=0;i<s.length;i++){const c=s.charCodeAt(i);h1=Math.imul(h1^c,16777619);h2=Math.imul(h2^c,1597334677);h3=Math.imul(h3^c,374761393);h4=Math.imul(h4^c,1103515245)}
...
function render(){
  ...
  const answer=ctx.host.querySelector('#spv1Answer'),bankEl=ctx.host.querySelector('#spv1Bank'),check=ctx.host.querySelector('#spv1Check'),feedback=ctx.host.querySelector('#spv1Feedback');
  ...
  ctx.host.querySelector('#spv1Clear').onclick=()=>{if(ctx.checked)return;const all=shuffle([...answer.children,...bankEl.children]);bankEl.innerHTML='';all.forEach(b=>bankEl.appendChild(b));update()};
  check.onclick=async()=>{
    ...
    ctx.checked=true;feedback.className=`spv1-feedback ${ok?'ok':'bad'}`;feedback.textContent=ok?'정답입니다!':`정답: ${q.text}`;check.disabled=false;check.textContent=ctx.index===ctx.queue.length-1?'끝내기':'다음 문장';
  };
  ctx.host.querySelector('#spv1Back').onclick=async()=>{const onExit=ctx?.onExit;await stopSentencePracticeV1();onExit?.()};
}
async function finish(){
  if(!ctx)return;
  const {host,score,answered,onExit,wrongIds}=ctx;
  try{await completeSession({correct:score,total:answered,wrongIds})}catch(e){console.warn('[sentence-v1] session close failed',e)}
  if(!ctx)return;ctx.sessionOpen=false;
  host.innerHTML=`<div class="spv1"><div class="spv1-result"><div class="spv1-score">${score}/${Math.max(1,answered)}</div><h2>본문외우기 완료</h2><p>이번 라운드를 끝냈어요.</p><button id="spv1Done" type="button">${esc(ctx.lesson)}로 돌아가기</button></div></div>`;
  host.querySelector('#spv1Done').onclick=()=>{ctx=null;onExit?.()};
}
...
export async function stopSentencePracticeV1(){
  if(!ctx)return;
  const active=ctx;ctx=null;
  if(active.sessionOpen){try{await completeSession({correct:active.score,total:active.answered,wrongIds:active.wrongIds})}catch(e){console.warn('[sentence-v1] close failed',e)}}
}
