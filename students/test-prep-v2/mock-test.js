...
function beforeUnload(event){if(!activeExam||activeExam.finished)return;event.preventDefault();event.returnValue=''}
function clearExamGuards(){try{clearNavigationGuard?.()}catch(_){}clearNavigationGuard=null;window.removeEventListener('beforeunload',beforeUnload)}
...
function formatTime(ms){const total=Math.max(0,Math.ceil(ms/1000)),minutes=Math.floor(total/60),seconds=total%60;return`${String(minutes).padStart(2,'0')}:${String(seconds).padStart(2,'0')}`}
...
function showQuestion(index){
  ...
  const current=ensureQuestionPanel(exam.index);for(const panel of exam.panels.values())panel.hidden=panel!==current;
  updateExamChrome();exam.host.scrollTo?.({top:0,behavior:'auto'});
}
...
async function persistResults(snapshot,exam){
  ...
  for(const item of snapshot.items){const practiceType=practiceTypeFor(item),key=`${item.lesson}\u0000${practiceType}`;if(!groups.has(key))groups.set(key,{lesson:item.lesson,practiceType,items:[]});groups.get(key).items.push(item)}
  for(const group of groups.values()){
    setTrackingContext(exam.plan,group.lesson);await startSession(group.practiceType);
    ...
    for(const item of group.items){
      ...
      await recordAttempt({question:item.question,response:item.response,result:item.result,practiceType:group.practiceType,skipped:!item.answered,source:'mock-test',metadata:{mock_test:true,mock_seed:snapshot.seed,mock_question_number:item.number,mock_bucket:item.bucket,mock_slot:item.slot,lesson:item.lesson,unit_id:item.unitId}});
    }
    await completeSession({correct,total:group.items.length,wrongIds:[...new Set(wrongIds)]});
  }
  setTrackingContext(exam.plan,'실전모의고사');await startSession('mock_test');await completeSession({correct:snapshot.correct,total:snapshot.total,wrongIds:[]});
  await refreshTrackingState();invalidateCardStats(exam.plan.id);await loadCardStats(exam.plan,exam.studentId,{force:true});
}
...
export async function renderMockTestPreflight({host,plan,studentId=null,onBack=()=>{}}={}){
  ...
  if(activeExam)stopMockTest();currentPlanId=String(plan.id);const token=++activeToken;
...
export function stopMockTest(){
  const planId=activeExam?.plan?.id||currentPlanId;activeToken++;
