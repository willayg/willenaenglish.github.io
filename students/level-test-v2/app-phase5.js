...
import {createSpeakingAssessment} from './speaking-assessment.js?v=20260912-phase5q3';
import {
  SESSION_PHASE,
  SESSION_STATUS,
...
function renderWelcome(){document.body.classList.add('welcome-mode');const resumable=isResumableSession(session);screen(`<div class="welcome-layout"><img class="welcome-logo" src="/Assets/Images/Logo.png?v=20260912-phase5q3" alt="Willena English Academy"><div class="welcome-panel"><h1>${tx('title')}</h1>${resumable?`<div class="welcome-resume-stack"><button class="welcome-start" id="resumeTest" type="button">${tx('resume')}</button><button class="welcome-new-test" id="newTest" type="button">${tx('newTest')}</button></div>`:`<button class="welcome-start" id="welcomeStart" type="button">${tx('start')}</button>`}</div></div>`);}
