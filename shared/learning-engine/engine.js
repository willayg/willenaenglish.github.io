...
ActivityEngine.prototype.setCheckEnabled=function(card,enabled){var check=card.querySelector('.activity-check');if(check)check.disabled=!enabled;};
ActivityEngine.prototype.renderChoices=function(card,choices){
 var wrap=el('div','activity-choices'),self=this;
 shuffle(choices).forEach(function(choice){var b=el('button','activity-choice',String(choice));b.type='button';b.addEventListener('click',function(){wrap.querySelectorAll('button').forEach(function(x){x.classList.remove('is-selected');});b.classList.add('is-selected');self.selected=choice;self.setCheckEnabled(card,true);});wrap.appendChild(b);});
 card.appendChild(wrap);
};
ActivityEngine.prototype.renderTextInput=function(card,type){
 var input=el('input','activity-input');input.type='text';input.autocomplete='off';input.setAttribute('autocorrect','off');input.setAttribute('autocapitalize','none');input.spellcheck=false;input.placeholder=type==='gap_fill_text'?'Type the missing word or phrase':'Type your answer';var self=this;input.addEventListener('input',function(){self.selected=input.value;self.setCheckEnabled(card,!!input.value.trim());});input.addEventListener('keydown',function(e){var check=card.querySelector('.activity-check');if(e.key==='Enter'&&check&&!check.disabled)check.click();});card.appendChild(input);
};
...
ActivityEngine.prototype.renderLetterOrder=function(card,tokens,wordLengths){
 ...
 function drawSlots(){slots.innerHTML='';var cursor=0;lengths.forEach(function(length,wordIndex){var row=el('div','activity-letter-word');for(var i=0;i<Number(length||0);i++){var slot=el('button','activity-letter-slot',chosen[cursor]?chosen[cursor].text.toUpperCase():'');slot.type='button';slot.disabled=!chosen[cursor];(function(index){slot.addEventListener('click',function(){if(chosen[index]){chosen.splice(index,1);draw();}});})(cursor);row.appendChild(slot);cursor++;}slots.appendChild(row);if(wordIndex<lengths.length-1)slots.appendChild(el('span','activity-letter-space',' '));});}
